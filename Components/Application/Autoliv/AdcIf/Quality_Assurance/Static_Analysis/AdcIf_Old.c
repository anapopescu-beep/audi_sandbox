...
/**
 * \brief
 *      End of converison OS alarm period.
 *      Based on timer configuration of FTM0_CH0, System clock is at 80Mhz and we have an prescaler of 16 =>
 *      => we have a timer clock of 5 Mhz => 1 tick is 2uS.
 * \remark
 *      5000 => 1mS interrupt.
 */
#define KU32_FTM_0_CH_0_RST_VALUE           ((uint32) 5000)
...
EXPORTED void AdcIf_StartConversion(void)
{
   ...
   Gpt_StartTimer(GPT_ADC_TIMER, KU32_FTM_0_CH_0_RST_VALUE);
}
...
*       This service behaves like an interrupt that is called by GPT timer every 500uS.
...
EXPORTED void AdcIf_runInit(void)
{
   ...
   Gpt_Init(&GptChannelConfigSet_TDK_4G);

   /* Enable Notification function on GPT timer on LPIT Channel 1 */
   Gpt_EnableNotification(GPT_ADC_TIMER);
...
EXPORTED void AdcIf_Calib(void)
{
   ...
   ADCIF_stCalibrationStatus0.Adc_UnitSelfTestStatus = (Std_ReturnType)E_NOT_OK;
   ADCIF_stCalibrationStatus1.Adc_UnitSelfTestStatus = (Std_ReturnType)E_NOT_OK;

   for (u8CalibAdcIndex = KU8_ZERO;
      (u8CalibAdcIndex < KU8_ADCIF_CALIBRATE_INDEX_MAX) &&
      (
         (E_NOT_OK == ADCIF_stCalibrationStatus0.Adc_UnitSelfTestStatus) ||
         (E_NOT_OK == ADCIF_stCalibrationStatus1.Adc_UnitSelfTestStatus)
      );
      u8CalibAdcIndex++)
   {
      if (E_NOT_OK == ADCIF_stCalibrationStatus0.Adc_UnitSelfTestStatus)
      {
         Adc_Calibrate(AdcGroup_0, &ADCIF_stCalibrationStatus0);
      }

      if (E_NOT_OK == ADCIF_stCalibrationStatus1.Adc_UnitSelfTestStatus)
      {
