...
/*
$Revision: 1.16 $
$ProjectName: e:/MKSProjects/SBE/eCS/AUDI_MCC/Phase_01/View_Development/Components/Application/Autoliv/EcuMIf/Implementation/src/project.pj $
*/
/*****************************************************************************
...
EXPORTED FUNC(EcuM_DeterminePbConfigurationRetType, ECUM_APPL_CODE) EcuM_DeterminePbConfiguration(void)
{
  ...
  EcuM_DeterminePbConfigurationRetType PbConfig = &(EcuM_Config_0_TDK_4G);
...
{
  if (EcuM_ECUM_RESET_MCU == reset)
  {
    /* If Mcu reset is requested perform it */
    Mcu_PerformReset();
  }
  else if (EcuM_ECUM_RESET_EXT_WDG == reset)
  {
...
EXPORTED FUNC(void,ECUM_APPL_CODE) EcuM_StartWakeupSources(EcuM_WakeupSourceType wakeupSource)
{
  ...
  if( ECUMIF_KU32_WKSOURCE_NONE_SOURCES != (wakeupSource & EcuM_ECUM_WKSOURCE_CAN) )
  {
     (void)CanIf_SetControllerMode(ctl_MQBw_Baseline_Airbag_SUBCANFD_KMatrix_V14_08_02E_20220819_MH_Alv, CANIF_CS_STOPPED);
     (void)CanIf_SetControllerMode(ctl_MQBw_Baseline_Airbag_SUBCANFD_KMatrix_V14_08_02E_20220819_MH_Alv, CANIF_CS_STARTED);
  }
  else
  {
    /* Do nothing */
...
EXPORTED FUNC(void,ECUM_APPL_CODE) EcuM_CheckValidation (EcuM_WakeupSourceType wakeupSource)
{
  ...
  if(ECUMIF_KU32_WKSOURCE_NONE_SOURCES != (wakeupSource & EcuM_ECUM_WKSOURCE_CAN))
  {
     (void)CanIf_CheckValidation(EcuM_ECUM_WKSOURCE_CAN);
  }
}
