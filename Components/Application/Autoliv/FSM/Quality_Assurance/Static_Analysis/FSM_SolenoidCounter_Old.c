...
/*
$Revision: 1.22 $
$ProjectName: e:/MKSProjects/SBE/eCS/AUDI_MCC/Phase_01/View_Development/Components/Application/Autoliv/FSM/Quality_Assurance/Static_Analysis/project.pj $
 */
...
EXPORTED void FSM_manageSolenoidCounters(void)
{
   ...
   /* Variable used to determine when to write in NVP */
   register boolean bCounterChanged = B_FALSE;
         ...
         if (FSM_au8SolenoidStates[u8SolenoidIndex] != fsm_au8LastSolenoidState[u8SolenoidIndex])
         {
            if((KU8_LOCK_STATE == FSM_au8SolenoidStates[u8SolenoidIndex]) || (KU8_UNLOCK_STATE == FSM_au8SolenoidStates[u8SolenoidIndex]))
            {
               ...
               if (KU32_MAX > NVP_au32ExecutedSolenoidCounters[u8SolenoidNVPArrayIndex])
               {
                  ...
                  /* Change status of variable */
                  bCounterChanged = B_TRUE;
               }
            }
            else
            {
   ...
   if (B_TRUE == bCounterChanged)
   {
      (void)Rte_Call_pclNvmIfServices_SetRamBlockStatus((NvM_ASR42_BlockIdType)NvM_EOL_COUNTERS, B_TRUE);
   }
}
