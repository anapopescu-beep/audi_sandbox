 ...
 */
#define KU8_FSM_SCHADULE_NB_OF_FCNS                ((uint8) 0x07)
 ...
 */
#define KU8_FSM_MASK_MODE_STATUS_INITIALIEZED      ((uint8) 0x01)
 ...
 *      Is equal with he biggest task period + corresponding decade value + 1
 *      (ex. 10 + 6 + 1);
 *      If there is a single task period value for all functions, the value
 *      should be set to the biggest decade value + 1 (in this case decade 5 + 1)
 *      check fsm_astSchadulerFunc.
 */
#define KU16_FSM_MAX_DECADE                        ((uint16) 7)
...
typedef struct fsm_SchadulerFunction_Type {
...
******************************************************************************/
LOCAL_INLINE void fsm_UpdateModeStatus_SolenoidErrorDisableConditions(void);
 ...
 *
 * \initialization
 *       DemConf_DemEventParameter_EVENT_POWER_SUPPLY_UV (13U)
 *       DemConf_DemEventParameter_EVENT_POWER_SUPPLY_OV (12U)
 *       DemConf_DemEventParameter_EVENT_NVM_NOT_PRG (6U)
 *       DemConf_DemEventParameter_EVENT_ADC_OUT_OF_ORDER (2U)
 */
LOCAL const uint8 fsm_kau8SolenoidDisableDemEventCfg[KU8_FSM_SOLENOID_DISABLE_DEM_EVENT_NB] =
{
      DemConf_DemEventParameter_EVENT_PFLASH_MEM_CORRUPT,
      DemConf_DemEventParameter_EVENT_RAM_MEM_CORRUPT,
      DemConf_DemEventParameter_EVENT_DFLASH_MEM_CORRUPT,
...
{
      {KST_FSM_PERIODIC_10_MS(KU8_FSM_IN_DECADE_0),   &fsm_UpdateModeStatus_SporadicHwError           },
      {KST_FSM_PERIODIC_10_MS(KU8_FSM_IN_DECADE_1),   &fsm_UpdateModeStatus_PermanentHwError          },
      {KST_FSM_PERIODIC_2_MS(KU8_FSM_IN_DECADE_0),    &fsm_UpdateModeStatus_SolenoidUnlockConfirmation},
      {KST_FSM_PERIODIC_10_MS(KU8_FSM_IN_DECADE_2),   &fsm_UpdateModeStatus_CanInhibit                },
      {KST_FSM_PERIODIC_1_MS,                         &fsm_UpdateModeStatus_SolenoidErrorDisableConditions },
...
/**
* \brief
*        The function is used to disable all solenoids if any
...
*        None
* \ddesignrequirement
*        DSG_fsm_UpdateModeStatus_SolenoidErrorDisableConditions
* \archrequirement
...
LOCAL_INLINE void fsm_UpdateModeStatus_SolenoidErrorDisableConditions(void)
{
   ...
   for (u8Idx = KU8_ZERO; u8Idx < KU8_FSM_SOLENOID_DISABLE_DEM_EVENT_NB; u8Idx++)
   {
      (void)Rte_Call_pclDemServices_GetEventStatus(fsm_kau8SolenoidDisableDemEventCfg[u8Idx], &u8EventStatus);
      u8MergedEventStatus |= u8EventStatus;
   }

   /* If an hardware error is detected, disable all solenoids */
   if (DEM_UDS_STATUS_TF == (u8MergedEventStatus & DEM_UDS_STATUS_TF))
   {
      /* Mode is set */
      if(KB_TRUE == KB_eCS_SOLENOID_1_ENABLED_VARIANT)
      {
         KU64_FSM_SET_MODE_MASK_ON(KU64_SOLENOID_1_ERROR_DISABLED);
      }
      if(KB_TRUE == KB_eCS_SOLENOID_2_ENABLED_VARIANT)
      {
         KU64_FSM_SET_MODE_MASK_ON(KU64_SOLENOID_2_ERROR_DISABLED);
      }
      if(KB_TRUE == KB_eCS_SOLENOID_3_ENABLED_VARIANT)
      {
         KU64_FSM_SET_MODE_MASK_ON(KU64_SOLENOID_3_ERROR_DISABLED);
      }
      if(KB_TRUE == KB_eCS_SOLENOID_4_ENABLED_VARIANT)
      {
         KU64_FSM_SET_MODE_MASK_ON(KU64_SOLENOID_4_ERROR_DISABLED);
      }
      if(KB_TRUE == KB_eCS_SOLENOID_5_ENABLED_VARIANT)
      {
         KU64_FSM_SET_MODE_MASK_ON(KU64_SOLENOID_5_ERROR_DISABLED);
      }
      if(KB_TRUE == KB_eCS_SOLENOID_6_ENABLED_VARIANT)
      {
         KU64_FSM_SET_MODE_MASK_ON(KU64_SOLENOID_6_ERROR_DISABLED);
      }
   }
   else
   {
      /* Mode is cleared */
      KU64_FSM_SET_MODE_MASK_OFF(KU64_SOLENOID_1_ERROR_DISABLED);
      KU64_FSM_SET_MODE_MASK_OFF(KU64_SOLENOID_2_ERROR_DISABLED);
      KU64_FSM_SET_MODE_MASK_OFF(KU64_SOLENOID_3_ERROR_DISABLED);
      KU64_FSM_SET_MODE_MASK_OFF(KU64_SOLENOID_4_ERROR_DISABLED);
      KU64_FSM_SET_MODE_MASK_OFF(KU64_SOLENOID_5_ERROR_DISABLED);
      KU64_FSM_SET_MODE_MASK_OFF(KU64_SOLENOID_6_ERROR_DISABLED);
   }
}

/**
* \brief
*        The function is used to disable all solenoids which are not enabled/used
*        in NVP variant.
* \inputparam
*        None
* \outputparam
   ...
   if(KU8_TRUE == u8CriticalAutoTestsFinishedFlag)
   {
      ...
      /* If initialization phase is finished:
            ...
            (u8SolenoidUsedBitMask == FSM_u8SolenoidInitState))
         {
            ...
            U8_BIT_SET(fsm_u8InitializationFinished, KU8_FSM_MASK_MODE_STATUS_INITIALIEZED);
         }
         else
         {
...
LOCAL_INLINE void fsm_UpdateModeStatus_EcuBatteryUnstableInhibition(void)
{
   ...
   else
   {
      ...
      if((NVP_u16BatteryUnstableDisqualificationThrs <= u16VBatt) && (NVP_u16BatteryUnstableDisqualificationCnt > fsm_u8BatteryUnstableCnt))
      {
         fsm_u8BatteryUnstableCnt++;

         if(NVP_u16BatteryUnstableDisqualificationCnt == fsm_u8BatteryUnstableCnt)
         {
            KU64_FSM_SET_MODE_MASK_OFF(KU64_ECU_BATTERY_UNSTABLE);
         }
      }
...
EXPORTED void FSM_runUpdateModeStatus(void)
{
   ...
   {
      if(KU8_FSM_MASK_MODE_STATUS_INITIALIEZED != (fsm_u8InitializationFinished & KU8_FSM_MASK_MODE_STATUS_INITIALIEZED))
      {
         fsm_UpdateModeStatus_Init();
      }
      ...
      fsm_u16CurrentDecade = (fsm_u16CurrentDecade + KU16_ONE) % KU16_FSM_MAX_DECADE;
   }
}
...
*       u32ModesToCheck;
*       Range: KU8_ZERO, FSM_KU8_MODE_STATUS_OFF, FSM_KU8_MODE_STATUS_ON;
* \exception
*        This function has no exceptions.
* \pre
*        fsm_b8ModesHasBeenInitialized variable has to be TRUE.
