...
STATIC FUNC(void, BSWM_CODE) BswM_GetPtrTableArrayFields(uint8 source, uint16 i, BswMBasicPortType** base, uint32* channel, void** mode, void** ptr);
...
STATIC CONST(uint8, AUTOMATIC) BswM_InitialValueMappings[10] = 
{
  /* BswM_RequestPort */
  0xFFU,
  /* BswmM_CanSM_RequestPort */
  0xFFU,
  /* BswM_EcuM_RequestPort */
  0xFFU,
  /* BswM_Dcm_RequestPort */
  0U,
  /* BswM_PassangerOccupancy_RequestPort */
  0xFFU,
  /* BswM_EcuState_RequestPort */
  0xFFU,
  /* BswM_WakeupType_RequestPort */
  0xFFU,
  /* BswM_TxPdu_RequestPort */
  0xFFU,
  /* BswM_EcuShutdown_RequestPort */
  0xFFU,
  /* BswM_Dcm_ComM_RequestPort */
  1U,
};
 ...
 */
STATIC CONST( uint16 , AUTOMATIC ) BswM_NumModeRequestPortsTable[14] = 
{
 ...
 */
STATIC BswMUInt8RteModeRequestPortType BswM_UInt8RteRequestPortTable[2] = 
{
  { /* BswM_Dcm_RequestPort */
    { /* base */
      UINT16_C( 3 ), /* id */
 ...
 */
BswMU8ChannelU8ModePortType BswMCanSMIndicationPortTable[1] = 
{
  { /* [0] */
    { /* base */
      UINT16_C( 1 ), /* id */
 ...
 */
BswMU8ChannelU8ModePortType BswMEcuMWakeupSourcePortTable[1] = 
{
  { /* [0] */
    { /* base */
      UINT16_C( 2 ), /* id */
 ...
 */
STATIC CONSTP2VAR( BswMPtrModeRequestPortType, BSWM_CONST, BSWM_VAR ) BswM_PtrModeRequestPortsTable[14] = 
{
  &BswMGenericRequestPortTable, /* BSWM_GENERIC_REQUEST */
  &BswMCanSMIndicationPortTable, /* BSWM_CAN_SM_INDICATION */
  ...
  &BswMEcuMWakeupSourcePortTable, /* BSWM_ECUM_WAKEUP_SOURCE */
...
FUNC(void, BSWM_CODE) BswM_LT_Init(void)
{
...

  {
    ...
    for (i = 0U; i < 14U; i++)
    {
      if ( BswM_PtrModeRequestPortsTable[i] != NULL_PTR)
      {
        for (j = 0U; j < BswM_NumModeRequestPortsTable[i]; j++)
        {
          void* mode;
          BswMBasicPortType* basicPort;
          BswM_GetPtrTableArrayFields(i, j, &basicPort, NULL_PTR, &mode, NULL_PTR);
          if (BswM_InitialValueMappings[basicPort->id] != BSWM_INVALID_INITIAL_VALUE_INDEX)
          {
            if ( i == BSWM_GENERIC_REQUEST)
            {
              *((uint16*) mode) = BSWM_GET_UINT16_INITIAL_VALUES_TABLE(BswM_Context.ID)[BswM_InitialValueMappings[basicPort->id]];
            }
            else
            {
              *((uint8*) mode) = BSWM_GET_UINT8_INITIAL_VALUES_TABLE(BswM_Context.ID)[BswM_InitialValueMappings[basicPort->id]];
            }
            basicPort->isDefined = TRUE;
          }
          else
          {
            basicPort->isDefined = FALSE;
          }
        }
      }
    }
...
FUNC(void, BSWM_CODE) BswM_MRPFunc_BswM_Dcm_RequestPort_NONE(void)
{
  ...
  {
    BswM_HandleRequest(&BswM_Context, &BswM_UInt8RteRequestPortTable[0], RTE_MODE_DcmEcuReset_NONE, BSWM_ARBITRATION_RTE_PORT_UINT8, BSWM_SID_INVALID, BswM_UInt8RteRequestPortTable[0].base.isImmediate);
  }
  else
  {
...
FUNC(void, BSWM_CODE) BswM_MRPFunc_BswM_Dcm_RequestPort_HARD(void)
{
  if (TRUE == BswM_Context.RunTimeContext.IsInitialized)
  {
    BswM_HandleRequest(&BswM_Context, &BswM_UInt8RteRequestPortTable[0], RTE_MODE_DcmEcuReset_HARD, BSWM_ARBITRATION_RTE_PORT_UINT8, BSWM_SID_INVALID, BswM_UInt8RteRequestPortTable[0].base.isImmediate);
  }
  else
  {
    /* No arbitration shall be performed if BswM is not initialized */
...
FUNC(void, BSWM_CODE) BswM_MRPFunc_BswM_Dcm_RequestPort_KEYONOFF(void)
{
  if (TRUE == BswM_Context.RunTimeContext.IsInitialized)
  {
    BswM_HandleRequest(&BswM_Context, &BswM_UInt8RteRequestPortTable[0], RTE_MODE_DcmEcuReset_KEYONOFF, BSWM_ARBITRATION_RTE_PORT_UINT8, BSWM_SID_INVALID, BswM_UInt8RteRequestPortTable[0].base.isImmediate);
  }
  else
  {
    /* No arbitration shall be performed if BswM is not initialized */
...
FUNC(void, BSWM_CODE) BswM_MRPFunc_BswM_Dcm_RequestPort_SOFT(void)
{
  if (TRUE == BswM_Context.RunTimeContext.IsInitialized)
  {
    BswM_HandleRequest(&BswM_Context, &BswM_UInt8RteRequestPortTable[0], RTE_MODE_DcmEcuReset_SOFT, BSWM_ARBITRATION_RTE_PORT_UINT8, BSWM_SID_INVALID, BswM_UInt8RteRequestPortTable[0].base.isImmediate);
  }
  else
  {
    /* No arbitration shall be performed if BswM is not initialized */
...
FUNC(void, BSWM_CODE) BswM_MRPFunc_BswM_Dcm_RequestPort_JUMPTOBOOTLOADER(void)
{
  if (TRUE == BswM_Context.RunTimeContext.IsInitialized)
  {
    BswM_HandleRequest(&BswM_Context, &BswM_UInt8RteRequestPortTable[0], RTE_MODE_DcmEcuReset_JUMPTOBOOTLOADER, BSWM_ARBITRATION_RTE_PORT_UINT8, BSWM_SID_INVALID, BswM_UInt8RteRequestPortTable[0].base.isImmediate);
  }
  else
  {
    /* No arbitration shall be performed if BswM is not initialized */
...
FUNC(void, BSWM_CODE) BswM_MRPFunc_BswM_Dcm_RequestPort_JUMPTOSYSSUPPLIERBOOTLOADER(void)
{
  if (TRUE == BswM_Context.RunTimeContext.IsInitialized)
  {
    BswM_HandleRequest(&BswM_Context, &BswM_UInt8RteRequestPortTable[0], RTE_MODE_DcmEcuReset_JUMPTOSYSSUPPLIERBOOTLOADER, BSWM_ARBITRATION_RTE_PORT_UINT8, BSWM_SID_INVALID, BswM_UInt8RteRequestPortTable[0].base.isImmediate);
  }
  else
  {
    /* No arbitration shall be performed if BswM is not initialized */
...
FUNC(void, BSWM_CODE) BswM_MRPFunc_BswM_Dcm_RequestPort_EXECUTE(void)
{
  if (TRUE == BswM_Context.RunTimeContext.IsInitialized)
  {
    BswM_HandleRequest(&BswM_Context, &BswM_UInt8RteRequestPortTable[0], RTE_MODE_DcmEcuReset_EXECUTE, BSWM_ARBITRATION_RTE_PORT_UINT8, BSWM_SID_INVALID, BswM_UInt8RteRequestPortTable[0].base.isImmediate);
  }
  else
  {
    /* No arbitration shall be performed if BswM is not initialized */
...
STATIC FUNC(BswMResultType, BSWM_CODE) BswM_ExprResult_BswMLogicalExpression_BswMRule_PrepareHWReset(void)
{
  return
  (
    (BswM_UInt8RteRequestPortTable[0].mode == RTE_MODE_DcmEcuReset_HARD)
  ) ? BSWM_TRUE : BSWM_FALSE;
}
...
STATIC FUNC(BswMResultType, BSWM_CODE) BswM_ExprResult_BswMLogicalExpression_BswMRule_ExecuteReset(void)
{
  return
  (
    (BswM_UInt8RteRequestPortTable[0].mode == RTE_MODE_DcmEcuReset_EXECUTE)
  ) ? BSWM_TRUE : BSWM_FALSE;
}
...
STATIC FUNC(BswMResultType, BSWM_CODE) BswM_ExprResult_BswMLogicalExpression_BswMRule_PrepareSWReset(void)
{
  return
  (
    (BswM_UInt8RteRequestPortTable[0].mode == RTE_MODE_DcmEcuReset_SOFT)
  ) ? BSWM_TRUE : BSWM_FALSE;
}
...
STATIC FUNC(BswMResultType, BSWM_CODE) BswM_ExprResult_BswMLogicalExpression_BswMRule_PrepareJumpToFbl(void)
{
  return
  (
    (BswM_UInt8RteRequestPortTable[0].mode == RTE_MODE_DcmEcuReset_JUMPTOBOOTLOADER)
  ) ? BSWM_TRUE : BSWM_FALSE;
}
...
STATIC FUNC(BswMResultType, BSWM_CODE) BswM_ExprResult_BswMLogicalExpression_BswMRule_EnterLowPwrFromPostRun(void)
{
  return
  (
    (BswMGenericRequestPortTable[2U].mode == BSWM_ENABLE_TX_PDU)
  ) ? BSWM_TRUE : BSWM_FALSE;
}
...
STATIC FUNC(BswMResultType, BSWM_CODE) BswM_ExprResult_BswMLogicalExpression_BswMRule_EnableTxPdu_DcmRequest(void)
{
  return
  (
    (BswM_UInt8RteRequestPortTable[1].mode == RTE_MODE_DcmCommunicationControl_DCM_ENABLE_RX_TX_NORM)
  ) ? BSWM_TRUE : BSWM_FALSE;
}
...
STATIC FUNC(BswMResultType, BSWM_CODE) BswM_ExprResult_BswMLogicalExpression_BswMRule_DisableTxPdu_DcmRequest(void)
{
  return
  (
    (BswM_UInt8RteRequestPortTable[1].mode == RTE_MODE_DcmCommunicationControl_DCM_ENABLE_RX_DISABLE_TX_NORM)
  ) ? BSWM_TRUE : BSWM_FALSE;
}
...
STATIC FUNC(boolean, BSWM_CODE) BswM_ExprState_BswMLogicalExpression_BswMRule_EnableTxPdu_DcmRequest(void)
{
  return
  (
    (TRUE == BswM_UInt8RteRequestPortTable[1].base.isDefined)
  ) ? TRUE : FALSE;
}
...
STATIC FUNC(void, BSWM_CODE) BswM_GetPtrTableArrayFields(uint8 source, uint16 i, BswMBasicPortType** base, uint32* channel, void** mode, void** ptr)
{
  void* portArr = BswM_PtrModeRequestPortsTable[source];
  switch(source)
  {
    case BSWM_COMM_PNC_REQUEST:
    {
      if (base != NULL_PTR)
      {
        *base = &(((BswMU16ChannelU8ModePortType*)portArr)[i].base);
      }
      if (channel != NULL_PTR)
      {
        *channel = (uint32)(((BswMU16ChannelU8ModePortType*)portArr)[i].channel);
      }
      if (mode != NULL_PTR)
      {
        *mode = &(((BswMU16ChannelU8ModePortType*)portArr)[i].mode);
      }
      if (ptr != NULL_PTR)
      {
        *ptr = &(((BswMU16ChannelU8ModePortType*)portArr)[i]);
      }
      break;
    }
    case BSWM_GENERIC_REQUEST:
    {
      if (base != NULL_PTR)
      {
        *base = &(((BswMGenericRequestPortType*)portArr)[i].base);
      }
      if (channel != NULL_PTR)
      {
        *channel = (uint32)(((BswMGenericRequestPortType*)portArr)[i].channel);
      }
      if (mode != NULL_PTR)
      {
        *mode = &(((BswMGenericRequestPortType*)portArr)[i].mode);
      }
      if (ptr != NULL_PTR)
      {
        *ptr = &(((BswMGenericRequestPortType*)portArr)[i]);
      }
      break;
    }
    default:
    {
      if (base != NULL_PTR)
      {
        *base = &(((BswMU8ChannelU8ModePortType*)portArr)[i].base);
      }
      if (channel != NULL_PTR)
      {
        *channel = (uint32)(((BswMU8ChannelU8ModePortType*)portArr)[i].channel);
      }
      if (mode != NULL_PTR)
      {
        *mode = &(((BswMU8ChannelU8ModePortType*)portArr)[i].mode);
      }
      if (ptr != NULL_PTR)
      {
        *ptr = &(((BswMU8ChannelU8ModePortType*)portArr)[i]);
      }
      break;
    }
  }
...
FUNC(Std_ReturnType, BSWM_CODE) BswM_HandleStaticRequest(uint32 channel, uint16 mode, uint8 source, uint8 sid, uint8 type)
{
  if (BswM_Context.RunTimeContext.IsInitialized == TRUE)
  {
    ...
    for (i = 0U; i < BswM_NumModeRequestPortsTable[source]; i++)
    {
      ...
      BswMBasicPortType* basicPort;
      BswM_GetPtrTableArrayFields(source, i, &basicPort, &currentChannel, NULL_PTR, &currentPort);

      switch(source)
      {
        case BSWM_ECUM_WAKEUP_SOURCE:
        eval = ((((uint32)1U << currentChannel) & channel) != 0U);
        break;
        default:
        eval = (currentChannel == channel);
        break;
      }
      if (eval == TRUE)
      {
        BswM_HandleRequest(&BswM_Context, currentPort, mode, type, sid, basicPort->isImmediate);
      }
    }
  }
  return E_OK;
