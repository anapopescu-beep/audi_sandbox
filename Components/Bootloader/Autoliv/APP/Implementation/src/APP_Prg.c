...
/*
$Revision: 1.4 $
$ProjectName: e:/MKSProjects/SBE/eCS/AUDI_MCC/Phase_01/View_Development/Components/Bootloader/Autoliv/APP/Implementation/src/project.pj $
 */
/*!****************************************************************************
 ...
 *
 **/
#define KU8_FLASH_ADRESS_VALUE       ((uint8) 0x45u)
/**
 * \brief
...
/**
 * \brief
 *       Errors check mask (FTFC_FSTAT_FPVIOL_MASK | FTFC_FSTAT_ACCERR_MASK).
 *
 **/
#define KU8_THIRTY                   ((uint8) 0x30u)
/******************************************************************************
MODULE TYPES
 ******************************************************************************/
 ...
 ******************************************************************************/
__ramfunc LOCAL uint8 dcmif_UnlockFlashRoutineRegOperations(u16 *u16DataLen, u8 *aubUdsData);
/******************************************************************************
...
/**
 * \brief
 *		  Function used to write flash address in memory controller register.
 * \inputparam
 *       Name: u16DataLen;
 *       Type: u16;
 ...
 *       None.
 * \return
 *      u8RegVal - Flash Security Register Status
 * \dynamicaspectcaller
 *        DcmIf module.
 * \dynamicaspectdescription
 * 		 Called in APP_UnlockFlash.
 * \constrains
 *      None.
 * \ddesignrequirement
 *		DSG_DCMIF_dcmif_UnlockFlashRoutineRegOperations
 * \archrequirement
 *       None.
 **/
__ramfunc LOCAL uint8 dcmif_UnlockFlashRoutineRegOperations(u16 *u16DataLen, u8 *aubUdsData)
{
	uint8 u8ArrayCounter = KU8_ZERO;
	uint8 u8RegVal = KU8_MAX;

	if((*u16DataLen) == KU8_TWELVE)
	{

		/* wait until the flash module is no longer busy */
		while(KU8_ONE_HUNDRED_AND_TWENTYEIGHT != (FTFC->FSTAT & FTFC_FSTAT_CCIF_MASK))
		{
			; /*wait*/
		}

		/* check if there are any errors */
		if((KU8_SIXTEEN == (FTFC->FSTAT & FTFC_FSTAT_FPVIOL_MASK)) ||
		   (KU8_THIRTYTWO == (FTFC->FSTAT & FTFC_FSTAT_ACCERR_MASK)))
		{
		...
		FTFC->FCCOB[KU8_THREE] = KU8_FLASH_ADRESS_VALUE;


		/* write Flash data in memory controller register (Flash Common Command Object Registers) */

		for (u8ArrayCounter = KU8_FOUR; u8ArrayCounter < KU8_TWELVE; u8ArrayCounter++)
		{
			FTFC->FCCOB[u8ArrayCounter] = aubUdsData[u8ArrayCounter];
		}
...
EXPORTED tUdsStatus APP_UnlockFlash(PduLengthType *pulLen, u8 *aubUdsData)
{
	...
	else
	{
		...
		aubUdsData[KU8_FOUR] =  dcmif_UnlockFlashRoutineRegOperations(&pulLen[0], &aubUdsData[0]);
