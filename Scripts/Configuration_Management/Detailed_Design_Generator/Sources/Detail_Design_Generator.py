...
def convert_png_files_to_bmp_files(file_to_convert):
    ...
    # ======================================================
    # Open old image
    old_image = Image.open(file_to_convert)
    ...
    if max_dimension <= 1500:
        ...
        Image.open(file_to_convert).save(file_to_convert.replace('.bmp', '.png'))

        return file_to_convert.replace('.bmp', '.png')
...
def function_parser():
    ...
    subprocess.run(path_to_yflowgen_tool + ' -d ' + path_to_yflowgen_folder_src + ' -format dot -no_comment -true_false', shell=False)
    print('=====================================================')
                ...
                for function, list_increment in zip( module_statistics[file][x], tqdm(module_statistics[file][x]) ):
                    ...
                    # ============================================================
                        ...
                        if function_name != '':
                            ...
                            # ==========================================================================
                            # ADD PARAGRAPH FOR IDENTIFY WHERE TO INSERT GRAPHICS FROM yflowgen
                            # ==========================================================================

                            paragraph = doc.add_paragraph(function_name + '.dot.svg')
...
list_of_file_paths = []
def svg_to_bmp(folder_path):
    # get all the SVG files in the folder
    svg_files = [f for f in os.listdir(folder_path) if f.endswith('.svg')]
    # loop through all the SVG files
    for svg_file in svg_files:

        # get the full path of the SVG file
        svg_file_path = os.path.join(folder_path, svg_file)

        # get the full path of the BMP file (with .bmp extension)
        bmp_file_path = os.path.splitext(svg_file_path)[0] + '.bmp'

        # convert the SVG to BMP using cairosvg library
        cairosvg.svg2png(url=svg_file_path, write_to=bmp_file_path)

        # Construct a list of all paths to graphic functions
        list_of_file_paths.append(bmp_file_path)

def add_graphics_in_word_document():

    print("ADDING yflowgen GRAPHICS")
    ...
    list_with_all_files_in_directory = os.listdir(path_to_yflowgen_folder_html)

    for folder in list_with_all_files_in_directory:
        svg_to_bmp(os.path.join(path_to_yflowgen_folder_html, folder))

    # if exists, for every paragraph in document
    for paragraph in docx.Paragraphs:

        if 'SW Detailed Design (DD)' in paragraph.Range.Text:
            # delete written name
            paragraph.Range.Text = yaml_config_file['Component to be analysed'] + ' SW Detailed Design (DD)'

        if 'AEM TEMPLATE' in paragraph.Range.Text:
    ...
    for function_name, list_increment in zip( list_of_functions, tqdm(list_of_functions) ):
        ...
        for path in list_of_file_paths:
            if function_name in path:
                ...
                path_to_graphic_function_bmp = convert_png_files_to_bmp_files(file_to_convert=path)

                # if exists, for every paragraph in document
                for paragraph in docx.Paragraphs:

                    # If found file name is in word
                    if function_name + '.dot.svg' in paragraph.Range.Text:
                        # delete written name
                        paragraph.Range.Text = ''
    ...
    for function in list_of_functions:
        if function not in list_of_functions_with_graphics:
            print(function + ' function has no graphic generated from yflowgen tool! ')
