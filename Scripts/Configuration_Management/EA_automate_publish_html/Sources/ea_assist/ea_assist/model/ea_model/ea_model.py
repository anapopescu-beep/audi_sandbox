...
class EAInstance:
    ...
    def findOrCreatePackage(self, parent: Package, name: str,
                            skip_guid: bool = False) -> Package:
        if not skip_guid:
    ...
    # def findOrCreateTaggedValueForTable(self, parent: Element, name: str, value: str) -> TaggedValue:
    #     tagged_value = self.findTaggedValue(parent, name)
    #     if (tagged_value == None):
    #         tagged_value = parent.TaggedValues.AddNew(name, value) # type: TaggedValue
    #     tagged_value.Update()
    ...
    def createConnectorBetweenTwoNods(self, source_element, target_element, connector_name):
        # create a new connector
        # listConectors= []
        # for idx in range(source_element.Connectors.Count):
        #     listConectors.append(source_element.Connectors.GetAt(idx))
    ...
    # def findCustomTable(self, parent: Element, name: str, skip_guid = False):
    #     guid = self.getGuid(EAInstance.GUID, self.getElementGuidKey(parent.Name, name))
    #     customTable = None
    #     if skip_guid:
    #         if (guid != ""):
    #             customTable = self.getRepository().GetElementByGuid(guid)
    #             if (customTable == None):
    #                 self.removeGuid(EAInstance.GUID_, name)
    #     if (port == None):
    #         #print("find port slowly <%s, %s>" % (name, guid))
    #         for idx in range(0, parent.EmbeddedElements.Count):
    #             current_port = parent.EmbeddedElements.GetAt(idx)  # type: Element
    ...
    #     return port
    #
    # def findOrCreateCustomTable(self, parent: Element, name: str,):
    #     port = self.findPort(parent, name)
    #     if not port:
    #         port = parent.EmbeddedElements.AddNew(name, "Port")  # type: Attribute
    #         self.addGuid(EAInstance.GUID_PORTS, self.getElementGuidKey(parent.Name, port.Name), port.ElementGUID)
    ...
    def findOrCreateDiagramObject(self, parentDiagram: 'Diagram', elmID: int, position):
        # diagramObj = parentDiagram.DiagramObjects.AddNew("l=10;r=120;t=10;b=70;", "")
