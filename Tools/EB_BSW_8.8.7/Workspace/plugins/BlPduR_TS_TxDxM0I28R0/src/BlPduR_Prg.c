...
/* Pointer to reset the Tx buffer if Suppress Positive response is true and NRC 78 is sent previously*/
u8* m_pubClearNRC78 = NULL_PTR;
/* Flag to stop NRC78 handling and to make the BlPduR status IDLE*/
u8 m_ubSuppressPositiveResponse;
...
/* Tab to store the pointer Rx Buffer configuration information  use for Physical Request queued buffers
...
/* Tab to store the pointer Rx Buffer configuration information  use for Functional Request queued buffers
   The pointers are stored in the order of treatment */
NCS_PRI_VAR tBufferConfigType* m_apstFuncQueuedReceptionBuffer[BLPDUR_RX_FUNC_BUFFER_NUM];
/* Number of Functional queued buffers */
NCS_PRI_VAR u8 m_ubFuncQueuedBuffCount;
/* Tab to store the Type ofreceived queued Requests */
NCS_PRI_VAR u8 m_aubReceptionTypBuffer[BLPDUR_RX_BUFFER];
/* Counter on the number of element queued in m_aubReceptionTypBuffer */
u8 m_ubReceiveReqTypCount;
#endif
...
#if (BLPDUR_MULTIPLE_BUFFERS_ENABLE == STD_ON)
NCS_PRI_FCT void BlPduR_MultipleBuffers_Manage(void);
...
void BlPduR_Init2(void)
{
    ...
    /* Initialize Suppress positive response flag */
    m_ubSuppressPositiveResponse = FALSE;
#if (BLPDUR_USE_STREAMING == STD_ON)
...
#if (BLPDUR_QUEUED_MANAGEMENT_ENABLE == STD_ON)
    /*Initialisation of counters used in Queued Buffer Management*/
    m_ubQueuedBuffCount     = 0U;
    m_ubFuncQueuedBuffCount = 0U;
    m_ubReceiveReqTypCount  = 0U;
    /*Initialisation of pointer to structure which holds the queued buffer of Physical Requests*/
    for (ubFlagIdx = 0U; ubFlagIdx < BLPDUR_RX_PHY_BUFFER_NUM; ubFlagIdx++)
    {
        m_apstQueuedReceptionBuffer[ubFlagIdx] = NULL_PTR;
    }
    /*Initialisation of pointer to structure which holds the queued buffer of Functional Requests*/
    for (ubFlagIdx = 0U; ubFlagIdx < BLPDUR_RX_FUNC_BUFFER_NUM; ubFlagIdx++)
    {
        m_apstFuncQueuedReceptionBuffer[ubFlagIdx] = NULL_PTR;
    }
    /*Initalisation of array which hold the type of request received in queued management*/
    for(ubFlagIdx = 0U; ubFlagIdx < (BLPDUR_RX_BUFFER - 1U); ubFlagIdx++)
    {
       m_aubReceptionTypBuffer[ubFlagIdx] = 0x00U;
    }
#endif
#if (BLPDUR_MULTIPLE_ID_ENABLE ==  STD_ON)
#if (BLPDUR_MULTIPLE_ID_TYPE == BLPDUR_MULTIPLE_ID_CAN_NOTIFICATION)
...
NCS_PRI_FCT void BlPduR_TpRxMsgManage (void)
{
    ...
    if ((BLPDUR_E_IDLE == m_uTreatmentStatus) && (NULL_PTR != m_apstReceptionBuffer[0]))
    {
        ...
        if ((m_ubQueuedBuffCount != 0 )|| (m_ubFuncQueuedBuffCount != 0))
        {
           /*Check if the next queued buffer request to be processed is Functional or Physical Request*/
           if(m_aubReceptionTypBuffer[0] == BLPDUR_FUNCTIONAL)
            {
              /* Get the config information for functional queued buffer*/
              curRxBufferInfo = m_apstFuncQueuedReceptionBuffer[0]->bufferInfo;
              /* Read the message length */
              ulLen = curRxBufferInfo->filledLen;
              /* Read the message data buffer address */
              paubUdsData = m_apstFuncQueuedReceptionBuffer[0]->ptrBuffer;
            }
           else
            {
              /* Get the config information for physical queued buffer*/
        ...
        if (ubRxPduId < BLPDUR_MAX_RX_PDU_NUM)
        {
            ...
            if(BLPDUR_FUNCTIONAL == stRxPdu[ubRxPduId].AddressingType)
            {
                                ...
                                || (m_pstRxCurrentBufferConfig->bufferInfo->state == BLPDUR_BUFF_IDLE)))
                    {
                        ...
                        /* switch to the functional buffer */
                        m_pstRxCurrentBufferConfig = &m_stRxFuncBufferConfig;
#endif
#if (BLPDUR_QUEUED_MANAGEMENT_ENABLE == STD_ON)
                       /* Check if the functional Queued buffer is full */
                        for (ubFlagIdx = 0U; ubFlagIdx < BLPDUR_RX_FUNC_BUFFER_NUM; ubFlagIdx++)
                        {
                            if (m_apstFuncQueuedReceptionBuffer[ubFlagIdx] == NULL_PTR)
                            {
                        ...
                        {
                            for(ubIdx = 0U; (ubIdx < BLPDUR_RX_FUNC_BUFFER_NUM) && (ubBufferFound == BLPDUR_FALSE); ubIdx++)
                            {
                                /* Get Functional Queued buffer Info */
                                curRxBufferInfo = stRxFuncBufferConfig[ubIdx].bufferInfo;
                                /* If the selected buffer is IDLE,It will be selected for receiving request */
                                if(BLPDUR_BUFF_IDLE == curRxBufferInfo->state)
                                {
                                    if((m_pstRxCurrentBufferConfig == NULL_PTR)
                                       || (m_pstRxCurrentBufferConfig->bufferInfo->state == BLPDUR_BUFF_IDLE))
                                    {
                                    ...
                                    {
                                       /* save current buffer pointer for handling immediately Tester present Frame */
                                       m_pstRxPreviousOnGoingBufferConfig = m_pstRxCurrentBufferConfig;
                                    }
                                    m_pstRxCurrentBufferConfig = &stRxFuncBufferConfig[ubIdx];
                        ...
                        {
                           /*Functional Queued buffer is full*/
...
/* CHECK: PARSE */
                            ...
                            ((m_pstRxCurrentBufferConfig == NULL_PTR) || (m_pstRxCurrentBufferConfig->bufferInfo->state != BLPDUR_BUFF_PROCESSING)))
                    {
                        ...
                        /* Check if the Physical Queued buffer is full */
                        for (ubFlagIdx = 0U; ubFlagIdx < BLPDUR_RX_PHY_BUFFER_NUM; ubFlagIdx++)
                        {
...
void BlPduR_RxInd(PduIdType uRxPduId, tBlPduRStatus eBlPduRStatus)
{
    ...
    if (BLPDUR_E_OK == eBlPduRStatus)
    {
          ...
          (m_pstRxCurrentBufferConfig->bufferInfo->filledLen == 2))
        {
...
#endif
#if(BLPDUR_MULTIPLE_BUFFERS_ENABLE == STD_ON)
          BlPduR_MultipleBuffers_Manage();
#endif
#if ((BLPDUR_NETWORK_ETH_SUPPORTED == STD_ON)||(BLPDUR_QUEUED_MANAGEMENT_ENABLE == STD_ON))
          ...
          {
              /* Set the functional TP processed buffer state to Idle */
              m_pstRxCurrentBufferConfig->bufferInfo->state = BLPDUR_BUFF_IDLE;
        ...
        else
        {
...
#endif
            {
                        ...
                        (m_ubQueuedBuffCount !=0))
                {
                    ...
                    {
                        if((stRxPdu[curRxBufferInfo->originatingRxPduId].AddressingType) == BLPDUR_FUNCTIONAL)
                        {
                           /* The buffer reception order shall stored for the queued treatment */
                           m_apstFuncQueuedReceptionBuffer[m_ubFuncQueuedBuffCount] = m_pstRxCurrentBufferConfig;
                           /* Increment the number of Queued requests */
                           m_ubFuncQueuedBuffCount++;
                        }
                        else
                        {
                           /* The buffer reception order shall stored for the queued treatment */
                        ...
                        /*Store the type of request in next free index*/
                        m_aubReceptionTypBuffer[m_ubReceiveReqTypCount] = stRxPdu[curRxBufferInfo->originatingRxPduId].AddressingType;
                        m_ubReceiveReqTypCount++;
                    }
                }
                else
#endif
    ...
    else
    {
        ...
        /*Release the buffer as there is timeout*/
...
void BlPduR_TxConf(PduIdType uTxPduId, tBlPduRStatus eBlPduRStatus)
{
    ...
    if (m_uTreatmentStatus == BLPDUR_E_BUSY)
    {
        ...
        {
            /*Check if suppress positive response has been requested after NRC 78*/
            if(m_ubSuppressPositiveResponse == TRUE)
            {
                /* Clear the previously sent NRC78 with suppressed Tx data*/
                m_pubTxBuffer = m_pubClearNRC78;
                /* Suppress positive response is requested, so make the status IDLE*/
                m_uTreatmentStatus = BLPDUR_E_IDLE;
            }
            else
            {
    ...
    /*Reset the suppress positive response indication flag*/
    m_ubSuppressPositiveResponse = FALSE;
    ...
    if((m_uTreatmentStatus == BLPDUR_E_IDLE) && (m_pstRxCurrentBufferConfig != NULL_PTR))
    {
        ...
        if(m_aubReceptionTypBuffer[0] == BLPDUR_FUNCTIONAL)
        {
           ...
           BlPduR_ReleaseBuffers(m_apstFuncQueuedReceptionBuffer[0U]->bufferInfo);
           /* shift reception flag */
           for(ubIdx = 0U; ubIdx < (BLPDUR_RX_FUNC_BUFFER_NUM - 1U); ubIdx++)
            {
                m_apstFuncQueuedReceptionBuffer[ubIdx] = m_apstFuncQueuedReceptionBuffer[ubIdx + 1U];
            }
            /* reset last reception flag to ensure the invalid value */
            m_apstFuncQueuedReceptionBuffer[BLPDUR_RX_FUNC_BUFFER_NUM - 1U] = NULL_PTR;
        ...
        /*Previous request has been processed,Move to next queued request*/
        for(ubIdx = 0U; ubIdx < (BLPDUR_RX_BUFFER - 1U); ubIdx++)
       {
          m_aubReceptionTypBuffer[ubIdx] = m_aubReceptionTypBuffer[ubIdx + 1U];
       }
/* reset last flag to ensure the invalid value */
m_aubReceptionTypBuffer[BLPDUR_RX_BUFFER - 1U] = 0x00U;
/*Decrement the count for storing type of queued requests*/
m_ubReceiveReqTypCount--;
#endif
        /* Release Rx buffer */
#if (BLPDUR_MULTIPLE_BUFFERS_ENABLE == STD_ON)
    ...
    {
#if (BLPDUR_QUEUED_MANAGEMENT_ENABLE == STD_ON)
        /*Check if there is any further queued request to process*/
        if ((m_ubQueuedBuffCount < 1)&&(m_ubFuncQueuedBuffCount < 1))
        {
          /* Re-Initialize current Connection since there are no further queued request to process */
          m_ubCurrentConnection = BLPDUR_NO_ACTIVE_CONNECTION;
        }
#else
...
void BlPduR_Send_TPFrame(PduLengthType ulLen, u8 *paubData)
{
...
#endif
    {
        ...
        {
            /* Update the local buffer with suppressed Tx data*/
            m_pubClearNRC78 = paubData;
            /*Set flag to indicate, suppress positive response has been requested after NRC 78*/
            m_ubSuppressPositiveResponse = TRUE;
            /* Simulate Tx confirmation to free the buffer */
            BlPduR_TxConf(uTxPduId, BLPDUR_E_OK);
        }
#endif
...
NCS_PRI_FCT void BlPduR_MultipleBuffers_Manage(void)
{
        ...
        #endif
        #endif
    /* Check if we got a functional tester present request */
        if (m_pstRxCurrentBufferConfig == &m_stRxFuncBufferConfig)
        {
            /* Check if no multiple buffer processing is ongoing */
            if ( PROG_GetMultBufProcesingValue() == PROG_FALSE)
            {
                /* Check if the current service (physical request)              */
                /* has been completely processed (including the transmission of */
                /* the answer) */
                if (m_uTreatmentStatus == BLPDUR_E_IDLE)
                {
#ifdef PROG_RESPOND_TO_PROG_SESSION
                ...
                {
                    /* service processing not completed yet! */
                    /* current connection still active  */
                }
            }
            /* multiple buffer processing is ongoing */
            else
            {
                 /* check if service processing is inactive and no previous buffer has */
                 /* been saved  */
                 if ( (m_uTreatmentStatus == BLPDUR_E_IDLE) &&
                     (m_pstRxPreviousOnGoingBufferConfig == NULL_PTR) )
                 {
                 ...
                 /* check if service processing is inactive and the previous buffer is */
                 /* currently not being processed and there is no response pending     */
                 /* due to multiple buffer processing (all buffers locked)             */
                 else
                 {
                    if(PROG_GetMultBufResPendingValue() == PROG_FALSE)
                     {
                           if((m_pstRxPreviousOnGoingBufferConfig != NULL_PTR) &&
                           (m_pstRxPreviousOnGoingBufferConfig->bufferInfo->state != BLPDUR_BUFF_PROCESSING))
                           {
#ifdef PROG_RESPOND_TO_PROG_SESSION
#if (PROG_RESPOND_TO_PROG_SESSION == STD_ON)
                      /* Re-initialize the current connection only if the simulate Rx request treatment is done */
            ...
            /* Restore the previous buffer pointer,             */
            /* the previous ongoing buffer config pointer gets  */
            /* assigned ONLY for functional requests during     */
            /* BlPduR_StartOfReception()!                       */
            m_pstRxCurrentBufferConfig = m_pstRxPreviousOnGoingBufferConfig;

            /* Set the functional buffer state to idle, */
            /* processing ends here*/
            m_stRxFuncBufferConfig.bufferInfo->state = BLPDUR_BUFF_IDLE;
        }
        /* we got a tester present as a physical request */
        else
        {
            /* there is no previous buffer to restore! */

            /* Re-initialize the connection */
            /* (safe because it arrived as a physical request) */
            m_ubCurrentConnection = BLPDUR_NO_ACTIVE_CONNECTION;

            /* Set the current buffer state to idle, */
            /* processing ends here                  */
            m_pstRxCurrentBufferConfig->bufferInfo->state = BLPDUR_BUFF_IDLE;
        }
#endif
}
/*----------------------------------{end BlPduR_MultipleBuffers_Manage}------------------------*/
#endif
