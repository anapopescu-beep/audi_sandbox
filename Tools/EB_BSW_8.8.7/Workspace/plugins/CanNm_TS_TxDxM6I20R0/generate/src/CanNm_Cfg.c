...
[!VAR "Application" = "node:name(as:modconf('Os')[1]/OsApplication/*[node:exists(OsAppEcucPartitionRef) and (node:value(OsAppEcucPartitionRef) = node:value(node:ref(node:current()/CanNmComMNetworkHandleRef)/ComMChannelPartitionRef))][1])"!][!//
    ...
    &SchM_Send_CanNm_1_ChOsApp_[!"$Application"!]_Channel_DataSet_[!"CanNmRxPdu/*[1]/CanNmRxPduId"!],
#endif
...
[!IF "node:exists(CanNmSynchronizedPncShutdownEnabled) and (CanNmSynchronizedPncShutdownEnabled = 'true')"!][!//
