...
[!IF "node:exists(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMChannel/*[($Partition = node:value(ComMChannelPartitionRef)) and (ComMBusType = 'COMM_BUS_TYPE_CAN') and ((ComMNetworkManagement/ComMNmVariant = 'FULL') or (ComMNetworkManagement/ComMNmVariant = 'PASSIVE'))])"!][!//
...
[!SELECT "node:order(as:modconf('CanNm')[1]/CanNmGlobalConfig/*[1]/CanNmChannelConfig/*,'CanNmRxPdu/*[1]/CanNmRxPduId')"!][!//
[!VAR "Channel_Idx" = "CanNmRxPdu/*[1]/CanNmRxPduId"!][!//
[!VAR "Application" = "node:name(as:modconf('Os')[1]/OsApplication/*[node:exists(OsAppEcucPartitionRef) and (node:value(OsAppEcucPartitionRef) = node:value(node:ref(node:current()/CanNmComMNetworkHandleRef)/ComMChannelPartitionRef))][1])"!][!//
...
{
[!IF "as:modconf('CanNm')[1]/CanNmGlobalConfig/*[1]/CanNmPnEiraCalcEnabled = 'true'"!][!//
  if (TRUE == CanNm_Initialized)
  {
