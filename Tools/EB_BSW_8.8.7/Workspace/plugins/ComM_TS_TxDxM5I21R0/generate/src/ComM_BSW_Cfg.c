...
CONST(uint8, COMM_CONST) ComM_SynchronizedShutDownPncMask[COMM_PN_INFO_LENGTH] =
{
  [!FOR "I" = "$macroRetValGetPnOffset" TO "$macroRetValGetPnOffset + $PncMaskLength -1"!]
    [!VAR "loopCount" = "$loopCount +1"!]
    [!VAR "Mask" = "0"!]
    [!LOOP "node:order(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMPnc/*[ComMPncId >= num:i($I * 8) and ComMPncId < num:i(($I + 1) * 8)], 'ComMPncId')"!]
      [!VAR "TempComMPncId" = "ComMPncId"!]
      [!LOOP "ComMChannelPerPnc/*"!]
        [!IF "node:exists(node:ref(.)/ComMPncGatewayType) and (node:ref(.)/ComMPncGatewayType = 'COMM_GATEWAY_TYPE_PASSIVE')"!]
          [!VAR "Mask" = "bit:bitset($Mask, ($TempComMPncId mod 8))"!]
        [!ENDIF!]
      [!ENDLOOP!]
    [!ENDLOOP!]
  [!"num:inttohex($Mask)"!]U,
...
{
    [!LOOP "node:order(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMUser/*,'ComMUserIdentifier')"!]
...
CONST(uint8, COMM_CONST) ComM_Multicore_UserToMasterPncSchmSend_FctPtr_IndexList[COMM_NUM_USERS] =
{
  ...
  [!"$UserToMasterPncFctPtrIndex"!],
      [!ENDLOOP!] [!/*[!LOOP "node:order(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMUser/*, 'ComMUserIdentifier')"!]*/!]
};
...
CONST(uint8, COMM_CONST) ComM_Multicore_ChToInhibitionSchmSend_FctPtr_IndexList[COMM_NUM_CHANNELS] =
{
  ...
  [!IF "node:exists(as:modconf('ComM')[1]/ComMGeneral/ComMSynchronizedPncShutdownEnabled) and (as:modconf('ComM')[1]/ComMGeneral/ComMSynchronizedPncShutdownEnabled = 'true')"!]
    ...
    [!CALL "GetNumberOfPassiveChannelsInPNCs"!]
...
[!"num:i($SynchShutDownChToMasterPncFctPtrIndex)"!],
    ...
    [!ENDIF!] [!/*[!IF "count(text:split($GetSynchShutDownChToMasterPnc_FromOsAppToOsAppList,' ')) > 0"!]*/!]
  ...
  ComM_Dcm_Notif_FpList[COMM_NUM_CHANNELS] =
{
    [!LOOP "node:order(as:modconf('ComM')[1]/ComMConfigSet/*/ComMChannel/*, 'ComMChannelId')"!]
  {
    ...
    &SchM_Call_ComM_1_Ch_OsApplication_[!"$ChannelOsApplication"!]_ComM_RequiredCSEntry_Dcm_ComM_NoComModeEntered_[!"node:name(.)"!],
      [!ELSE!] [!/*[!IF "count(node:refs(as:modconf('Dcm')[1]/DcmConfigSet/*/DcmDsl/DcmDslProtocol/DcmDslProtocolRow/*/DcmDslConnection/*/DcmDslProtocolRx/*/DcmDslProtocolRxComMChannelRef)[node:exists(ComMChannelId) and (ComMChannelId = node:current()/ComMChannelId)]) != 0"!]*/!]
    NULL_PTR,
    NULL_PTR,
    NULL_PTR,
      [!ENDIF!] [!/*[!IF "count(node:refs(as:modconf('Dcm')[1]/DcmConfigSet/*/DcmDsl/DcmDslProtocol/DcmDslProtocolRow/*/DcmDslConnection/*/DcmDslProtocolRx/*/DcmDslProtocolRxComMChannelRef)[node:exists(ComMChannelId) and (ComMChannelId = node:current()/ComMChannelId)]) != 0"!]*/!]
  },
    [!ENDLOOP!] [!/*[!LOOP "node:order(as:modconf('ComM')[1]/ComMConfigSet/*/ComMChannel/*, 'ComMChannelId')"!]*/!]
};
...
{
    [!LOOP "node:order(node:refs(ComMPnc/*/ComMPncComSignal/*[ComMPncComSignalKind='EIRA'][ComMPncComSignalDirection='RX']/ComMPncComSignalRef), 'ComHandleId')"!]
  {
    ...
    [!ENDLOOP!] [!/* [!LOOP "node:order(node:refs(ComMPnc/*/ComMPncComSignal/*[ComMPncComSignalKind='EIRA'][ComMPncComSignalDirection='RX']/ComMPncComSignalRef), 'ComHandleId')"!]*/!]
};
    [!IF "as:modconf('ComM')[1]/ComMGeneral/ComMPncGatewayEnabled = 'true'"!]
...
CONST(ComM_RxSignal_Struct_Type, COMM_CONST) ComM_RxComSignalActiveEraCfg[COMM_NUM_RX_ACTIVE_ERA_SIGNALS] =
{
                             ...
                             'ComHandleId')"!]
  {
      ...
      [!ENDLOOP!] [!/* [!LOOP "node:order(node:refs(ComMPnc/*/ComMPncComSignal/*
                             [ComMPncComSignalKind='ERA']
                             [ComMPncComSignalDirection='RX']
                             [(node:exists(node:ref(ComMPncComSignalChannelRef)/ComMPncGatewayType)) and (node:ref(ComMPncComSignalChannelRef)/ComMPncGatewayType='COMM_GATEWAY_TYPE_ACTIVE')]/ComMPncComSignalRef),
                             'ComHandleId')"!] */!]
...
[!CALL "GetMasterPncOsApplication"!]
[!IF "num:i(count(node:refs(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMPnc/*/ComMPncComSignal/*[ComMPncComSignalDirection='TX']/ComMPncComSignalRef))) > 0"!]
#define COMM_START_SEC_CONST_UNSPECIFIED
#include <ComM_MemMap.h>
  ...
  [!IF "num:i($MulticoreNoOfTxBuffers) != 0"!]
    ...
    [!LOOP "as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMPnc/*/ComMPncComSignal/*[ComMPncComSignalDirection='TX']"!]
      ...
      COMM_EIRA_ERA_ACTIVE
        ...
        [!ENDIF!] [!/*[!IF "(node:ref(ComMPncComSignalRef)/ComHandleId) = $Handle"!] */!]
...
#define COMM_STOP_SEC_CONST_UNSPECIFIED
#include <ComM_MemMap.h>
[!ENDIF!] [!/*[!IF "num:i(count(node:refs(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMPnc/*/ComMPncComSignal/*[ComMPncComSignalDirection='TX']/ComMPncComSignalRef))) > 0"!]*/!]
[!ELSE!] [!/*[!IF "(count(text:split($FoundUserPlusChannePlusMasterPncPlusInhibitionOsApplications,' ')) > 1) and (as:modconf('ComM')[1]/ComMGeneral/ComMMultiCoreSupport = 'false')"!]*/!]
...
#define COMM_START_SEC_CONST_8
#include <ComM_MemMap.h>
CONST(boolean, COMM_CONST) ComM_IsTopLevelCoordinator[COMM_NUM_PNC] =
{
[!LOOP "node:order(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMPnc/*,'ComMPncId')"!]
  [!VAR "AllChannelsAreActive" = "'true'"!]
  [!LOOP "ComMChannelPerPnc/*"!][!//
    [!IF "not(node:exists(node:ref(.)/ComMPncGatewayType)) or
         (node:exists(node:ref(.)/ComMPncGatewayType) and (node:ref(.)/ComMPncGatewayType != 'COMM_GATEWAY_TYPE_ACTIVE'))"!]
      [!VAR "AllChannelsAreActive" = "'false'"!]
      [!BREAK!]
    [!ENDIF!]
  [!ENDLOOP!]
  [!IF "$AllChannelsAreActive = 'true'"!]
    TRUE,
  [!ELSE!]
    FALSE,
  [!ENDIF!]
...
[!LOOP "node:order(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMChannel/*, 'ComMChannelId')"!]
  ...
  [!"$ComMGatewayTypePassiveCount"!],
    ...
    [!ELSE!] [!/* [!IF "$ChannelIsPartOfAPnc = 'true'"!] */!]
  0xFF,
    [!ENDIF!] [!/* [!IF "$ChannelIsPartOfAPnc = 'true'"!] */!]
...
CONST(boolean, COMM_CONST) ComM_IsPncActiveMixedWithPassive[COMM_NUM_PNC] =
{
[!LOOP "node:order(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMPnc/*,'ComMPncId')"!]
