...
[!MACRO "GetSignalOsApplicationTemp","SignalHandle","Direction"!]
  ...
  [!VAR "MainFunctionRef" = "as:modconf('Com')[1]/ComConfig/*/ComIPdu/*/ComIPduSignalRef/*[node:exists(.) and (. = $SignalRef)]/../../ComMainFunctionRef"!]
  [!VAR "PartitionRef" = "node:ref($MainFunctionRef)/ComPartitionRef"!]
  [!VAR "SignalOsApplicationTemp" = "node:name(as:modconf('Os')[1]/OsApplication/*[node:exists(OsAppEcucPartitionRef) and (OsAppEcucPartitionRef = $PartitionRef)])"!]
[!ENDMACRO!]
...
[!IF "node:exists(as:modconf('EcuM')[1]/EcuMGeneral/EcuMCrossCoreComMWakeup) and (node:value(as:modconf('EcuM')[1]/EcuMGeneral/EcuMCrossCoreComMWakeup) = 'true')"!]
  [!LOOP "node:order(node:refs(as:modconf('EcuM')[1]/EcuMConfiguration/*[1]/EcuMCommonConfiguration/EcuMWakeupSource/*/EcuMComMChannelRef), 'ComMChannelId')"!]
    ...
    [!IF "$OsApplicationAssignmentFound = 'false'"!]
      ...
      [!ENDIF!]
    [!ENDIF!]    
  [!ENDLOOP!] [!/*[!LOOP "node:order(node:refs(as:modconf('EcuM')[1]/EcuMConfiguration/*[1]/EcuMCommonConfiguration/EcuMWakeupSource/*/EcuMComMChannelRef), 'ComMChannelId')"!]*/!]
...
[!MACRO "GetNumberOfPassiveChannelsInPNCs"!]
[!VAR "NrOfPassiveChannels" = "num:i(0)"!]
[!LOOP "node:order(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMChannel/*, 'ComMChannelId')"!]
  [!IF "node:exists(ComMPncGatewayType) and (ComMPncGatewayType = 'COMM_GATEWAY_TYPE_PASSIVE')"!]
    ...
    [!IF "$ChannelIsPartOfAPnc = 'true'"!]
      [!VAR "NrOfPassiveChannels" = "num:i($NrOfPassiveChannels + 1)"!]
    [!ENDIF!]
  [!ENDIF!]
[!ENDLOOP!]  
[!ENDMACRO!]

[!MACRO "GetNumberOfActiveChannelsInPNCs"!]
[!VAR "NrOfActiveChannels" = "num:i(0)"!]
[!LOOP "node:order(as:modconf('ComM')[1]/ComMConfigSet/*[1]/ComMChannel/*, 'ComMChannelId')"!]
  [!IF "node:exists(ComMPncGatewayType) and (ComMPncGatewayType = 'COMM_GATEWAY_TYPE_ACTIVE')"!]
    [!CALL "GetIfChannelIsPartOfAPnc","LocalComMChannelId"="ComMChannelId"!]
    [!IF "$ChannelIsPartOfAPnc = 'true'"!]
      [!VAR "NrOfActiveChannels" = "num:i($NrOfActiveChannels + 1)"!]
    [!ENDIF!]
  [!ENDIF!]
[!ENDLOOP!]  
[!ENDMACRO!]
