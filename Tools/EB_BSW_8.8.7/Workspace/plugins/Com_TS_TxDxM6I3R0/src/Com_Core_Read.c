...
static FUNC(boolean, COM_CODE) Com_EB_Read_32_Sign_SetRemainBit
(
  P2VAR(void, AUTOMATIC, COM_APPL_DATA) SignalDataPtr,
  P2CONST(ComSignalBaseType, AUTOMATIC, AUTOMATIC) Com_EB_ReadPropPtr
);
...
)
{
  ...
  else
  {
    ...
    if(COM_SINT64 == COM_GET_RX_SIGNAL_SIGNAL_TYPE(Com_EB_ReadPropPtr))
    {
      ...
      boolean const MSB = Com_EB_Read_32_Sign_SetRemainBit(&SignalDataPtr_loc->LoWord, Com_EB_ReadPropPtr);
      if (MSB != 0U)
      {
