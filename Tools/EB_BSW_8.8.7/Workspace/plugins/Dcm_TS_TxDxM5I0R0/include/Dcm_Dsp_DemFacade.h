...
typedef struct
{
  ...
  /* This flag is to disable calling the Dem interface DemGetNextFilteredDTCAndSeverity()
   * again once it returns with DEM_NO_SUCH_ELEMENT during paged-buffering since all the
   * remaining DTCs are padded with zero */
  boolean SkipDemGetNextFilteredDTCAndSeverityCall;
...
typedef struct
{
   ...
   * for sub-services 0x19-0x01, -0x02, -0x0A, -0x14, -0x15, -0x17 and 0x42*/
  uint8 RequestedStatusMask;
