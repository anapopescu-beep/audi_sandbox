  ...
  if (TRUE == ProcessAuthenticationCalled)
  {
    switch (ServiceProcessStatus)
    {
      case E_OK:
      {
        ...
        {
          /* !LINKSTO Dcm.EB.UDSServices.AuthenticationServiceRequest.VerifyCertificateUnidirectional.TxBufferShort.TransmitNRC0x10,1 */
...
FUNC(Std_ReturnType, DCM_CODE) Dcm_Dsp_Authentication_ProofOfOwnership_SSvcH(
  ...
  P2VAR(Dcm_MsgContextType, AUTOMATIC, DCM_VAR) pMsgContext)
{
  Dcm_AuthenticationProofOfOwnershipContextType ProofOfOwnershipContext;
  Std_ReturnType ServiceProcessStatus = DCM_E_ABORT;
  Dcm_AuthenticationConnectionIndexType AuthenticationConnectionIndex = DCM_NUM_MAIN_CONNECTIONS;
      ...
      if (pMsgContext->reqDataLen >= DCM_MINIMUM_LENGTH_OFFSET_SSVC_0X03)
      {
        ...
        if (ClientProofLength != 0U)
        {
              ...
              (DCM_MANDATORY_LENGTH_OFFSET_SSVC_0X03 + (Dcm_MsgLenType)ClientProofLength))
          {
            ...
            {
              /* set up context for authentication */
              ProofOfOwnershipContext.ProofOfOwnershipClient.ProofOfOwnership = &(pMsgContext->reqData[2U]);
              ProofOfOwnershipContext.ProofOfOwnershipClient.LengthOfProofOfOwnership = ClientProofLength;
              ProofOfOwnershipContext.ErrorCode = E_NOT_OK;
  ...
  if (TRUE == ProofOfOwnershipCalled)
  {
    switch (ServiceProcessStatus)
    {
      ...
      case E_NOT_OK:
        Dcm_ExternalSetNegResponse(pMsgContext, ProofOfOwnershipContext.ErrorCode);
        break;
