...
[!IF "(as:modconf('Dem')[1]/DemGeneral/DemExtendedDataCapture = 'DEM_TRIGGER_ON_FDC_THRESHOLD') and
  ...
  [!IF "node:exists(DemConfigSet/*/DemAdvDisplacementPassiveTableLine/*)"!]
    [!VAR "NumOfLines" = "0"!]
...
/*------------------[PID class configuration]---------------------------------*/
    ...
    [!IF "node:exists(DemConfigSet/*/DemPidClass/*)"!]
    #define DEM_START_SEC_CALIB_UNSPECIFIED
    #include <Dem_MemMap.h>
...
[!//
[!INDENT "0"!]
  [!IF "node:exists(DemGeneral/DemDidClass/*)"!]
    #define DEM_START_SEC_CONST_16
    #include <Dem_MemMap.h>
...
[!//
[!INDENT "0"!]
  [!IF "DemGeneral/DemTypeOfFreezeFrameRecordNumeration = 'DEM_FF_RECNUM_CONFIGURED' and node:exists(DemGeneral/DemFreezeFrameRecNumClass/*)"!]
    #define DEM_START_SEC_CONST_8
    #include <Dem_MemMap.h>
...
[!//
[!INDENT "0"!]
  [!IF "node:exists(DemGeneral/DemExtendedDataRecordClass/*)"!]
    #define DEM_START_SEC_CONST_16
    #include <Dem_MemMap.h>
...
#include <Dem_MemMap.h>
[!//
[!IF "node:exists(DemGeneral/DemDataElementClass/*[. = 'DemExternalCSDataElementClass' or . = 'DemExternalSRDataElementClass'])"!]
  [!VAR "SRSupport" = "(DemGeneral/DemRteUsage = 'true') and node:exists(DemGeneral/DemDataElementClass/*[. = 'DemExternalSRDataElementClass'])"!]
  [!INDENT "0"!]
...
[!IF "(node:exists(DemGeneral/DemGroupOfDTC/*[name(.) != 'DEM_DTC_GROUP_EMISSION_REL_DTCS']))"!]
...
[!/*  - [EventFailureClassIdx] - */!][!//
      [!CALL "EVENT_DESC_BITFIELD_EVENT_FAILURE_CLASS_IDX", "Indent" = "4", "Prefix" = "$Operation"!]
[!/*  - [Priority] - */!][!//
      [!IF "$EventClassSupport = 'false'"!]
...
[!/*  - [EventFailureClassIdx] - */!][!//
    [!CALL "EVENT_DESC_BITFIELD_EVENT_FAILURE_CLASS_IDX", "Indent" = "4"!]
[!/*  - [OpCycleIdx] - */!][!//
    [!IF "($EventClassSupport = 'false') or ($CommonOperationCycleEnabled = 'false')"!][!//
[!/* !LINKSTO dsn.Dem.CommonOperationCycle.Dem_EventDesc,1 */!][!//
      [!CALL "EVENT_DESC_BITFIELD_OPERATION_CYCLE", "Indent" = "4"!]
...
{
  [!INDENT "2"!][!//
    ((uint32)(DEM_J1939_INVALID_NODE_ID) << DEM_J1939_NODE_OFFSET), /* invalid event id and node entry */
    [!LOOP "node:order(DemConfigSet/*/DemEventParameter/*, 'DemEventId')"!][!//
      /* [!"name(.)"!] */
...
[!//
[!INDENT "0"!]
  [!IF "node:exists(DemGeneral/DemSPNClass/*)"!]
  ...
  { /* [!"name(.)"!] */
    ...
    [!/*  -----[ObdDTC]----- */!][!//
      ...
      [!//
        ...
        [!CALL "EVENT_DESC_BITFIELD_OBD_MIN_RATIO_VALUE", "Indent" = "4", "Prefix" = "$Operator"!][!//
      [!IF "$CalibrationSupport = 'false'"!][!//
        [!/*  -----[Dem_EventOBDReadinessGroupType]----- */!][!//
        [!/* !LINKSTO dsn.Dem.OBD.config.OBDEvConf1.DemEventOBDReadinessGroup.0001,3 */!][!//
          [!"$Operator"!]((uint16)(DEM_OBD_READINESS_MASK & [!"DemEventClass/DemEventOBDReadinessGroup"!]) <<
...
[!/*  - [Min Event Ratio] - */!][!//
          [!CALL "EVENT_DESC_BITFIELD_OBD_MIN_RATIO_VALUE", "Indent" = "4", "Prefix" = "$Operator"!][!//
[!/*  - [Readiness Group] - */!][!//
          [!CALL "EVENT_DESC_BITFIELD_OBD_READINESS_VALUE", "Indent" = "4", "Prefix" = "$Operator"!][!//
          [!WS "2"!]},
[!/* If output was generated, stop searching for matching EventDTCClassname for the current DTCClassname since
   * only one event is mapped to an UDS-DTC (the Master Configuration Event in case of event combination) */!][!//
          [!BREAK!][!//
...
{
  DEM_OBD_J1939_RDY_HCCAT - DEM_OBD_J1939_RDY_HCCAT_OFFSET,
  DEM_OBD_J1939_RDY_NOXCAT - DEM_OBD_J1939_RDY_NOXCAT_OFFSET,
  DEM_OBD_J1939_RDY_NONE,
  DEM_OBD_J1939_RDY_BOOSTPR - DEM_OBD_J1939_RDY_BOOSTPR_OFFSET,
  DEM_OBD_J1939_RDY_NONE,
  DEM_OBD_J1939_RDY_EGSENS - DEM_OBD_J1939_RDY_EGSENS_OFFSET,
  DEM_OBD_J1939_RDY_PMFLT - DEM_OBD_J1939_RDY_PMFLT_OFFSET,
  DEM_OBD_J1939_RDY_EGR - DEM_OBD_J1939_RDY_EGR_OFFSET
};
#define DEM_STOP_SEC_CONST_8
#include <Dem_MemMap.h>
[!ENDIF!]
  ...
  Dem_OBDReadinessGroupMap[DEM_NUM_OBD_RDY_GROUPS] =
{
  ...
  DEM_OBD_RDY_FLSYS_NONCONT, /* not mapped to output */
...
[!IF "node:exists(as:modconf('Dem')[1]/DemConfigSet/*/DemEventParameter/*[DemEventClass/DemEventOBDReadinessGroup = 'DEM_OBD_RDY_GPF']) != 0"!]
  DEM_OBD_RDY_GPF,           /* DataC:4 */
[!ENDIF!]
  DEM_OBD_RDY_O2SENS,        /* DataC:5 */
  DEM_OBD_RDY_O2SENSHT,      /* DataC:6 */
  DEM_OBD_RDY_AC,            /* J1939 non continously monitored:4 */
...
{
  [!CALL "GET_READINESSGROUP_LIST"!][!//
  [!FOR "GroupIndex" = "1" TO "$NumberOfGroups"!][!//
    [!VAR "ReadinessGroupName" = "text:split($ReadinessGroups, '#')[position() = $GroupIndex]"!][!//
    [!CALL "GET_EVENTS_IN_READINESSGROUP", "GroupName" = "$ReadinessGroupName"!][!//
  [!ENDFOR!][!//
};
...
[!INDENT "0"!]
  ...
  [!IF "node:exists(DemGeneral/DemEnableCondition/*)"!]
  ...
  [!IF "node:exists(DemGeneral/DemEnableConditionGroup/*)"!]
...
[!INDENT "0"!]
  ...
  [!IF "(node:exists(DemConfigSet/*/DemEventParameter/*[DemEventClass/DemDebounceAlgorithmClass = 'DemDebounceCounterBased'])) or (node:exists(DemGeneral/DemMaxNumberDebCounterClasses) and (DemGeneral/DemMaxNumberDebCounterClasses > 0))"!]
  #define DEM_START_SEC_CALIB_16
  #include <Dem_MemMap.h>
  ...
  /** \brief Time based debouncing configuration description table */
  [!IF "node:exists(DemConfigSet/*/DemEventParameter/*[DemEventClass/DemDebounceAlgorithmClass = 'DemDebounceTimeBase'])"!]
    CONST(Dem_EventIdType, DEM_CONST)
  ...
  [!IF "node:exists(DemConfigSet/*/DemEventParameter/*[DemEventClass/DemDebounceAlgorithmClass = 'DemDebounceFrequencyBased'])"!]
...
[!INDENT "0"!]
  [!IF "node:exists(DemConfigSet/*/DemEventParameter/*[node:exists(DemEventClass/DemEventFailureCycleRef)])"!]
...
[!INDENT "0"!]
  ...
  CONST(Dem_CbPropertyType, DEM_CONST) Dem_CbProperty[DEM_NUMBER_OF_EVENTS] =
  {
    ...
    [!LOOP "node:order(DemConfigSet/*/DemEventParameter/*, 'DemEventId')"!]
     ...
     [!//
      [!ENDIF!]
      [!IF "node:exists(DemCallbackEventStatusChanged/*)"!]
        [!IF "node:exists(DemCallbackEventStatusChanged/*[not(node:exists(DemCallbackEventStatusChangedFnc))])"!]
  ...
  [!INDENT "0"!]
   [!/* vars for calculation of AutomaticEndEnabled bitmask */!]
    ...
    [!VAR "opCycCount" = "0"!]
    [!IF "node:exists(DemGeneral/DemOperationCycle/*[DemOperationCycleAutomaticEnd = 'true'])"!]
  ...
  [!IF "count(DemConfigSet/*/DemEventParameter/*/DemCallbackInitMForE) != 0"!]
  #define DEM_START_SEC_CONST_UNSPECIFIED
  #include <Dem_MemMap.h>
    ...
    [!LOOP "DemConfigSet/*/DemEventParameter/*"!]
      ...
      [!LOOP "DemCallbackEventStatusChanged/*[not(node:exists(DemCallbackEventStatusChangedFnc))]"!]
        ...
        [!IF "as:modconf('Dem')[1]/DemGeneral/DemServiceAPI/DemDefaultASRServiceAPI = 'AUTOSAR_2011'"!]
          [!VAR "CallbackName" = "concat('Rte_Call_CBEventUdsStatusChanged_', name(../../.), '_', name(.), '_CallbackEventUdsStatusChanged')"!]
        [!ELSE!]
    ...
    [!WS "2"!]Dem_CbFuncPtrTrigOnDTCSt[DEM_CB_TABLE_TRIG_ON_DTCST_SIZE] =
  {
    ...
    [!LOOP "DemGeneral/DemCallbackDTCStatusChanged/*"!]
      ...
      [!ELSE!]
        [!IF "as:modconf('Dem')[1]/DemGeneral/DemServiceAPI/DemDefaultASRServiceAPI = 'AUTOSAR_2011'"!]
          [!VAR "CallbackName" = "concat('Rte_Call_CBStatusDTC_', name(.), '_DTCStatusChanged')"!]
        [!ELSE!]
          [!VAR "RunningNr" = "num:integer($RunningNr + 1)"!]
  ...
  [!IF "count(DemConfigSet/*/DemEventParameter/*/DemEventClass/DemDebounceAlgorithmClass[. = 'DemDebounceMonitorInternal']/DemCallbackGetFDC) != 0"!]
...
[!IF "((DemGeneral/DemOBDSupport = 'true') or (DemGeneral/DemGeneralJ1939/DemJ1939Readiness1Support = 'true'))"!]
...
[!ELSE!]
CONST(uint8, DEM_CONST) Dem_ObdCompliancyPID1CByte = 0x5U; /* default value for non-OBD ECU */
[!ENDIF!]
[!ENDIF!]
