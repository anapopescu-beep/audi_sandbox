 ...
 *
 * Reason:
 * "EventEntryAllocResult" is DEM_EVENTENTRYALLOC_INVALID only if the function
 * Dem_FindOrAllocateEventEntry() returns DEM_EVENTENTRYALLOC_INVALID. In other
 * configurations, the code coverage reaches sections where EventEntryAllocResult
 * is equal to the value DEM_EVENTENTRYALLOC_INVALID.
 *
...
/* patch version check */
#if (DEM_SW_PATCH_VERSION < 7U)
#error DEM_SW_PATCH_VERSION wrong (< 7U)
#endif
...
#if(((DEM_NUM_J1939FFSPNS > 0U) && (DEM_J1939_FREEZEFRAME_SUPPORT==STD_ON)) || (DEM_NUM_FFSEGS > 0U))
#if (DEM_DEV_ERROR_DETECT == STD_ON)
/** \brief This function provided the Fim and Masking support in context
 **        of Dem_ProcessEvent
 **
 ** \param[in] ProcessEDAndFFResult
 ** \param[in] Result
 ** \Reentrancy{Non reentrant}
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(void, DEM_CODE) Dem_CheckDevError(
  Std_ReturnType                                   ProcessEDAndFFResult,
  P2VAR(Std_ReturnType, AUTOMATIC, AUTOMATIC)      Result);
#endif /* (DEM_DEV_ERROR_DETECT == STD_ON) */
#endif /* (((DEM_NUM_J1939FFSPNS > 0U) && (DEM_J1939_FREEZEFRAME_SUPPORT==STD_ON)) || (DEM_NUM_FFSEGS > 0U)) */
...
/** \brief Function to call Dem_ProcessClearDTC()
 **
 ** \Reentrancy{Non reentrant - function shall be called from within a critical section}
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(void, DEM_CODE) Dem_CallProcessClearDTC(void);
...
/** \brief This function tries to check for TF
 **
 ** \param[in] EventId  0 < EventId <= DEM_MAX_EVENTID
 ** \param[in] ErrorReenterPrecondition
 **
 ** \Reentrancy{Non reentrant}
 ** \Synchronicity{Synchronous}
 */
STATIC FUNC(void, DEM_CODE) Dem_CheckTF(
  Dem_EventIdType                         EventId,
  P2VAR(boolean, AUTOMATIC, AUTOMATIC)    ErrorReenterPrecondition );
    ...
    if (DEM_IS_EV_ST_BIT_TNCTOC_SET(EventId))
    {
      ...
      if (DEM_IS_EV_ST_BIT_TF_SET(EventId))
      {
        ...
        if (DEM_IS_EVENT_COMBINED(EventId))
        {
          ...
          NewCombinedStatus = Dem_CalculateCmbDTCStatus(DTCConfIdx);
        }
        else
        {
...
#endif
        {
          ...
          if (Dem_GbiAgingCycleIdx(EvId) == AgingCycleId)
          {
              ...
              if ((DEM_IS_EVENT_COMBINED(EvId)) && (DTCStatusChanged == TRUE))
              {
                ...
                DTCConfIdx = Dem_GbiDTCConfIdx(EvId);
                OldCombinedStatus = Dem_CmbDTCStatus[DTCConfIdx];
                NewCombinedStatus = Dem_CalculateCmbDTCStatus(DTCConfIdx);
                /*
   ...
   * events are processed (fault confirmation, event entry, debounce) */
    ...
    do
    {
      ...
      (DEM_J1939_READING_DTC_SUPPORT == STD_ON) )
        ...
        Dem_CheckTF(EventId, &ErrorReenterPrecondition);
#endif /* DEM_FREEZE_FRAME_CAPTURE == DEM_TRIGGER_ON_FDC_THRESHOLD */
...
#endif
#if (DEM_DEV_ERROR_DETECT == STD_ON)
      Dem_CheckDevError(ProcessEDAndFFResult, &Result);
#endif /* (DEM_DEV_ERROR_DETECT == STD_ON) */
#else
      /* process freeze frame data for event */
  ...
  boolean         CurrentEventStatusChanged)
{
  ...
  /* if at least one status has changed, call Dem_CalculateCmbDTCStatus */
  /* Deviation TASKING-9 */
...
#else
#if (DEM_NUM_CMB_DTCS > 0U)
     /* for combined events check if the current event owns the memory entry,
      *  if it exists */
     if ( ((EntryExists == TRUE) && (EvMemEntry->EventId == EventId)) ||
          (EntryExists == FALSE) )
#endif
                              ...
                              DEM_UDS_STATUS_TFTOC))
  {
    ...
    if (Continue == TRUE)
    {
      ...
      if (DEM_INDICATOR_CYC_COUNTER[IndicatorId][CounterDataIdx] >= HealingCycleThreshold)
      {
...
#if (DEM_MAX_NUMBER_EVENT_ENTRY_PER != 0U)
#if (DEM_MILGROUP_SUPPORT == STD_OFF)
#if (DEM_INDICATORCLASS_SUPPORT == STD_ON)
        boolean IsMIL = (IndicatorId == DEM_MIL_ID) ? TRUE : FALSE;
#else
        const uint32 IndMILStart = Dem_IndicatorDesc[DEM_MIL_ID].StartIdx;
        ...
        boolean IsMIL = ((IndicatorIdx >= IndMILStart) && (IndicatorIdx < IndMILEnd)) ? TRUE : FALSE;
#endif
#endif /* DEM_MILGROUP_SUPPORT == STD_OFF */
