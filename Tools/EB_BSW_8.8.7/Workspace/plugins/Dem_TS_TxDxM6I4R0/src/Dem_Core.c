 ...
 *
 * Reason:
 * "IndicatorUsed" is TRUE only if DEM_INDICATORCLASS_SUPPORT == STD_ON or the indicator
  ...
  boolean              FirstFailed)
{
  ...
  if (CheckThreshold == TRUE)
  {
    ...
    if (FailureCounter >= FailureThreshold)
    {
      ...
      /* MIL is not activated for suppressed DTCs */
      if ( (!IsMIL) || (Dem_ObdDTCOfEvent != DEM_NO_DTC) )
#endif
      {
...
FUNC(Dem_DTCKindType, DEM_CODE) Dem_GbiDTCKind(Dem_EventIdType EventId)
{
  ...
  if (Dem_OBDConfig[DEM_DESC_OBD_IDX(EventId)].ObdDTC != DEM_NO_DTC)
  {
  ...
  Dem_EventIdType EventId)
{
  ...
  /* If calibration of OBD variant is supported, a run time check
   * needs to be added */
  if (Dem_GbiDTCKind(EventId) == DEM_DTC_KIND_EMISSION_REL_DTCS)
  {
    Result |= DEM_SEVERITY_WWHOBD_CLASS_A;
  }
#endif
...
FUNC(boolean, DEM_CODE) Dem_GbiMinRatioEvent(Dem_EventIdType EventId)
{
  ...
  const boolean Result = (DEM_BIT_FIELD_VALUE_8(
    ...
    Dem_OBDConfig[DEM_DESC_OBD_IDX(EventId)].OBDEvConf1,
  ...
  Dem_EventIdType EventId)
{
  const Dem_EventOBDReadinessGroupType ReadinessGroup =
    (Dem_EventOBDReadinessGroupType)
      DEM_BIT_FIELD_VALUE_16(Dem_OBDConfig[DEM_DESC_OBD_IDX(EventId)].OBDEvConf1,
...
FUNC(Dem_EventStatusExtendedType, DEM_CODE) Dem_CalculateCmbDTCStatus(Dem_DTCConfIdxType DTCConfIdx)
{
  Dem_CmbDTCEvLnkType CmbEvIdx;
  Dem_EventStatusExtendedType CombinedStatus = 0x00U;

  DBG_DEM_CALCULATECMBDTCSTATUS_ENTRY(DTCConfIdx);
  ...
  DBG_DEM_CALCULATECMBDTCSTATUS_EXIT(CombinedStatus, DTCConfIdx);
...
#if ((DEM_CALIBRATION_SUPPORT == STD_ON) && (DEM_USE_AGING == STD_ON))
  if (MaximalSize == TRUE)
  {
...
STATIC FUNC(uint32, DEM_CODE) Dem_InternalGetObdDTC(Dem_EventIdType EventId)
{
  ...
  DTC = ((uint32)((uint32)Dem_OBDConfig[DEM_DESC_OBD_IDX(EventId)].ObdDTC) << 8U);
