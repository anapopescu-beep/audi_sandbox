...
STATIC FUNC(void, DEM_CODE) Dem_ResetOBDCounters(void)
{
  ...
  /* Check if was a Dcm service $04 or $14 request – Clear/reset emission-related
   * diagnostic information */
  if (
#if (DEM_OBDONUDS_SUPPORT)
      (Dem_ClearDTCSelection.DTCInfo.DTCFormat != DEM_DTC_FORMAT_J1939) &&
#else
...
STATIC FUNC(Std_ReturnType, DEM_CODE) Dem_ClearEvents(void)
{
  ...
  if (EventId > DEM_MAX_EVENTID)
  {
    ...
    else/* else: status already set to E_OK */
    {
      ...
      if ((DEM_DTC_GROUP_ALL_DTCS == (Dem_DTCGroupType)DTC) && (E_OK == Result))
      {
