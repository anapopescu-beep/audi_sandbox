...
#endif /* (DEM_ORDERED_DTC_REPORTING_USED == STD_ON) */
        /* !LINKSTO Dem.ASR21-11.SWS_Dem_01263,1 */
  ...
  uint8                      ExtendedDataRecordNumber)
{
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    ...
    /* !LINKSTO Dem.SetDTCFilterByEDRecordNumber.InvalidEDRecordNumber,1 */
...
/* !LINKSTO Dem.ASR431.SWS_Dem_01177,1 */
  ...
  P2VAR(Dem_DTCSeverityType, AUTOMATIC, DEM_APPL_DATA) DTCSeverityMask)
{
  ...
  TS_PARAM_UNUSED(ClientId);
#if (DEM_DEV_ERROR_DETECT != STD_ON)
  TS_PARAM_UNUSED(DTCSeverityMask);
#elif (DEM_DEV_ERROR_DETECT == STD_ON)
  if (Dem_InitializationState != DEM_INITIALIZED)
  {
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
    ...
    /* Below line (if condition) is needed as soon as calibration is supported */
    /* if (DEM_GET_OBD_VARIANT() == DEM_OBD_VARIANT_OBDONUDS) */
    {
      /* !LINKSTO Dem.ASR431.SWS_Dem_01181,1 */
      /* !LINKSTO Dem.GetDTCSeverityAvailabilityMask.DTCSeverityMask,1 */
      *DTCSeverityMask = DEM_SEVERITY_WWHOBD_CLASS_A;
      Result = E_OK;
    }
#endif
  }
...
#endif
          {
            /* filter DTC based on different filter type */
            switch (Dem_DTCFilter[ClientId].CurrentFilterType)
            {
...
#endif
        {
          ...
          switch (Dem_DTCFilter[ClientId].CurrentFilterType)
          {
            ...
            case DEM_DTC_FILTER_EXTDATAFILTER:
            {
              /* !LINKSTO Dem.ASR21-11.SWS_Dem_01362,1 */
              FilterResult = Dem_ApplyEDRNFilter(ClientId,
                  NextEventId, &DTCStatusValue, &DTCValue);
              break;
            }
            /* CHECK: NOPARSE */
            default: /* should never be reached */
