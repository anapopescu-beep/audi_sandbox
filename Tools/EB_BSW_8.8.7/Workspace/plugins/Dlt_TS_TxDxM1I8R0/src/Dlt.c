...
/* patch version check */
#if (DLT_SW_PATCH_VERSION < 11U)
#error DLT_SW_PATCH_VERSION wrong (< 11U)
#endif
...
#endif /* (DLT_DEFAULT_ASR_RETVAL == AUTOSAR_431) */
      {
        ...
        for (chnIdx = 0U; chnIdx < DLT_TXPDU_NO; chnIdx++)
        {
...
#endif
          {
            ...
            retVal = DLT_SEND_MESSAGE(chnIdx, session_id,
...
#if (DLT_ENABLE_BSW_DISTRIBUTION == STD_OFF)
        /* Deviation MISRAC2012-1 */
        retVal = DLT_SEND_MESSAGE(DEFAULT_LOG_CHN_IDX, session_id,
                   (P2CONST(Dlt_MessageCommonInfoType, AUTOMATIC, DLT_APPL_DATA)) log_info,
                   log_data, log_data_length, DLT_SID_SendLogMessage, timestamp);
#else
...
#if (DLT_ENABLE_BSW_DISTRIBUTION == STD_ON)
      /* Deviation MISRAC2012-1 */
      retVal = DLT_SEND_MESSAGE(ChannelsIdx, session_id,
               (P2CONST(Dlt_MessageCommonInfoType, AUTOMATIC, DLT_APPL_DATA)) log_info,
               log_data, log_data_length, DLT_SID_SendLogMessage, timestamp);
#if (DLT_DEFAULT_ASR_RETVAL == AUTOSAR_431)
      if (retVal == DLT_E_OK)
  ...
  if ((cur_log_level != DLT_LOG_OFF) && (log_info->log_level <= cur_log_level))
  {
...
#endif /* (DLT_DEFAULT_ASR_RETVAL == AUTOSAR_431) */
    {
      ...
      for (ChnIdx = 0U; ChnIdx < DLT_TXPDU_NO; ChnIdx++)
      {
        ...
        if ((LocalCoreContext.ContextIdTable[identified_ctxt_id_idx].ChannelsIdx[ChnIdx] == TRUE) &&
            (ChnIdx != DEFAULT_LOG_CHN_IDX) &&
            (log_info->log_level <= Dlt_LogChannelThresholdInfo[ChnIdx].DltLogChannelThreshold))
#endif
        {
#if (DLT_ENABLE_BSW_DISTRIBUTION == STD_OFF)
          /* !LINKSTO Dlt.swdd.DltCore.API.SendLogMessage.MessageFilterDisabled,1,
...
#endif /* #if (DLT_DEFAULT_ASR_RETVAL == AUTOSAR_431) */
{
  ...
  else
  {
...
#endif /* #if ( (DLT_DEFAULT_ASR_RETVAL == AUTOSAR_431) && (DLT_DEV_ERROR_DETECT == STD_ON) ) */
    {
      ...
      /* !LINKSTO Dlt.RegisterContext.DetOn.WrongAppCtxDescriptionLength,1 */
...
/* !LINKSTO Dlt.ASR431.Ref_SWS_Dlt_00651_MessageTrafficShapingSettings,1 */
#if (DLT_TRAFFIC_SHAPER_ENABLED == STD_ON)
   ...
   Dlt_MsgLengthType MsgLen, uint32 timestamp)
{
  ...
  if((hdrbuf[DLT_HTYP_HEADER_OFFSET] & DLT_HEADER_ECU_ID_BIT) != 0U)
  {
    ...
    hdrbuf[nextidx]      = Dlt_UserEcuId[0];
    ...
    uint32 storeindex)
{
...
#if (DLT_NOTIFY_USER_THRESHOLD_REACHED == STD_ON)
  ThresholdValue = Dlt_CfgLogChannelThreshold[chnIdx];
#endif
#else
...
#if (DLT_NOTIFY_USER_THRESHOLD_REACHED == STD_ON)
  {
    ...
    {
      len = BufferSize - Dlt_InternalVarsTxRx[chnIdx].Dlt_BufferStoreIdx - Dlt_InternalVarsTxRx[chnIdx].Dlt_BufferReadIdx;
    }

    Dlt_BSWMasterContextPtr->ExitExclusiveArea();

    /* !LINKSTO EB_Req_Dlt_InternalBufferThresholdReachedNotification,1 */
    if ((ThresholdValue != 0U) && (len >= ThresholdValue))
    {
      ...
      {
        (void)Dlt_ThresholdReachedNotification[Idx].UserCalloutThresholdReached((const Dlt_LogChannelNameType *)&logChannelName);
      }
    }
  }
#endif
...
{
  static Dlt_IOCSyncTableType Dlt_IOCSyncTable;
  ...
  /* CHECK: PARSE */
  {
    ...
    *(SatelliteCoreContext.ContextIdTableIdx)  = Dlt_IOCSyncTable.numberOfContextIds;
    ...
    do{
      ...
      i++;
    }while(i<(Dlt_IOCSyncTable.numberOfContextIds));
  }
  /* CHECK: NOPARSE */
  else
  {
