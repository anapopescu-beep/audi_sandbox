 ...
 * \pre All input parameters are valid
 *
 * \return Profile 11 communication status value of the passed data
 */
static FUNC(E2E_P11CheckStatusType, E2E_CODE) E2E_EB_P11Check
(
  P2CONST(E2E_P11ConfigType, AUTOMATIC, E2E_APPL_CONST) ConfigPtr,
...
)
{
   ...
   else
   {
      ...
      const E2E_P11CheckStatusType E2EP11_EB_Status = E2E_EB_P11Check(
              ConfigPtr,
              &(StatePtr->Counter),
              DataPtr);

        /* map generic profile status to profile specific status value */
        switch (E2EP11_EB_Status)
        {
...
)
{
   ...
   {
      Crc = SCrc_CalculateCRC8(&DataPtr[1], ((uint32)Length - 1U), Crc, FALSE);
   }
...
{
    E2E_P11CheckStatusType RetVal = E2E_P11STATUS_ERROR;
    /* If DataPtr equals NULL, then no data was received in this cycle. */
    const boolean NewDataAvailable = (boolean)((DataPtr != NULL_PTR) ? TRUE : FALSE);
       ...
       /* Read DataIDNibble value which is placed at different positions depending on tx of Length */
        ...
        else
        {
            ...
            const uint8 DeltaCounter = (ReceivedCounter >= LastStateCounter) ?
                  ...
                  (uint8)((E2EP11_COUNTER_MAX +  ReceivedCounter + 1U )
                      - LastStateCounter);
            ...
            if (DeltaCounter > MaxDeltaCounter)
            {
                ...
                RetVal = E2E_P11STATUS_WRONGSEQUENCE;
            }
    ...
    else
    {
        ...
        RetVal = E2E_P11STATUS_NONEWDATA;
    }
