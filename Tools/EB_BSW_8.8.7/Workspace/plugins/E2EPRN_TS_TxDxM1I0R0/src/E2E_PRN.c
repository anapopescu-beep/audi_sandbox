...
)
{
   ...
   /* If arraylength is less than 8U */
   if (((uint16)8U) > (uint16)ArrayLength)
   {
      /* Initialize the 8 byte buffer with initial value as 0 */
      uint8 Buffer[8U] = {0x00U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U};

      Crc = SCrc_CalculateCRC8(&Buffer[ArrayLength],
                  ((uint32)8U - ((uint32)ArrayLength)), Crc, FALSE);
   }
