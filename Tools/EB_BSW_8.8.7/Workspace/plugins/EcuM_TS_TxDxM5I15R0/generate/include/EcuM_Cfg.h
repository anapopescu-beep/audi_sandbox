...
[!VAR "PREHALT_SLAVE" = "node:exists(as:modconf('EcuM')/EcuMGeneral/EcuMPreHaltSlaveCallout) and node:value(as:modconf('EcuM')/EcuMGeneral/EcuMPreHaltSlaveCallout) = 'true'"!]
[!VAR "WKS_LASTISBEST" = "node:value(as:modconf('EcuM')/EcuMGeneral/EcuMBswMWakeupIndicationLastIsBest) = 'true'"!]
...
((EcuM_WakeupSourceType)1U << EcuMConf_[!"name(as:ref(.)/..)"!]_[!"name(as:ref(.))"!]) | \
[!ENDLOOP!][!//
0U)
[!ENDLOOP!][!//
