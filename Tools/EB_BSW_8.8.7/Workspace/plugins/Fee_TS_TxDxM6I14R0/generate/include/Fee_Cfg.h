...
[!IF "((node:exists(FeeGeneral/FeeMemAccUsage)) and (FeeGeneral/FeeMemAccUsage = 'true'))"!][!//
#include <MemAcc.h>
[!ELSE!][!//
/* !LINKSTO FEE084,1, FEE002,1 */
...
#endif
[!IF "(node:exists(FeeMemAccUsage) and (FeeMemAccUsage = 'true'))"!][!//
...
[!IF "(node:exists(FeeUseBufferForJobs)) and (FeeUseBufferForJobs = 'true') "!][!//
[!IF "(node:exists(FeeMemAccUsage) and (FeeMemAccUsage = 'true'))"!][!//
#define FEE_ALIGNMENT_VALUE [!"node:value(node:ref(FeeGeneral/FeeBufferAlignmentValue)/MemAccBufferAlignmentValue)"!]U
[!ELSE!]
...
[!IF "(as:modconf('Fls')[1]/FlsGeneral/FlsCancelApi = 'true') or (FeeGeneral/FeeMemAccUsage = 'true')"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]
...
[!IF "FeeGeneral/FeeMemAccUsage = 'false'"!]
...
#define FEE_CANCEL_FLASHJOB() \
  do\
  {\
[!IF "((as:modconf('Fls')[1]/FlsGeneral/FlsCancelApi = 'true') or (FeeGeneral/FeeMemAccUsage = 'true'))"!]
