...
)
{
  ...
  do{
    ...
    {
    case FEE_SECTION_ACTIVE:
      /* If the working section is a consistent active section with the correctly increased section counter
         with respect to the previous Active section found, update the Active section index */
      ...
      {
      /* If the working section is a consistent active section with the correct section counter
         with respect to the previous oldest active section found, update the oldest section index */
        DiferenceBetweenSectionIndexes = FEE_NUMBER_OF_SECTIONS - (Fee_Gv.FeeWorkingSectionIndex - Fee_Gv.FeeOldestSectionIndex);
        if(Fee_Gv.Fee_SectionManagement[Fee_Gv.FeeOldestSectionIndex].SCounter == FEE_BYTESUM(Fee_Gv.Fee_SectionManagement[Fee_Gv.FeeWorkingSectionIndex].SCounter,
            DiferenceBetweenSectionIndexes))
        {
        ...
        {
          /* The current section is not older then the oldest section found */
        }
      }
    ...
    case FEE_SECTION_NO_FOOTER:
      /* The oldest empty section need to be found in case there are no active section present. */
      if(ActiveSectionFound == FALSE)
      {
        if(EmptySectionFound == TRUE)
        {
          /* If the working section is an older Empty section than the current Empty section, update the Empty section index */
          DiferenceBetweenSectionIndexes = Fee_Gv.FeeWorkingSectionIndex - OldestEmptySectionIndex;
          if(Fee_Gv.Fee_SectionManagement[Fee_Gv.FeeWorkingSectionIndex].SCounter != FEE_BYTESUM(Fee_Gv.Fee_SectionManagement[OldestEmptySectionIndex].SCounter,
              DiferenceBetweenSectionIndexes))
          {
            OldestEmptySectionIndex = Fee_Gv.FeeWorkingSectionIndex;
          }
          else
          {
            /* The current empty section is newer. Ignore it. */
          }
        }
        else
        {
      ...
      {
        /* Active sections exist. The oldest empty section doesn't matter any more. */
      }
      break;
    ...
    if(ActiveSectionFound == FALSE)
    {
      /* The active section could only be at a higher index. */
      Fee_Gv.FeeActiveSectionIndex = FEE_SECTIONINCREMENT(Fee_Gv.FeeWorkingSectionIndex);
    ...
    /* Exits when overflow occurs. */
  }while(Fee_Gv.FeeWorkingSectionIndex != 0U);
...
#endif
      /* !LINKSTO Fee.Dsn.Initialization.Startup1.Sections.09,1 */
      /* If empty sections are found, the oldest one will be considered the Active section,
       * otherwise take 0 as a reference index. */
      Fee_Gv.FeeActiveSectionIndex = OldestEmptySectionIndex;
      Fee_Gv.FeeOldestSectionIndex = OldestEmptySectionIndex;
  }
  else
  {
