...
[!ENDIF!]
    [!SELECT "FrNmChannelIdentifiers"!]
    {
      ...
      [!IF "$SynchronizedPncShutdownEnabled = 'true'"!]
        [!IF "node:exists(FrNmSynchronizedPncShutdownEnabled) and (FrNmSynchronizedPncShutdownEnabled = 'true')"!]
      ...
      FRNM_GATEWAY_TYPE_UNUSED, /* CanNmTypeOfGateway */
          [!ENDIF!]
        [!ELSE!] [!/* [!IF "node:exists(FrNmSynchronizedPncShutdownEnabled) and (FrNmSynchronizedPncShutdownEnabled = 'true')"!] */!]
          [!IF "node:exists(as:modconf('FrNm')[1]/FrNmGlobalConfig/FrNmGlobalProperties/FrNmPnShutdownMessageRetransmissionDuration)"!]
      0U,  /*FrNmPnShutdownMessageRetransmissionDuration*/  
          [!ENDIF!] [!/**/!] 
      FRNM_GATEWAY_TYPE_UNUSED, /* CanNmTypeOfGateway */       
        [!ENDIF!] [!/* [!IF "node:exists(FrNmSynchronizedPncShutdownEnabled) and (FrNmSynchronizedPncShutdownEnabled = 'true')"!] */!]
      [!ENDIF!] [!/*[!IF "$SynchronizedPncShutdownEnabled = 'true'"!]*/!]
#if (FRNM_PN_ERA_CALC_ENABLED == STD_ON)
      [!IF "(node:exists(FrNmPnEraCalcEnabled)) and (FrNmPnEraCalcEnabled='true')"!][!//
      1U, /* EraCalcEnabled */
      [!ELSE!]
