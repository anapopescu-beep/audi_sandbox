...
/* patch version check */
#if (IPDUM_SW_PATCH_VERSION < 50U)
#error IPDUM_SW_PATCH_VERSION wrong (< 50U)
#endif
 ...
 * \retval FALSE  PDU already exists in the container.
  */
IPDUM_STATIC FUNC(boolean, IPDUM_CODE) IsPduNewToContainer
(
...
#endif /* IPDUM_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    else
    {
      if(0U < PduInfoPtr->SduLength)
      {
...
#endif /* IPDUM_CNTND_COLLECT_QUEUED_TX == STD_ON */
#if (IPDUM_USE_METADATA == STD_ON)
        uint32 PduMetaData = 0U;
        uint32 ResultMetaData = 0U;
        ...
        if((IPDUM_CFG_GET_CNTR_TX(ParentContainerIndex)->ContainerMetaDataLength) > 0)
        {
          if(ContainedPdu->ContainedMetaDataLength > 0)
          {
            (void) EcuC_GetMetaDataCanId(ContainedPdu->ContainedMetaDataPduId, PduInfoPtr, &PduMetaData);
          }
         /*Calculate Metadata even no metadata configured for that Contained Pdu, these for PDUs that has no metadata configured as some modules don't support metadata as Com module.*/
           /*In addtion, we need to overwrite some parts from the getting metaData as CanId32 type
           has four bytes , We care only for the first two least bytes (SA & DA) and the reamining should be discarded => SAE J1939-22*/
          ResultMetaData = (PduMetaData & ContainedPdu->IpduMTxMetaDataMask) | (ContainedPdu->IpduMTxMetaDataValue & (~ContainedPdu->IpduMTxMetaDataMask));
        }
#endif /* IPDUM_USE_METADATA == STD_ON */
        ...
        {
#if (IPDUM_USE_METADATA == STD_ON)
          RetValue = IpduM_CntrTransmitLB(ParentContainerIndex, ContainedPduIndex, ResultMetaData);
#else
...
#if (IPDUM_USE_METADATA == STD_ON)
  uint8 PduMetaData[CONTAINERRXMETADATAMAXLENGTH];
  PduInfoType LocalPduForMetadata;
  LocalPduForMetadata.SduDataPtr = NULL_PTR;
  LocalPduForMetadata.SduLength = 0U;

  /* !LINKSTO IpduM.EB.MetaData.Rx.GetMetadata,1 */
  if(ContainerPdu->ContainerMetaDataLength > 0)
  {
        ...
        if ((ContainedDLCLength + HeaderSize) > ContainedDLCLength)
        {
          ...
          if(RemainLen >= (ContainedDLCLength + HeaderSize))
          {
            ...
            if(E_OK == IpduM_RxCntd_BinSearch(ContainerPdu, &outGoingPduId, &Contained_Rx_Idx,  IPDUM_SWAPU32_IF_MUST(PduHeaderId)))
            {
              ...
              if(ContainerPdu->ContainerMetaDataLength > 0 && ContainedPdu->ContainedMetaDataLength > 0)
              {
    ...
    while(IPDUM_TXCNTR_EV_NONE != event)
    {
      ...
      switch(state)
      {
        ...
        case IPDUM_TXCNTR_ST_WAIT_TX:
        {
          ...
          boolean added = FALSE;
#if (IPDUM_USE_METADATA == STD_ON)
          boolean triggerCntrDiffMetaData = FALSE;
          /* In Case the Container is configured with MetaData*/
          if(( txCntrPtr->ContainerMetaDataLength) > 0)
          {
            if(instPtr->NumCntd == 0U)
            {
             /* Empty Container , set MetaData for that Container by the first receive request*/
               instPtr->MetaDataValue = MetaDataValue;
            }
            else
            {
            /* Non Empty Container:  check if the new request has the same metadata or not */
            /*In case different MetaData, Trigger the current Container Instance for transmision
             and add the new request with new MetaData value to a new Container instance. */
              if(instPtr->MetaDataValue != MetaDataValue)
              {
                triggerCntrDiffMetaData = TRUE; /*Trigger the Cntr as differenet MetaData*/
              }
            }
          }
#endif /* IPDUM_USE_METADATA == STD_ON */
          ...
          if(TRUE == IsPduNewToContainer(instPtr,ContainerIdx,IdxContained))
          {
          /* Does it fit ? */
#if (IPDUM_USE_METADATA == STD_ON)
            ...
            {
                triggerCntr = TRUE; /*either exceed the size or receive different Metadata Values*/
            }
          }
...
/* Set MetaData for Tx Container */
        ...
        if(TxCntrPtr->ContainerMetaDataLength > 0)
        {
          ...
          (void) EcuC_SetMetaDataCanId(TxCntrPtr->ContainerMetaDataPduId, &LocalPduForMetadata, (InstPtr->MetaDataValue));
        }
#endif /* IPDUM_USE_METADATA == STD_ON */
...
#if (IPDUM_CANFD_PADDING_SUPPORT == STD_ON)
      /* Check if container has the CANFD padding service enabled */
      if(IPDUM_CNTRTX_HAS_CANFD_PADDING(TxCntrPtr->CfgFlags) && (isDirect))
      {
        /* Perform the padding service */
        IpduM_CanFdPaddingService(&pduInfo, TxCntrPtr);
      }
    ...
    while(IPDUM_TXCNTR_EV_NONE != event)
    {
      ...
      switch(state)
      {
        ...
        case IPDUM_TXCNTR_ST_WAIT_TX:
        {
          ...
          {
            triggerCntr = TRUE; /*either exceed the size or receive differenet Metadata Values*/
          }
...
)
{
  ...
  if (interval != IPDUM_INVALID_CANFD_INTERVAL)
  {
    ...
    if ((newLength - pduInfoPtr->SduLength) > 3)
    {
      /* Set the 00 header */
      ...
      /* Set the AA padding */
      TS_MemSet(&pduInfoPtr->SduDataPtr[pduInfoPtr->SduLength], IPDUM_CANFD_PADDING_VALUE, newLength - pduInfoPtr->SduLength);
    ...
    {
      /* Set the 00 header */
      TS_MemSet(&pduInfoPtr->SduDataPtr[pduInfoPtr->SduLength], 0U, newLength - pduInfoPtr->SduLength);
