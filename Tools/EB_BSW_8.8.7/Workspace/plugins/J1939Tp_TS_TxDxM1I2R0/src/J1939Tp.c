...
static FUNC(void, J1939TP_CODE) J1939Tp_BAM_Transmit(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_BAM_Transmit_Data(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Transmit(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Transmit_Data(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Wait_CTS(P2VAR(J1939Tp_NSduCtrlType, AUTOMATIC, J1939TP_APPL_DATA) nsdu_ctrl_ptr);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Wait_EOM(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

#if (J1939TP_TX_CANFD_SUPPORT == STD_ON)
static FUNC(Std_ReturnType, J1939TP_CODE) J1939Tp_BAM_Set_EOMS_Frame(P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr, uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_FD_Data_Transmition (P2VAR(PduInfoType,AUTOMATIC,J1939TP_APPL_DATA) tPduInfo,uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_BAM_Transmit_FD_Data(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_BAM_Transmit_Send_Abort(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_BAM_Transmit_Data_Timeout(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_FD_Transmit_Data(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(uint8, J1939TP_CODE) J1939Tp_FD_Transmit_Calculate_Frame_Size(uint8 data);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_FD_Transmit_EOMS(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_EOMS_Handling(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_TxConfirmation_CMDT_EOMS(P2VAR(J1939Tp_NSduCtrlType, AUTOMATIC, J1939TP_APPL_DATA) nsdu_ctrl_ptr, uint8 Chan_Idx, Std_ReturnType result);

static FUNC(uint8 , J1939TP_CODE) J1939Tp_Set_Session_Number(uint8 MaxSessions, uint8 u8SA_val, uint8 u8DA_val);
#endif

static FUNC(void, J1939TP_CODE) J1939Tp_CM_BAM_Transmit(P2VAR(J1939Tp_NSduCtrlType,AUTOMATIC,J1939TP_APPL_DATA) nsdu_ctrl_ptr, uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Transmit_Data_Timeout(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_DirectPg_Transmit(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_DirectPg_Data_Transmit(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_BAM_Reception(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_BAM_Reception_CopyRxData(P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr, uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Reception(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_Rx_CMDT_Wait_TxConfirmation(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

#if (J1939TP_RX_CANFD_SUPPORT == STD_ON)
static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Wait_EOMS(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_CM_EOMS(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3, P2VAR(uint8, AUTOMATIC, J1939TP_APPL_DATA) SduDataPtr);
#endif

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Reception_CTS(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3, boolean request_EOMS);
...
static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Wait_Data(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Reception_Data_Complete(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Reception_CTS0(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Reception_RTS(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Reception_Send_Abort(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Transmit_RTS(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Build_RTS(P2VAR(uint8, AUTOMATIC, J1939TP_APPL_DATA) tBuf, uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(boolean, J1939TP_CODE) J1939Tp_Calculate_MaxPacketsPerBlock(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3,P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) tPduInfo,P2VAR(uint8, AUTOMATIC, J1939TP_APPL_DATA) tBuf);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Transmit_Send_Abort(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Reception_CopyRxData(P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr, uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_Reinit_Tx(uint8 Chan_Idx, uint8 Pg_Idx, uint8 Nsdu_Idx);

static FUNC(void, J1939TP_CODE) J1939Tp_Reinit_Rx(uint8 Chan_Idx, uint8 Pg_Idx, uint8 Nsdu_Idx);
...
static FUNC(J1939TP_FrameStateType, J1939TP_CODE) J1939Tp_Abort_Conn(uint8 reason, P2CONST(J1939Tp_TxChannelType,AUTOMATIC,J1939TP_APPL_CONST) TxChannel, P2CONST(J1939Tp_RxChannelType,AUTOMATIC,J1939TP_APPL_CONST) RxChannel, uint32 pgn, uint8 Sa, uint8 Da, uint8 session_num, P2VAR(J1939Tp_NsduAbortCtrlType,AUTOMATIC,J1939TP_APPL_DATA) nsdu_abort_ptr);

static FUNC(J1939TP_FrameStateType, J1939TP_CODE) J1939Tp_EndOfMsgAck(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_TxTimerJob(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_RxTimerJob(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);
...
static FUNC(void, J1939TP_CODE) J1939Tp_SendFCWait_CTS(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3);

static FUNC(void, J1939TP_CODE) J1939Tp_NrOfRxPacketsInCts(P2VAR(J1939Tp_NSduCtrlType, AUTOMATIC, J1939TP_APPL_DATA) nsdu_ctrl_ptr, uint8 loop_idx1,PduLengthType tRxbufferSize);

static FUNC(boolean, J1939TP_CODE) J1939Tp_CheckAvailableBuffer(P2VAR(J1939Tp_NSduCtrlType, AUTOMATIC, J1939TP_APPL_DATA) nsdu_ctrl_ptr, uint8 loop_idx1,PduLengthType tRxbufferSize);
...
static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_CM_Abort(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3, P2VAR(uint8, AUTOMATIC, J1939TP_APPL_DATA) SduDataPtr);
...
static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_CM(uint8 Chan_Idx, P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr);

static FUNC(boolean, J1939TP_CODE) J1939Tp_CM_Validation(uint8 Chan_Idx, P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr, P2VAR(J1939Tp_SduInfoType, AUTOMATIC, J1939TP_APPL_DATA) SduInfo);

static FUNC(boolean, J1939TP_CODE) J1939Tp_CM_Type_check(uint8 Chan_Idx, P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr, P2VAR(J1939Tp_SduInfoType, AUTOMATIC, J1939TP_APPL_DATA) SduInfo, P2VAR(boolean, AUTOMATIC, J1939TP_APPL_DATA) CorrectLength, P2VAR(boolean, AUTOMATIC, J1939TP_APPL_DATA) ValidSession);

static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_CM_DT(PduIdType RxPduId,uint8 Chan_Idx, P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr);

static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_DT(uint8 Chan_Idx,P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr,uint8 u8SA_val,uint8 u8DA_val );

static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_TxFcN_CM(uint8 Chan_Idx, P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr);

static FUNC(boolean, J1939TP_CODE) J1939Tp_Validate_TxFcN_CM_Length(uint8 Chan_Idx, P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr, P2VAR(uint8, AUTOMATIC, J1939TP_APPL_DATA) CM_Type_ptr, P2VAR(uint32, AUTOMATIC, J1939TP_APPL_DATA) NrPackets_ptr, P2VAR(uint32, AUTOMATIC, J1939TP_APPL_DATA) MsgLength_ptr, P2VAR(uint32, AUTOMATIC, J1939TP_APPL_DATA) c_pgn_ptr);

static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_TxFcN_CTS(uint8 Chan_Idx,uint8 Pg_Idx,uint8 Nsdu_Idx,P2VAR(uint8, AUTOMATIC, J1939TP_APPL_DATA) c_RxDataPtr);

static FUNC(Std_ReturnType, J1939TP_CODE) J1939Tp_Tx_Store_MetaData(P2VAR(uint8, AUTOMATIC, J1939TP_APPL_DATA) metaDataInfo, uint8 Chan_Idx, uint8 Pg_Idx, uint8 Nsdu_Idx);

static FUNC(Std_ReturnType, J1939TP_CODE) J1939Tp_Transmit_Internal(uint8 Chan_Idx, uint8 Pg_Idx, uint8 Nsdu_Idx, uint8 u8SA_val, uint8 u8DA_val, uint8 MetaDataLen, uint8 metaDataInfo[]);

static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_DirectPg(uint8 Chan_Idx,uint8 Pg_Idx,P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr,uint32 metaDataCanId);
...
static FUNC(void, J1939TP_CODE) J1939Tp_TxConfirmation_CM(uint8 Chan_Idx , boolean IsControl, Std_ReturnType result);

static FUNC(void, J1939TP_CODE) J1939Tp_TxConfirmation_CMDT_Data(P2VAR(J1939Tp_NSduCtrlType, AUTOMATIC, J1939TP_APPL_DATA) nsdu_ctrl_ptr, uint8 Chan_Idx, Std_ReturnType result);
...
static FUNC(void, J1939TP_CODE) J1939Tp_FCNPduMetaData_Handle(uint8 FcNPduMetaData, uint8 CmNPduMetaData, P2VAR(J1939Tp_NSduCtrlType, AUTOMATIC, J1939TP_APPL_DATA) nsdu_ctrl_ptr, uint8 MetaDataInfo[], uint8 loop_idx1);
...
static FUNC(void, J1939TP_CODE) J1939Tp_TxPgNsduSerialization(uint8 Chan_Idx,uint8 Pg_Idx);

static FUNC(void, J1939TP_CODE) J1939Tp_TxChannelPgSerialization(uint8 Chan_Idx);

static FUNC(void, J1939TP_CODE) J1939Tp_RxPgNsduSerialization(uint8 Chan_Idx,uint8 Pg_Idx);

static FUNC(void, J1939TP_CODE) J1939Tp_RxChannelPgSerialization(uint8 Chan_Idx);

/* BAM Wait Confirmation state handling*/
static FUNC(void, J1939TP_CODE) J1939Tp_BAM_NsduTxWaitConfirmationHandling(uint8 Chan_Idx, uint8 Pg_Idx, uint8 Nsdu_Idx);

#if (J1939TP_CANCELLATION_SUPPORT == STD_ON)
static FUNC(void, J1939TP_CODE) J1939Tp_BAM_CancelationHandle(uint8 Chan_Idx, uint8 Pg_Idx, uint8 Nsdu_Idx);
#endif
...
static FUNC(void, J1939TP_CODE) J1939Tp_CM_BAM_Transmit(P2VAR(J1939Tp_NSduCtrlType,AUTOMATIC,J1939TP_APPL_DATA) nsdu_ctrl_ptr, uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3)
{
    ...
    MetaDataInfo[0] = (J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).J1939TpTxSa;
    MetaDataInfo[1] = (J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).J1939TpTxDa;
...
#endif
    {
        ...
        {
            J1939Tp_NPduMetaData_Handle(CmNPduMetaData, TxNSduMetaData, nsdu_ctrl_ptr, MetaDataInfo, pgn, (J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).J1939TpTxCmNPduTxConfIdEcuC);
        }
        ...
        && ((nsdu_ctrl_ptr->TbamminTimeElapsed == TRUE) || (nsdu_ctrl_ptr->data_cnt >= nsdu_ctrl_ptr->msglength)))
        {
            ...
            else
            {
                ...
                if((TRUE == (J1939Tp_Cfg_Ptr->J1939TpTxChannel[Chan_Idx]).J1939TpTxCanFDSupport)
                    ...
                    &&(J1939TP_BAM_TX_STATE_WAIT_CONFIRMATION_DT == nsdu_ctrl_ptr->state))
                {
    ...
    else if ((J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).ChannelStatus->J1939TpTxDtNPduAvailable == TRUE)
    {
        ...
        switch (tBufReq_ReturnType)
        {
            case BUFREQ_OK:
            {
                ...
                {
                    J1939Tp_NPduMetaData_Handle(DtNPduMetaData,TxNSduMetaData,nsdu_ctrl_ptr,MetaDataInfo,J1939TP_TP_DT_PGN,(J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).J1939TpTxDtNPduTxConfIdEcuC);
                }
                /* Update the state before the calling of CanIf_Transmit as fast confirmation may be received within the call of CanIf_Transmit*/
...
static FUNC(void, J1939TP_CODE) J1939Tp_BAM_Transmit_FD_Data(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3)
{
    ...
    /* For Issue 260555 */
    MetaDataInfo[0U] = (J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).J1939TpTxSa;
    MetaDataInfo[1U] = (J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).J1939TpTxDa;
    ...
    else if ((J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).ChannelStatus->J1939TpTxDtNPduAvailable == TRUE)
    {
        ...
        if(BUFREQ_OK == tBufReq_ReturnType)
        {
            ...
            if(DtNPduMetaData > 0U)
            {
                ...
                J1939Tp_NPduMetaData_Handle(DtNPduMetaData,TxNSduMetaData,nsdu_ctrl_ptr,MetaDataInfo,J1939TP_CANFD_TP_DT_PGN,(J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).J1939TpTxDtNPduTxConfIdEcuC);
            }
            /* Update the state before the calling of CanIf_Transmit as fast confirmation may be received within the call of CanIf_Transmit*/
            (J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).ChannelStatus->J1939TpTxDtNPduAvailable = FALSE;
            /* Change the state */
...
uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3)
{
    /* Local Variables */
    BufReq_ReturnType tBufReq_ReturnType = BUFREQ_E_NOT_OK;
    ...
    if (nsdu_ctrl_ptr->sn == sn)
    {
        ...
        switch (tBufReq_ReturnType)
        {
            ...
            case BUFREQ_E_BUSY:
            {
                /* !LINKSTO EB_Ref.SWS_J1939Tp_00040.CopyRxData.BAM,1 */
...
static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Reception_RTS(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3)
{
    ...
    MetaDataInfo[METADATA_SA_INDEX_0] = (J1939Tp_Cfg_Ptr->J1939TpRxChannel[loop_idx1]).J1939TpRxDa;
    MetaDataInfo[METADATA_DA_INDEX_1] = (J1939Tp_Cfg_Ptr->J1939TpRxChannel[loop_idx1]).J1939TpRxSa;
    ...
    if(MetaDataLen > 0U)
    {
        J1939Tp_SetEcuCMetaDataRx(MetaDataLen,MetaDataInfo,nsdu_cfg_ptr->J1939TpRxNSduIdEcuC);
    }
    ...
    switch (tBufReq_ReturnType)
    {
        ...
        case BUFREQ_E_OVFL:
        {
            /* !LINKSTO EB_Ref.SWS_J1939Tp_00040.StartOfReception.CMDT,1 */
            /* !LINKSTO EB_Ref.SWS_J1939Tp_00173.StartOfReception.CMDT,1 */
            ...
            nsdu_ctrl_ptr->AbortReason = (uint8)1U;
            break;
        }
...
static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Reception_Send_Abort(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3)
{
    ...
    FrameState = J1939Tp_Abort_Conn(nsdu_ctrl_ptr->AbortReason, NULL_PTR, (&(J1939Tp_Cfg_Ptr->J1939TpRxChannel[loop_idx1])), nsdu_ctrl_ptr->pgn, nsdu_ctrl_ptr->da, nsdu_ctrl_ptr->sa, nsdu_ctrl_ptr->session_num, &Nsdu_AbortInfo); /*RxChannel: Swap SA and DA */
    ...
    {  /*FrameState == J1939TP_FRAME_PENDING*/
        ...
        {
            #if (J1939TP_DEV_ERROR_DETECT == STD_ON)
            (void)Det_ReportError(J1939TP_MODULE_ID, J1939TP_INSTANCE_ID, J1939TP_RXINDICATION_SERVICEID, J1939TP_E_TIMEOUT_TR);
            #endif
            /* Reset tr timeElapsed flag */
            nsdu_ctrl_ptr->TrTimeElapsed = FALSE;
...
static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Transmit_Send_Abort(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3)
{
    ...
    FrameState = J1939Tp_Abort_Conn(nsdu_ctrl_ptr->AbortReason, (&(J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1])), NULL_PTR, nsdu_ctrl_ptr->pgn, nsdu_ctrl_ptr->sa, nsdu_ctrl_ptr->da, nsdu_ctrl_ptr->session_num, &Nsdu_AbortInfo);
    /* change the state */
    if(FrameState == J1939TP_FRAME_SENT)
    {
     /* Already updated the state before calling CanIf_transmit as ealier confirmation may be received*/
...
static FUNC(void, J1939TP_CODE) J1939Tp_BAM_Transmit_Send_Abort(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3)
{
    ...
    FrameState = J1939Tp_Abort_Conn(nsdu_ctrl_ptr->AbortReason, (&(J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1])), NULL_PTR, nsdu_ctrl_ptr->pgn, nsdu_ctrl_ptr->sa, 0xFF, nsdu_ctrl_ptr->session_num, &Nsdu_AbortInfo);
    /* change the state */
    if(FrameState == J1939TP_FRAME_SENT)
    {
     /* Already updated the state before calling CanIf_transmit as ealier confirmation may be received*/
...
static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Wait_CTS(P2VAR(J1939Tp_NSduCtrlType, AUTOMATIC, J1939TP_APPL_DATA) nsdu_ctrl_ptr)
{
    ...
    if ((nsdu_ctrl_ptr->T3TimeElapsed != FALSE) ||(nsdu_ctrl_ptr->T4TimeElapsed != FALSE))
    {
        ...
        { /* !LINKSTO EB_Ref.SWS_J1939Tp_00100.Tx.T4,1 */
            (void)Det_ReportError(J1939TP_MODULE_ID, J1939TP_INSTANCE_ID, J1939TP_RXINDICATION_SERVICEID, J1939TP_E_TIMEOUT_T4);
        }
        else
        {
            /* !LINKSTO EB_Ref.SWS_J1939Tp_00100.Tx.T3,1 */
            /* !LINKSTO EB_Ref.SWS_J1939Tp_00234.E_TIMEOUT_T3,1 */
            (void)Det_ReportError(J1939TP_MODULE_ID, J1939TP_INSTANCE_ID, J1939TP_RXINDICATION_SERVICEID, J1939TP_E_TIMEOUT_T3);
        }
#endif
...
#if (J1939TP_DEV_ERROR_DETECT == STD_ON)
            (void)Det_ReportError(J1939TP_MODULE_ID, J1939TP_INSTANCE_ID, J1939TP_RXINDICATION_SERVICEID, J1939TP_E_TIMEOUT_T5);
#endif
            /* Stop T5 timer */
            J1939Tp_NSDU_ChangeState(nsdu_ctrl_ptr, J1939TP_CMDT_TX_STATE_ABORT);
            nsdu_ctrl_ptr->AbortReason = J1939TP_TPCM_ABORT_REASON_TIMEOUT;
        ...
        switch (tBufReq_ReturnType)
        {
            case BUFREQ_OK:
            {
                ...
                if(DirectNPduMetaData > 0U)
                {
                    ...
                    J1939Tp_NPduMetaData_Handle(DirectNPduMetaData, TxNSduMetaData, nsdu_ctrl_ptr, MetaDataInfo, pgn, ((J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).J1939TpTxPg[loop_idx2]).J1939TpTxDirectNPduTxConfIdEcuC);
                }
                /* Update the state before the calling of CanIf_Transmit as fast confirmation may be received within the call of CanIf_Transmit*/
...
#if (J1939TP_DEV_ERROR_DETECT == STD_ON)
                (void)Det_ReportError(J1939TP_MODULE_ID, J1939TP_INSTANCE_ID, J1939TP_RXINDICATION_SERVICEID, J1939TP_E_TIMEOUT_T1);
#endif
        ...
        if((RtsMsgLen == nsdu_ctrl_ptr->msglength) && (RtsNoOfPackets == nsdu_ctrl_ptr->RxNoPacketsInRTS))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    /* !LINKSTO J1939Tp.EB.FD.CMDT.AssuranceData.MisMatch,1 */
                    J1939Tp_NSDU_ChangeState(nsdu_ctrl_ptr, J1939TP_CMDT_RX_STATE_ABORT);
...
static FUNC(void, J1939TP_CODE) J1939Tp_CMDT_Wait_Data(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3)
{
    ...
    if((nsdu_ctrl_ptr->T2TimeElapsed == TRUE) || (nsdu_ctrl_ptr->T1TimeElapsed == TRUE))
    {
        ...
        else
        {
            ...
            (void)Det_ReportError(J1939TP_MODULE_ID, J1939TP_INSTANCE_ID, J1939TP_RXINDICATION_SERVICEID, J1939TP_E_TIMEOUT_T2);
        }
#endif
        /* Reception is abort and Send the Abort Frame */
        J1939Tp_NSDU_ChangeState(nsdu_ctrl_ptr, J1939TP_CMDT_RX_STATE_ABORT);
    ...
    else if ((J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).ChannelStatus->J1939TpTxCmNPduAvailable == TRUE)
    {
        ...
        if(SendRTS == TRUE)
        {
            ...
            if(CmNPduMetaData>0U)
            {
                ...
                if(TRUE == (J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).J1939TpTxCanFDSupport)
                {
                    J1939Tp_NPduMetaData_Handle(CmNPduMetaData,TxNSduMetaData,nsdu_ctrl_ptr,MetaDataInfo,J1939TP_CANFD_TP_CM_PGN,(J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).J1939TpTxCmNPduTxConfIdEcuC);
                }
                else
#endif
                {
                    J1939Tp_NPduMetaData_Handle(CmNPduMetaData,TxNSduMetaData,nsdu_ctrl_ptr,MetaDataInfo,J1939TP_TP_CM_PGN,(J1939Tp_Cfg_Ptr->J1939TpTxChannel[loop_idx1]).J1939TpTxCmNPduTxConfIdEcuC);
                }
            }
            /* Update the state before the calling of CanIf_Transmit as fast confirmation may be received within the call of CanIf_Transmit*/
...
#if (J1939TP_DEV_ERROR_DETECT == STD_ON)
        (void)Det_ReportError(J1939TP_MODULE_ID, J1939TP_INSTANCE_ID, J1939TP_RXINDICATION_SERVICEID, J1939TP_E_TIMEOUT_TH);
#endif
...
static FUNC(void, J1939TP_CODE) J1939Tp_RxChannelAbortHandler(P2CONST(J1939Tp_RxChannelType,AUTOMATIC,J1939TP_APPL_CONST) RxChannel)
{
    ...
    {
        /* Send the Abort Frame - Call the local function to abort */
        FrameState = J1939Tp_Abort_Conn(ChannelAbort->AbortReason, NULL_PTR, RxChannel, ChannelAbort->pgn , ChannelAbort->sa, ChannelAbort->da, ChannelAbort->session_num,&Nsdu_AbortInfo); /*RxChannel: Swap SA and DA */
        /* Reinit ChannelAbortType struct */
...
static FUNC(J1939TP_FrameStateType, J1939TP_CODE) J1939Tp_Abort_Conn(uint8 reason, P2CONST(J1939Tp_TxChannelType,AUTOMATIC,J1939TP_APPL_CONST) TxChannel, P2CONST(J1939Tp_RxChannelType,AUTOMATIC,J1939TP_APPL_CONST) RxChannel, uint32 pgn, uint8 Sa, uint8 Da, uint8 session_num, P2VAR(J1939Tp_NsduAbortCtrlType,AUTOMATIC,J1939TP_APPL_DATA) nsdu_abort_ptr)
{
    ...
    DBG_J1939TP_ABORT_CONN_ENTRY(reason,TxChannel,RxChannel,pgn,Sa,Da,session_num,nsdu_abort_ptr);
    if(TxChannel != NULL_PTR)
    {
        ...
        EcuCId = TxChannel->J1939TpTxCmNPduTxConfIdEcuC;
#if (J1939TP_TX_CANFD_SUPPORT == STD_ON)
    ...
    else
    {
        ...
        EcuCId = RxChannel->J1939TpTxFcNPduRxConfIdEcuC;
#if (J1939TP_RX_CANFD_SUPPORT == STD_ON)
    ...
    {
        uint8 Cm_pgn;
        MetaDataInfo[METADATA_SA_INDEX_0] = Sa;
        MetaDataInfo[METADATA_DA_INDEX_1] = Da;
    ...
    DBG_J1939TP_ABORT_CONN_EXIT(reason,TxChannel,RxChannel,pgn,Sa,Da,session_num,nsdu_abort_ptr);
...

{
    ...
    if (RxChannel->ChannelStatus->J1939TpTxFcNPduAvailable == TRUE)
    {
        ...
        if (MetaDataLen != 0U)
        {
            EcuC_Ret=EcuC_SetMetaDataCanId(RxChannel->J1939TpTxFcNPduRxConfIdEcuC,&PduInfoTemp,metaDataCanId);
        }
...
#if (J1939TP_TX_CANFD_SUPPORT == STD_ON)
           || (NsduState == J1939TP_BAM_TX_STATE_EOMS_TRANSMISSION) || (NsduState == J1939TP_BAM_TX_STATE_WAIT_CONFIRMATION_EOMS)
#endif
           )
        {
...
*  Parameters           : Input: J1939Tp_GeneralConfigType ConfigPtr
*  Return               : None
*  Critical/explanation : [No]
******************************************************************************/
/* !LINKSTO J1939Tp.SWS_J1939Tp_00087,1 */
FUNC(void, J1939TP_CODE) J1939Tp_Init(P2CONST(J1939Tp_GeneralConfigType,AUTOMATIC,J1939TP_APPL_CONST) ConfigPtr)
{
    /* Local Variable */
    uint8 Chan_Idx;
    uint8 Pg_Idx;
    ...
    J1939Tp_Cfg_Ptr = ConfigPtr;

    DBG_J1939TP_INIT_ENTRY(ConfigPtr);

    if (J1939Tp_Int_State != J1939TP_ON)
    {
        if (J1939Tp_Cfg_Ptr != NULL_PTR)
        {
        ...
        if (J1939Tp_Cfg_Ptr->J1939TpTxChannel !=NULL_PTR)
        {
            ...
            if (tTxNSduOk == TRUE)
            {
                ...
                if (nsdu_ctrl_ptr->state == J1939TP_TX_IDLE)
                {
                    ...
                    {
                        /* set the length of the data to be transmitted */
                        nsdu_ctrl_ptr->msglength = (uint32)(PduInfoPtr->SduLength);
                    }
                    /* set the pgn */
                    ...
                    if(MetaDataLen != 0U)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                u8DA_val = (J1939Tp_Cfg_Ptr->J1939TpTxChannel[Chan_Idx]).J1939TpTxDa ;
                            }
                        }
                    }
                    else
                    {
                        ...
                        u8SA_val = (J1939Tp_Cfg_Ptr->J1939TpTxChannel[Chan_Idx]).J1939TpTxSa;
                        if ((J1939Tp_Cfg_Ptr->J1939TpTxChannel[Chan_Idx]).J1939TpTxProtocolType == J1939TP_PROTOCOL_BAM)
                        {
                            u8DA_val = J1939TP_GLOBAL_ADDRESS;
                        }
...
#if (J1939TP_TX_CANFD_SUPPORT == STD_ON)
    if((TRUE == (J1939Tp_Cfg_Ptr->J1939TpTxChannel[Chan_Idx]).J1939TpTxCanFDSupport)&&
       ((nsdu_ctrl_ptr->state == J1939TP_BAM_TX_STATE_WAIT_CONFIRMATION_DT)||
        (nsdu_ctrl_ptr->state == J1939TP_BAM_TX_STATE_WAIT_CONFIRMATION_CM)||
...
static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_CM_Abort(uint8 loop_idx1, uint8 loop_idx2, uint8 loop_idx3, P2VAR(uint8, AUTOMATIC, J1939TP_APPL_DATA) SduDataPtr)
{
    ...
    if(TRUE == J1939Tp_Cfg_Ptr->J1939TpRxChannel[loop_idx1].J1939TpRxCanFDSupport)
    {
        /* !LINKSTO J1939Tp.SAE.FD.ConnectionAbortReasons,1 */
...
*  Critical/explanation : [No]
******************************************************************************/
static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_CM_BAM(
P2VAR(J1939Tp_SduInfoType, AUTOMATIC, J1939TP_APPL_DATA) SduInfo,
...
*  Critical/explanation : [No]
******************************************************************************/
static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_CM_RTS_IDLE(
P2VAR(J1939Tp_SduInfoType, AUTOMATIC, J1939TP_APPL_DATA) SduInfo)
{
    P2CONST(J1939Tp_RxChannelType,AUTOMATIC,J1939TP_APPL_CONST) RxChannelPtr =
    &(J1939Tp_Cfg_Ptr->J1939TpRxChannel[SduInfo->ch_idx]);
...
static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_CM_DT(PduIdType RxPduId,uint8 Chan_Idx, P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr)
{
    ...
    if ((J1939Tp_Cfg_Ptr->J1939TpRxChannel[Chan_Idx]).J1939TpRxCmNPduId == RxPduId)
    {
        ...
        EcuC_Pdu_ID=J1939Tp_Cfg_Ptr->J1939TpRxChannel[Chan_Idx].J1939TpRxCmNPduRxConfIdEcuC;
    }
    else
    {
        MetaDataLen = J1939Tp_Cfg_Ptr->J1939TpRxChannel[Chan_Idx].J1939TpRxDtNPduMetaDataLen;
        EcuC_Pdu_ID=J1939Tp_Cfg_Ptr->J1939TpRxChannel[Chan_Idx].J1939TpRxDtNPduRxConfIdEcuC;
    }
    ...
    {
        /* get the configured SA & DA */
        u8SA_val  = (J1939Tp_Cfg_Ptr->J1939TpRxChannel[Chan_Idx]).J1939TpRxSa;
    }
    ...
    else
    {
        ...
        {
            /* get the configured DA */
            u8DA_val = (J1939Tp_Cfg_Ptr->J1939TpRxChannel[Chan_Idx]).J1939TpRxDa ;
        }
    }
...
P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr,uint16 J1939TpEcucId)
{
    ...
    if(EcuC_Ret == E_OK)
    {
        ...
        {
            /* get the configured SA & DA */
            SduInfo->sa  = (J1939Tp_Cfg_Ptr->J1939TpRxChannel[SduInfo->ch_idx]).J1939TpRxSa;
        }
        ...
        else
        {
            ...
            {
                /* get the configured DA */
                SduInfo->da = (J1939Tp_Cfg_Ptr->J1939TpRxChannel[SduInfo->ch_idx]).J1939TpRxDa;
            }
        }
    }
    else
...
*  Critical/explanation : [No]
******************************************************************************/
static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_CM(uint8 Chan_Idx,
P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr)
{
    uint8 i;
        ...
        switch(SduInfo.MessageType)
        {
            ...
            case J1939TP_TPCM_RTS:
            {
                            ...
                            (RtsNoOfPackets == VerfiyNrPackets)))
                {
                    ...
                    {
                        EcuC_Ret = EcuC_GetMetaDataCanId(J1939Tp_Cfg_Ptr->J1939TpRxChannel[SduInfo.ch_idx].J1939TpRxCmNPduRxConfIdEcuC,NULL_PTR,&metaDataCanId);
...
*  Critical/explanation : [No]
******************************************************************************/
static FUNC(boolean, J1939TP_CODE) J1939Tp_CM_Validation(uint8 Chan_Idx,
P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr,
P2VAR(J1939Tp_SduInfoType, AUTOMATIC, J1939TP_APPL_DATA) SduInfo)
{
    boolean SduOK = FALSE;
...
#endif /* (J1939TP_RX_CANFD_SUPPORT == STD_ON) */
            {
                J1939Tp_RxGet_MetaDataInfo(SduInfo, PduInfoPtr,J1939Tp_Cfg_Ptr->J1939TpRxChannel[SduInfo->ch_idx].J1939TpRxCmNPduRxConfIdEcuC);
                ...
                if(J1939Tp_RxGet_pg_id(SduInfo) == E_OK)
                {
                    ...
                    else
                    {
                        ...
                        {
                            /* !LINKSTO EB_Ref.SWS_J1939Tp_00173.DiffSaDa,1 */
...
*  Critical/explanation : [No]
******************************************************************************/
static FUNC(boolean, J1939TP_CODE) J1939Tp_CM_Type_check(uint8 Chan_Idx,
P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr,
P2VAR(J1939Tp_SduInfoType, AUTOMATIC, J1939TP_APPL_DATA) SduInfo,
...
*  Critical/explanation : [No]
******************************************************************************/
static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_TxFcN_CM(uint8 Chan_Idx,
P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr)
{
        ...
        if(tTxPduOk2 == TRUE)
        {
            ...
            for (loop_idx9 = 0U; (loop_idx9<(((J1939Tp_Cfg_Ptr->J1939TpTxChannel[Chan_Idx]).J1939TpTxPg[Pg_Idx]).J1939TpNumberOfTxNSdu))&& (tTxPduOk3 != TRUE); loop_idx9++)
            {
...
#endif  /*J1939TP_TX_CANFD_SUPPORT == STD_ON*/
                {
                    ...
                    if(MetaDataLen > 0U)
                    {
                        EcuC_Ret= EcuC_GetMetaDataCanId((J1939Tp_Cfg_Ptr->J1939TpTxChannel[Chan_Idx]).J1939TpRxFcNPduTxConfIdEcuC,NULL_PTR,&metaDataCanId);
...
*  Critical/explanation : [No]
******************************************************************************/
static FUNC(void, J1939TP_CODE) J1939Tp_RxIndication_DT(uint8 Chan_Idx,
P2VAR(PduInfoType, AUTOMATIC, J1939TP_APPL_DATA) PduInfoPtr, uint8 u8SA_val,
uint8 u8DA_val)
{
    uint8 loop_idx5;
    uint8 loop_idx6;
    ...
    if (tRxPduOk4 == TRUE )
    {
        ...
        if(ReceivedLength >= ExpectedLength)
        {
            ...
            if(MetaDataLen != 0U)
            {
                ...
                EcuC_Ret=EcuC_GetMetaDataCanId(J1939Tp_Cfg_Ptr->J1939TpRxChannel[Chan_Idx].J1939TpRxDtNPduRxConfIdEcuC,NULL_PTR,&metaDataCanId);
...
static FUNC(Std_ReturnType, J1939TP_CODE) J1939Tp_Tx_Store_MetaData(P2VAR(uint8, AUTOMATIC, J1939TP_APPL_DATA) metaDataInfo, uint8 Chan_Idx, uint8 Pg_Idx, uint8 Nsdu_Idx)
{
    ...
    if(MetaDataLen == 1U)
    {
        ...
        ecuC_source_Ret=EcuC_GetMetaDataSourceAddr(nsdu_cfg_ptr->J1939TpTxNSduIdEcuC, NULL_PTR, &ecuCSAAddress);
    ...
    else if(MetaDataLen == 2U)
    {
        ...
        ecuC_source_Ret=EcuC_GetMetaDataSourceAddr(nsdu_cfg_ptr->J1939TpTxNSduIdEcuC, NULL_PTR, &ecuCSAAddress);
        metaDataInfo[0U] = (uint8)ecuCSAAddress;
        ecuC_target_Ret = EcuC_GetMetaDataTargetAddr(nsdu_cfg_ptr->J1939TpTxNSduIdEcuC, NULL_PTR, &ecuCDAAddress);
    ...
    else if(MetaDataLen == 4U)
    {
        ...
        ecuC_target_Ret = EcuC_GetMetaDataTargetAddr(nsdu_cfg_ptr->J1939TpTxNSduIdEcuC, NULL_PTR, &ecuCDAAddress);
        metaDataInfo[1U] = (uint8)ecuCDAAddress;
        ecuC_priority_Ret = EcuC_GetMetaDataPriority(nsdu_cfg_ptr->J1939TpTxNSduIdEcuC, NULL_PTR, &metaDataPriority);
...
*  Critical/explanation : [No]
******************************************************************************/
static FUNC(Std_ReturnType, J1939TP_CODE) J1939Tp_Transmit_Internal(uint8 Chan_Idx, uint8 Pg_Idx, uint8 Nsdu_Idx,
        ...
        else
        {
            ...
            for (loop_idx2=0U;(loop_idx2<((J1939Tp_Cfg_Ptr->J1939TpRxChannel[loop_idx1]).J1939TpNumberOfRxPg))&& (tRxPduOk2 != TRUE);loop_idx2++)
            {
                if (((J1939Tp_Cfg_Ptr->J1939TpRxChannel[loop_idx1]).J1939TpRxPg[loop_idx2]).J1939TpRxDirectNPduId == RxPduId)
                {
                    ...
                    if(SduInfo->MetaDataLen >0U)
                    {
                        EcuC_Ret = EcuC_GetMetaDataCanId(((J1939Tp_Cfg_Ptr->J1939TpRxChannel[loop_idx1]).J1939TpRxPg[loop_idx2]).J1939TpTxDirectNPduRxConfIdEcuC,NULL_PTR,&SduInfo->metaDatacanId);
                    }
                    if(EcuC_Ret != E_OK)
                    {
                        #if (J1939TP_DEV_ERROR_DETECT == STD_ON)
    ...
    else
    {
        ...
        else
        {
            if (SduInfo.PduId_CM_BAM== TRUE)
            {
                ...
                /* Handling of TP.CM/BAM or RTS or DT frame confirmation */
                /* BAM RTS and DT frame handling*/
                J1939Tp_TxConfirmation_CM(Chan_Idx, IsControl, result);
            }
            else
            {
...
static FUNC(void, J1939TP_CODE) J1939Tp_TxConfirmation_CM(uint8 Chan_Idx, boolean IsControl, Std_ReturnType result)
{
    ...
    DBG_J1939TP_TXCONFIRMATION_CM_ENTRY(Chan_Idx,IsControl,result);

    for (loop_idx4=0U;(loop_idx4<((J1939Tp_Cfg_Ptr->J1939TpTxChannel[Chan_Idx]).J1939TpNumberOfTxPg))&& (tTxNPduOk3 != TRUE);loop_idx4++)
    {
        for (loop_idx5=0U;(loop_idx5<(((J1939Tp_Cfg_Ptr->J1939TpTxChannel[Chan_Idx]).J1939TpTxPg[loop_idx4]).J1939TpNumberOfTxNSdu))&& (tTxNPduOk3 != TRUE);loop_idx5++)
        {
            J1939Tp_NSduCtrlType * TempPtr = (((J1939Tp_Cfg_Ptr->J1939TpTxChannel[Chan_Idx]).J1939TpTxPg[loop_idx4]).J1939TpTxNSdu[loop_idx5]).J1939TpTxNSduCtrl;
            switch (TempPtr->state)
            {
                case J1939TP_BAM_TX_STATE_WAIT_CONFIRMATION_DT:
                {
                ...
                case J1939TP_CMDT_TX_STATE_WAIT_CONFIRMATION_RTS:
                {
                    if(IsControl == TRUE)
                    {
...
#endif
                      {
                        ...
                        /* change the state */
                        J1939Tp_NSDU_ChangeState(TempPtr, J1939TP_CMDT_STATE_CTS);
    ...
    DBG_J1939TP_TXCONFIRMATION_CM_EXIT(Chan_Idx,IsControl,result);
}
...
static FUNC(void, J1939TP_CODE) J1939Tp_FCNPduMetaData_Handle(uint8 FcNPduMetaData,uint8 CmNPduMetaData,P2VAR(J1939Tp_NSduCtrlType,AUTOMATIC,J1939TP_APPL_DATA) nsdu_ctrl_ptr,uint8 MetaDataInfo[],uint8 loop_idx1)
{
    ...
    EcuC_Ret=EcuC_SetMetaDataCanId((J1939Tp_Cfg_Ptr->J1939TpRxChannel[loop_idx1]).J1939TpTxFcNPduRxConfIdEcuC, &PduInfoTemp, metaDataCanId);
