...
)
{
    ...
    if (pIfChannel->cFSM == LINIF_CHANNEL_OPERATIONAL)
    {
        ...
        boolean RescheduleThisTB = FALSE;  /* Force rescheduleing this timebase */
        ...
        if((1U == LINIF_CFG_GET_CHANNEL_MASTER(LINIF_CFG_GET_CHANNEL[currentChannel].LinIfChIndex)->SchChgNxTB) && (0U == pIfChannelMaster->StatusDelay))
        {
          ...
          RescheduleThisTB = LinIf_ReSchedule(currentChannel);
        ...
        if(NextFrameToSendDue == 1U)
        {
            ...
            if(ret != E_OK)
            {
                ...
                if(TRUE != RescheduleThisTB) /* LinIf_SetupCollisionRes aleardy called before */
                {
                    LinIf_SetupCollisionRes(currentChannel);
                }
               ...
               )
            {
                ...
                if ( pfunction != NULL_PTR )
                {
                    ...
                    if (LIN_RX_OK == status)
                    {
                        ...
                        retVal = BUSA_LINIF_RXINDICATION(linId, pPduInfo, frameType, currentChannel);
                        if(retVal == TRUE)
                        {
...
)
{
    ...
    if(TRUE == ReScheduleIsCalled) /* LinIf_ReSchedule is aleardy called and returned TRUE */
    {
        Reschedule = TRUE;
    }
    else
