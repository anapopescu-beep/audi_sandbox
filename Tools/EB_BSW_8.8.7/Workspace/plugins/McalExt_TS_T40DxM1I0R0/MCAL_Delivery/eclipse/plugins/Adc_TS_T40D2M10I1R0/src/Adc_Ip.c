...
#define ADC_IP_SW_PATCH_VERSION_C               1
...
void Adc_Ip_IRQHandler(const uint32 Instance)
{
    ...
    */
    ...
    if ((Adc_Ip_axState[Instance].Init == TRUE) &&
        (Adc_Ip_axState[Instance].ConversionCompleteNotification != NULL_PTR))
    {
        for (ControlChanIdx = 0u; ControlChanIdx < ADC_MAX_CHAN_COUNT; ControlChanIdx++)
        {
            ...
            else if ((ADC_IP_INPUTCHAN_DISABLED == Adc_Ip_axState[Instance].ChannelConfig[ControlChanIdx]) && (TRUE == ChanCocoFlag))
            {
