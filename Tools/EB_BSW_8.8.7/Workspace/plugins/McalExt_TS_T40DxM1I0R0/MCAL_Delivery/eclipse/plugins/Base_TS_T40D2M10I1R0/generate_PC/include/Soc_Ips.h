...
[!SELECT "as:modconf('Resource')[1]"!][!//
[!//
[!/* This errata is not exited on s32k144w device. For s32k142w service, there is no information about errata from hardware team, so it is considered to not apply.*/!][!//
[!IF "not(contains(node:value(node:fallback(ResourceGeneral/ResourceSubderivative,'')),'s32k142w') or contains(node:value(node:fallback(ResourceGeneral/ResourceSubderivative,'')),'s32k144w'))"!][!//
/**
