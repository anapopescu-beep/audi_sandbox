...
#define CRC_IP_LOOKUP_TABLES_AR_RELEASE_MINOR_VERSION_C       4
#define CRC_IP_LOOKUP_TABLES_AR_RELEASE_REVISION_VERSION_C    0
#define CRC_IP_LOOKUP_TABLES_SW_MAJOR_VERSION_C               1
#define CRC_IP_LOOKUP_TABLES_SW_MINOR_VERSION_C               0
#define CRC_IP_LOOKUP_TABLES_SW_PATCH_VERSION_C               1
...
*/
static const uint64 Crc_u64CrcTable[256U] = {
...
==================================================================================================*/
static uint8 Crc_TableCalculateCRC8(const Crc_Ip_ProtocolType Protocol,
                                    const Crc_Ip_CrcProtocolInfoType *pxCrcProtocolInfo,
                                    const uint8 *DataPtr,
                                    const uint32 Length,
                                    const uint8 StartValue8
                                   );

static uint16 Crc_TableCalculateCRC16(const Crc_Ip_ProtocolType Protocol,
                                      const Crc_Ip_CrcProtocolInfoType *pxCrcProtocolInfo,
                                      const uint8 *DataPtr,
                                      const uint32 Length,
...
static uint32 Crc_TableCalculateCRC32(const Crc_Ip_ProtocolType Protocol,
                                      const Crc_Ip_CrcProtocolInfoType *pxCrcProtocolInfo,
                                      const uint8 *DataPtr,
                                      const uint32 Length,
...
static uint64 Crc_TableCalculateCRC64(const Crc_Ip_ProtocolType Protocol,
                                      const Crc_Ip_CrcProtocolInfoType *pxCrcProtocolInfo,
                                      const uint8 *DataPtr,
                                      const uint32 Length,
                                   ...
                                   )
{
    ...
    for (LengthIdx = 0UL; LengthIdx < Length; LengthIdx++)
    {
        ...
        if (CRC_PROTOCOL_8BIT_SAE_J1850 == Protocol)
        {
            CrcValue = s_TableCrc8[CrcValue ^ CrcDataU8];
        }
        else if (CRC_PROTOCOL_8BIT_H2F == Protocol)
        {
                                     ...
                                     )
{
    ...
    for (LengthIdx = (uint32)0UL; LengthIdx < Length; LengthIdx++)
    {
        ...
        /* XOR-in remainder from lookup table using the calculated index */
        /* Check CRC Table types */
        if (CRC_PROTOCOL_16BIT_CCITT_FALSE == Protocol)
        {
            CrcValue ^= s_TableCrc16[TableIndex];
        }
        else if (CRC_PROTOCOL_16BIT_ARC == Protocol)
        {
                                     ...
                                     )
{
    ...
    for (LengthIdx = (uint32)0UL; LengthIdx < Length; LengthIdx++)
    {
        ...
        /* XOR-in remainder from lookup table using the calculated index */
        /* Check CRC Table types */
        if (CRC_PROTOCOL_32BIT_ETHERNET == Protocol)
        {
            CrcValue ^= s_TableCrc32[TableIndex];
        }
        else if (CRC_PROTOCOL_32BIT_E2E_P4 == Protocol)
        {
                                     ...
                                     )
{
    ...
    for (LengthIdx = (uint32)0UL; LengthIdx < Length; LengthIdx++)
    {
        ...
        /* XOR-in remainder from lookup table using the calculated index */
        /* Check CRC Table types */
        if (CRC_PROTOCOL_64BIT_ECMA == Protocol)
        {
            CrcValue ^= Crc_u64CrcTable[TableIndex];
        }
        else
        {
            /* CRC not supported */
                                   ...
                                   )
{
    ...
    {
        case CRC_WIDTH_8BIT:
            LocResult = Crc_TableCalculateCRC8(pxLogicChannelState->Protocol, pxLogicChannelState->CrcProtocolInfo, DataPtr, Length, (uint8)InitialSeedStartValue);
            break;
        case CRC_WIDTH_16BIT:
            LocResult = Crc_TableCalculateCRC16(pxLogicChannelState->Protocol, pxLogicChannelState->CrcProtocolInfo, DataPtr, Length, (uint16)InitialSeedStartValue);
            break;
        case CRC_WIDTH_32BIT:
            LocResult = Crc_TableCalculateCRC32(pxLogicChannelState->Protocol, pxLogicChannelState->CrcProtocolInfo, DataPtr, Length, (uint32)InitialSeedStartValue);
            break;
        case CRC_WIDTH_64BIT:
            LocResult = Crc_TableCalculateCRC64(pxLogicChannelState->Protocol, pxLogicChannelState->CrcProtocolInfo, DataPtr, Length, InitialSeedStartValue);
            break;
        default:
            /* Do nothing */
            break;
