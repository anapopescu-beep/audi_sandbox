...
#define CRC_IP_SOFTWARE_AR_RELEASE_MINOR_VERSION_C       4
#define CRC_IP_SOFTWARE_AR_RELEASE_REVISION_VERSION_C    0
#define CRC_IP_SOFTWARE_SW_MAJOR_VERSION_C               1
#define CRC_IP_SOFTWARE_SW_MINOR_VERSION_C               0
#define CRC_IP_SOFTWARE_SW_PATCH_VERSION_C               1
...
#define CRC8_IP_SW_WIDTH_VALUE     (8U)
#define CRC8_IP_SW_SHIFT_VALUE     (1U)

#define CRC16_IP_SW_MASK_VALUE     (0x8000U)
#define CRC16_IP_SW_WIDTH_VALUE    (8U)
#define CRC16_IP_SW_SHIFT_VALUE    (1U)

#define CRC32_IP_SW_MASK_VALUE     (0x80000000U)
#define CRC32_IP_SW_WIDTH_VALUE    (8U)
#define CRC32_IP_SW_SHIFT_VALUE    (1U)

#define CRC64_IP_SW_MASK_VALUE     (0x8000000000000000U)
#define CRC64_IP_SW_WIDTH_VALUE    (8U)
#define CRC64_IP_SW_SHIFT_VALUE    (1U)
                                   ...
                                   )
{
    ...
    for (LengthIdx = 0UL; LengthIdx < Length; LengthIdx++)
    {
        ...
        for (BitsIdx = 0U; BitsIdx < CRC8_IP_SW_WIDTH_VALUE; BitsIdx++)
        {
            if (0U != (u8CrcValue & CRC8_IP_SW_MASK_VALUE))
            {
                u8CrcValue = (uint8)(((uint8)(u8CrcValue << CRC8_IP_SW_SHIFT_VALUE)) ^ pxCrcProtocolInfo->PolynomValue);
            }
            else
            {
                u8CrcValue <<= CRC8_IP_SW_SHIFT_VALUE;
            }
        }
    }
                                     ...
                                     )
{
    ...
    for (LengthIdx = 0UL; LengthIdx < Length; LengthIdx++)
    {
        ...
        for (BitsIdx = 0U; BitsIdx < CRC16_IP_SW_WIDTH_VALUE; BitsIdx++)
        {
            if (0U != (u16CrcValue & CRC16_IP_SW_MASK_VALUE))
            {
                u16CrcValue = (uint16)(((uint16)(u16CrcValue << CRC16_IP_SW_SHIFT_VALUE)) ^ pxCrcProtocolInfo->PolynomValue);
            }
            else
            {
                u16CrcValue <<= CRC16_IP_SW_SHIFT_VALUE;
            }
        }
    }
    /* Set red transpose for CRC data */
                                     ...
                                     )
{
    ...
    for (LengthIdx = 0UL; LengthIdx < Length; LengthIdx++)
    {
        ...
        for (BitsIdx = 0U; BitsIdx < CRC32_IP_SW_WIDTH_VALUE; BitsIdx++)
        {
            if (0U != (u32CrcValue & CRC32_IP_SW_MASK_VALUE))
            {
                u32CrcValue = (uint32)(((uint32)(u32CrcValue << CRC32_IP_SW_SHIFT_VALUE)) ^ pxCrcProtocolInfo->PolynomValue);
            }
            else
            {
                u32CrcValue <<= CRC32_IP_SW_SHIFT_VALUE;
            }
        }
    }
    /* Set red transpose for CRC data */
                                     ...
                                     )
{
    ...
    for (LengthIdx = 0UL; LengthIdx < Length; LengthIdx++)
    {
        ...
        for (BitsIdx = 0U; BitsIdx < CRC64_IP_SW_WIDTH_VALUE; BitsIdx++)
        {
            if (0U != (CrcValue & CRC64_IP_SW_MASK_VALUE))
            {
                CrcValue = (CrcValue << CRC64_IP_SW_SHIFT_VALUE) ^ pxCrcProtocolInfo->PolynomValue;
            }
            else
            {
                CrcValue <<= CRC64_IP_SW_SHIFT_VALUE;
            }
        }
    }
    /* Set red transpose for CRC data */
