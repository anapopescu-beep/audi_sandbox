...
typedef enum
{
    EEP_FTFM_KEY_SIZE_0_BYTES   = 0x0,  /**< @brief control code for key size 0 bytes, used for the partitioning command */
    EEP_FTFM_KEY_SIZE_128_BYTES = 0x1,  /**< @brief control code for key size 128 bytes, used for the partitioning command */
    EEP_FTFM_KEY_SIZE_256_BYTES = 0x2,  /**< @brief control code for key size 256 bytes, used for the partitioning command */
    EEP_FTFM_KEY_SIZE_512_BYTES = 0x3   /**< @brief control code for key size 512 bytes, used for the partitioning command */
} TestEep_CsecKeySize;
...
typedef enum
{
    EEP_FTFM_EERAM_SIZE_0K  = 0xF,   /**< @brief control code for flexram partitioned as sram, used for the partitioning command */
    EEP_FTFM_EERAM_SIZE_4K  = 0x2,   /**< @brief control code for flexram partitioned as eeram, used for the partitioning command */
    EEP_FTFM_EERAM_SIZE_2K  = 0x3    /**< @brief control code for flexram partitioned as eeram, used for the partitioning command */
...
typedef enum
{
    EEP_FTFM_EEEPROM_SIZE_0K_V1   =  0x0,   /**< @brief control code to partition EEPROM backup size as 0K, used for the partitioning command */
    EEP_FTFM_EEEPROM_SIZE_0K_V2   =  0xC,   /**< @brief control code to partition EEPROM backup size as 0K, used for the partitioning command */
    EEP_FTFM_EEEPROM_SIZE_0K_V3   =  0x0F,  /**< @brief control code to partition EEPROM backup size as 0K, used for the partitioning command */
    EEP_FTFM_EEEPROM_SIZE_32K_V1  = 0x3,    /**< @brief control code to partition EEPROM backup size as 32K, used for the partitioning command */
    EEP_FTFM_EEEPROM_SIZE_32K_V2  = 0xB,    /**< @brief control code to partition EEPROM backup size as 32K, used for the partitioning command */
    EEP_FTFM_EEEPROM_SIZE_48K_V1  = 0xA,    /**< @brief control code to partition EEPROM backup size as 48K, used for the partitioning command */
    EEP_FTFM_EEEPROM_SIZE_64K_V1  = 0x8,    /**< @brief control code to partition EEPROM backup size as 64K, used for the partitioning command */
    EEP_FTFM_EEEPROM_SIZE_64K_V2  = 0x4,    /**< @brief control code to partition EEPROM backup size as 64K, used for the partitioning command */
    EEP_FTFM_EEEPROM_SIZE_24K_V2  = 0x9     /**< @brief control code to partition EEPROM backup size as 24K, used for the partitioning command */
...
typedef enum
{
    EEP_FTFM_VERIFY_ONLY_DISABLED  = 0x0,    /**< @brief control code for sfe verify only disabled, used for the partitioning command */
    EEP_FTFM_VERIFY_ONLY_ENABLED   = 0x1     /**< @brief control code for sfe verify only enabled, used for the partitioning command */
} TestEep_SfeType;
...
typedef enum
{
    EEP_FTFM_LOAD_AT_RESET_ENABLED  = 0x0,  /**< @brief control code for loading flexram at reset, used for the partitioning command */
    EEP_FTFM_LOAD_AT_RESET_DISABLED = 0x1   /**< @brief control code for not loading flexram at reset, used for the partitioning command */
...
#define T_EEEPROM_SIZE                     EEP_FTFM_EEEPROM_SIZE_32K_V2
#define TEST_EEP_EEPROM_FSTAT_ADDR32                               ((uint32)(IP_FTFM_BASE + (uint32)0x00UL)) /**< @brief Eeprom Status Register (FTFE_FSTAT) */
#define TEST_EEP_EEPROM_FCCOB3_ADDR32                              ((uint32)(IP_FTFM_BASE + (uint32)0x04UL)) /**< @brief Eeprom Common Command Object Registers(FTFE_FCCOB3) */
#define TEST_EEP_EEPROM_FCCOB2_ADDR32                              ((uint32)(IP_FTFM_BASE + (uint32)0x05UL)) /**< @brief Eeprom Common Command Object Registers(FTFE_FCCOB2) */
#define TEST_EEP_EEPROM_FCCOB1_ADDR32                              ((uint32)(IP_FTFM_BASE + (uint32)0x06UL)) /**< @brief Eeprom Common Command Object Registers(FTFE_FCCOB1) */
#define TEST_EEP_EEPROM_FCCOB0_ADDR32                              ((uint32)(IP_FTFM_BASE + (uint32)0x07UL)) /**< @brief Eeprom Common Command Object Registers(FTFE_FCCOB0) */
#define TEST_EEP_EEPROM_FCCOB7_ADDR32                              ((uint32)(IP_FTFM_BASE + (uint32)0x08UL)) /**< @brief Eeprom Common Command Object Registers(FTFE_FCCOB7) */
#define TEST_EEP_EEPROM_FCCOB6_ADDR32                              ((uint32)(IP_FTFM_BASE + (uint32)0x09UL)) /**< @brief Eeprom Common Command Object Registers(FTFE_FCCOB6) */
#define TEST_EEP_EEPROM_FCCOB5_ADDR32                              ((uint32)(IP_FTFM_BASE + (uint32)0x0AUL)) /**< @brief Eeprom Common Command Object Registers(FTFE_FCCOB5) */
#define TEST_EEP_EEPROM_FCCOB4_ADDR32                              ((uint32)(IP_FTFM_BASE + (uint32)0x0BUL)) /**< @brief Eeprom Common Command Object Registers(FTFE_FCCOB4) */
...
void Eep_DepartParitition(TestEep_Eeprom_FlexNvmPartitionType T_EEP_SIZE)
{
    ...
    if (u32FlexNvmPartSize == 0xF) /* We just partition again if curent size different with expected */
    {
        ...
        TestEep_FlexNvmProgramPartCmd(EEP_FTFM_KEY_SIZE_0_BYTES, EEP_FTFM_VERIFY_ONLY_DISABLED, \
                                               EEP_FTFM_LOAD_AT_RESET_ENABLED, EEP_FTFM_EERAM_SIZE_4K, T_EEP_SIZE);
    }
...
int main(void)
{
    ...
    retReturnType = Eep_Erase(EEP_ERASE_START_ADD, EEP_WRTESTPATT_SIZE);
    ...
    retReturnType = Eep_Write(EEP_ERASE_START_ADD, dataWrTestPatt, EEP_WRTESTPATT_SIZE);
    if(E_OK != retReturnType)
    {
        Exit_Example(FALSE);
        return 0;
    ...
    retReturnType = Eep_Compare(EEP_ERASE_START_ADD, dataWrTestPatt, EEP_WRTESTPATT_SIZE);
    if(E_OK != retReturnType)
    {
        Exit_Example(FALSE);
        return 0;
    ...
    retReturnType = Eep_Read(EEP_ERASE_START_ADD, BufferRead, EEP_RDTESTPATT_SIZE);
    if(E_OK != retReturnType)
    {
        Exit_Example(FALSE);
        return 0;
