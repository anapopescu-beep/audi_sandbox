...
*                                       LOCAL MACROS
==================================================================================================*/
/* Test the last sector of P_FLASH: FLS_CODE_ARRAY_0_BLOCK_2_S383 */
#define FTFC_SECTOR_ADDR             (FTFC_P_FLASH_BASE_ADDR + 0x17F000UL)

/* Address offset = 16 bytes */
#define FTFC_WRITE_ADDR              (FTFC_SECTOR_ADDR + 16U)

/* Test data size (must be double word aligned) */
#define FTFC_BUF_SIZE                (64U)
...
int main(void)
{
    ...
    {
        Ftfc_Fls_Ip_Write(FTFC_WRITE_ADDR + Index, &TxBuffer[Index], FTFC_WRITE_DOUBLE_WORD);
        do
        {
    ...
    FtfcStatus = Ftfc_Fls_Ip_Compare(FTFC_WRITE_ADDR, TxBuffer, FTFC_BUF_SIZE);
    Fls_ExampleAssert(STATUS_FTFC_FLS_IP_SUCCESS == FtfcStatus);

    /* Read data */
    FtfcStatus = Ftfc_Fls_Ip_Read(FTFC_WRITE_ADDR, RxBuffer, FTFC_BUF_SIZE);
    Fls_ExampleAssert(STATUS_FTFC_FLS_IP_SUCCESS == FtfcStatus);
