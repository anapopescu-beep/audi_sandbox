...
#define SAI_IP_AR_RELEASE_MINOR_VERSION_C       4
#define SAI_IP_AR_RELEASE_REVISION_VERSION_C    0
#define SAI_IP_SW_MAJOR_VERSION_C               1
#define SAI_IP_SW_MINOR_VERSION_C               0
#define SAI_IP_SW_PATCH_VERSION_C               1
...
static void Sai_Ip_TxIRQHandler(const uint8 u8Instance)
{
    ...
    if (NULL_PTR != pState)
    {
        ...
        if (Sai_Ip_TxGetFifoErrorFlag(pBase))
        {
            ...
            {
                if ((NULL_PTR != pState->pCallback) && (Sai_Ip_TxGetBitEnable(pBase, SAI_TCSR_FEIE_MASK)))
                {
        ...
        if (Sai_Ip_TxGetWordStartFlag(pBase))
        {
            ...
            {
                if ((NULL_PTR != pState->pCallback) && (Sai_Ip_TxGetBitEnable(pBase, SAI_TCSR_WSIE_MASK)))
                {
        ...
        if (Sai_Ip_TxGetSyncErrorFlag(pBase))
        {
            ...
            {
                if ((NULL_PTR != pState->pCallback) && (Sai_Ip_TxGetBitEnable(pBase, SAI_TCSR_SEIE_MASK)))
                {
    ...
    {
        /* Clear all tx flags because interrupt is spurious*/
...
static void Sai_Ip_RxIrqCheckFifo(const uint8 u8Instance)
{
    ...
    if (Sai_Ip_RxGetFifoErrorFlag(pBase))
    {
        ...
        {
            if ((NULL_PTR != pState->pCallback) && (Sai_Ip_RxGetBitEnable(pBase, SAI_RCSR_FEIE_MASK)))
            {
                pState->pCallback(SAI_IP_RUN_ERROR);
            }
        }
...
static void Sai_Ip_RxIRQHandler(const uint8 u8Instance)
{
    ...
    if (NULL_PTR != pState)
    {
        ...
        if (Sai_Ip_RxGetWordStartFlag(pBase))
        {
            ...
            {
                if ((NULL_PTR != pState->pCallback) && (Sai_Ip_RxGetBitEnable(pBase, SAI_RCSR_WSIE_MASK)))
                {
                    pState->pCallback(SAI_IP_FRAME_START);
                }
            }
        ...
        if (Sai_Ip_RxGetSyncErrorFlag(pBase))
        {
            ...
            {
                if ((NULL_PTR != pState->pCallback) && (Sai_Ip_RxGetBitEnable(pBase, SAI_RCSR_SEIE_MASK)))
                {
                    pState->pCallback(SAI_IP_SYNC_ERROR);
                }
            }
    ...
    {
        /* Clear all rx flags because interrupt is spurious*/
