...
/* Check if current file and Lin header file are of the same Software version */
    ...
    #if ((LIN_AR_RELEASE_MAJOR_VERSION_C    != LINIF_AR_RELEASE_MAJOR_VERSION) || \
         (LIN_AR_RELEASE_MINOR_VERSION_C    != LINIF_AR_RELEASE_MINOR_VERSION))
        #error "AutoSar Version Numbers of Lin.c and LinIf.h are different"
    #endif
...
extern const Lin_ConfigType * const Lin_PBCfgVariantPredefined[LIN_MAX_PARTITIONS];
#endif
...
*/
const Lin_ConfigType * Lin_apConfigPtr[LIN_MAX_PARTITIONS];

/**
* @brief          Global configuration array.
* @details        Array containing channel configurations.
*/
const Lin_ChannelConfigType *Lin_apChannelConfigPtr[LIN_HW_MAX_MODULES];
...
*/
uint8 Lin_au8LinDrvStatus[LIN_MAX_PARTITIONS] = LIN_UNINIT_ARRAY;
...
*/
uint8 Lin_au8LinChStatus[LIN_HW_MAX_MODULES];
...
==================================================================================================*/
static void Lin_InitImplementation(void);

static Std_ReturnType Lin_CommonCheckingChannel(const uint8 Channel, const uint8 FunctionalId);

#if (LIN_MASTER_NODE_USED == STD_ON)
static Std_ReturnType Lin_ChkParamFuncSendFrame(const uint8 Channel, const Lin_PduType * const PduInfoPtr);
...
static void  Lin_InitImplementation(void)
{
    ...
    {
        if ((TRUE == Lin_apConfigPtr[CoreId]->Lin_ChannelPtr[Lin_ChLoop]->AllocatedPartition) &&
            (CoreId == Lin_apConfigPtr[CoreId]->Lin_ChannelPtr[Lin_ChLoop]->ChannelCoreId)
           )
        {
            Lin_apChannelConfigPtr[Lin_ChLoop] = (Lin_apConfigPtr[CoreId]->Lin_ChannelPtr)[Lin_ChLoop];
            Lin_Ipw_InitChannel(Lin_ChLoop, Lin_apChannelConfigPtr[Lin_ChLoop]->ChannelConfigPtr);
            /* Set LIN channel state machine to LIN_CH_SLEEP_STATE */
            Lin_au8LinChStatus[Lin_ChLoop] = LIN_CH_SLEEP_STATE;
        }
    }
}
...
static Std_ReturnType Lin_CommonCheckingChannel(const uint8 Channel, const uint8 FunctionalId)
{
    ...
    else
    {
        ...
        else if ((FALSE == Lin_apChannelConfigPtr[Channel]->AllocatedPartition) ||
                 (CoreId != Lin_apChannelConfigPtr[Channel]->ChannelCoreId)
                )
        {
...
static Std_ReturnType Lin_ChkParamFuncSendFrame(const uint8 Channel, const Lin_PduType * const PduInfoPtr)
{
    ...
    TempReturn = Lin_CommonCheckingChannel(Channel, LIN_SENDFRAME_ID);
...
void Lin_Init(const Lin_ConfigType * Config)
{
    ...
    if (LIN_UNINIT != Lin_au8LinDrvStatus[CoreId])
    {
    ...
    else
    {
        ...
        else if ((NULL_PTR == Lin_PBCfgVariantPredefined[CoreId]) || (Lin_PBCfgVariantPredefined[CoreId]->PartitionCoreId != CoreId))
        {
            #if (LIN_DEV_ERROR_DETECT == STD_ON)
            /* Invalid requested resource */
            (void)Det_ReportError((uint16)LIN_MODULE_ID, \
        ...
        {
            Lin_apConfigPtr[CoreId] = Lin_PBCfgVariantPredefined[CoreId];
    #endif /* LIN_PRECOMPILE_SUPPORT */

            Lin_InitImplementation();

            /* Update LIN driver status to LIN_INIT */
            Lin_au8LinDrvStatus[CoreId] = LIN_INIT;
        }
...
Std_ReturnType Lin_CheckWakeup(uint8 Channel)
{
    ...
    TempReturn = Lin_CommonCheckingChannel(Channel, LIN_CHECKWAKEUP_ID);
    ...
    else
    {
        ...
        /* Check if the addressed LIN channel is in LIN_CH_SLEEP_STATE or LIN_CH_SLEEP_PENDING state*/
        if ((LIN_CH_SLEEP_STATE == Lin_au8LinChStatus[Channel]) || (LIN_CH_SLEEP_PENDING == Lin_au8LinChStatus[Channel]))
        {
            /* Lin channel is in LIN_CH_SLEEP_STATE state */
            /* Check if Lin channel has detected a wake-up */
            if ((uint8)E_OK == Lin_Ipw_CheckWakeup(Channel))
            {
#if (LIN_WAKEUP_DETECTION == STD_ON)
                if (TRUE == Lin_apChannelConfigPtr[Channel]->ChannelConfigPtr->LinChannelWakeupSupport)
                {
                    /* Indicates a valid timer wakeup event to ECU State Manager */
                    EcuM_SetWakeupEvent(Lin_apChannelConfigPtr[Channel]->ChannelConfigPtr->LinChannelEcuMWakeupSource);
                    /* report the wakeup source after the successful wakeup to LinInterface */
                    LinIf_WakeupConfirmation(Lin_apChannelConfigPtr[Channel]->ChannelConfigPtr->LinChannelEcuMWakeupSource);
                }
#endif
            }
        }
...
Lin_StatusType Lin_GetStatus(uint8 Channel, uint8 ** Lin_SduPtr)
{
    ...
    /* General channel checking */
    CheckChannel = Lin_CommonCheckingChannel(Channel, LIN_GETSTATUS_ID);
    ...
    else
    {
        ...
        {
            Lin_ChannelState = Lin_au8LinChStatus[Channel];
...
Std_ReturnType Lin_GoToSleep(uint8 Channel)
{
    ...
    /* General channel checking */
    TempReturn = Lin_CommonCheckingChannel(Channel, LIN_GOTOSLEEP_ID);
    ...
    else
    {
        ...
        else
        {
            ...
            /* Lin_Ipw_GoToSleep can return E_NOT_OK or LIN_TIMEOUT_ERROR, So check TempReturn != E_OK */
            if ((uint8)E_OK != TempReturn)
            {
                if ((uint8)LIN_TIMEOUT_ERROR == TempReturn)
                {
...
Std_ReturnType Lin_GoToSleepInternal(uint8 Channel)
{
    ...
    /* General channel checking */
    TempReturn = Lin_CommonCheckingChannel(Channel, LIN_GOTOSLEEPINTERNAL_ID);
    ...
    else
    {
        ...
        else
        {
            ...
            /* Lin_Ipw_GoToSleepInternal can return E_NOT_OK or LIN_TIMEOUT_ERROR, So check TempReturn != E_OK */
            if ((uint8)E_OK != TempReturn)
            {
                if ((uint8)LIN_TIMEOUT_ERROR == TempReturn)
                {
...
Std_ReturnType Lin_SendFrame(uint8 Channel, const Lin_PduType * PduInfoPtr)
{
    ...
    else
    {
        ...
        /* Lin_Ipw_SendFrame can return E_NOT_OK or LIN_TIMEOUT_ERROR, So check TempReturn != E_OK */
        if ((uint8)E_OK != TempReturn)
        {
            if((uint8)LIN_TIMEOUT_ERROR == TempReturn)
            {
...
Std_ReturnType Lin_Wakeup(uint8 Channel)
{
    ...
    /* General channel checking */
    TempReturn = Lin_CommonCheckingChannel(Channel, LIN_WAKEUP_ID);
...
Std_ReturnType Lin_WakeupInternal(uint8 Channel)
{
    ...
    /* General channel checking */
    TempReturn = Lin_CommonCheckingChannel(Channel, LIN_WAKEUPINTERNAL_ID);
