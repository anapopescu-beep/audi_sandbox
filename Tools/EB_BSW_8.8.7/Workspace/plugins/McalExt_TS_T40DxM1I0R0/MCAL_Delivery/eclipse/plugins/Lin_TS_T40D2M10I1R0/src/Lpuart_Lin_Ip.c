...
#define LPUART_LIN_IP_SW_MAJOR_VERSION_C               1
#define LPUART_LIN_IP_SW_MINOR_VERSION_C               0
#define LPUART_LIN_IP_SW_PATCH_VERSION_C               1
/*==================================================================================================
*                                       FILE VERSION CHECKS
==================================================================================================*/
...
/* Minimum Wakeup field time */
#define LPUART_LIN_IP_AUTOBAUD_WAKEUP_TIME_MIN_IN_NS            (150000U)
#define LPUART_LIN_IP_AUTOBAUD_WAKEUP_TIME_MIN_IN_US            (150U)
...
static uint8 Lpuart_Lin_Ip_au8WakeupDetectInvert[LPUART_INSTANCE_COUNT];
...
static void Lpuart_Lin_Ip_CheckWakeupSignal(const uint8 Instance, const uint8 TmpByte);
...
static void Lpuart_Lin_Ip_FrameIrqHandler(const uint8 Instance)
{
    ...
    switch (LinCurrentState->CurrentNodeState)
    {
        ...
        case LPUART_LIN_IP_NODE_STATE_SLEEP_MODE:
            ...
            Lpuart_Lin_Ip_CheckWakeupSignal(Instance, TmpByte);
            break;
...
static void Lpuart_Lin_Ip_AutobaudTimerValEval(const uint8 Instance, const uint32 TwoBitTimeLength)
{
    ...
    uint8 WakeupDetectByteInvert = 0UL;
    uint16 BitLengthInUs = 0UL;
    uint8 WakeupByteNumOfBits0 = 0UL;
    uint8 WakeupDetectByteNumOfBits0 = 0UL;
       ...
       )
    {
        ...
        if ((MasterBaudrate >= LPUART_LIN_IP_AUTOBAUD_BAUDRATE_MIN_RANGE) && (MasterBaudrate <= LPUART_LIN_IP_AUTOBAUD_BAUDRATE_MAX_RANGE))
        {
            ...
            /* Calculate bit length in microsecond to generate wakeup pulse */
            BitLengthInUs = LPUART_LIN_IP_CONVERT_SEC_TO_MICROSEC / MasterBaudrate;
            /* Maximum duration 5000us of wakeup pulse */
            WakeupByteNumOfBits0 = 5000U / BitLengthInUs;
            if (WakeupByteNumOfBits0 > 9U)
            {
                /* For maximum baudrate 20kbps, 8bits 0 + 1 Start bit = 450us. */
                WakeupByteNumOfBits0 = 8U;
            }
            else
            {
                /* For minimum baudrate 1kbps, number of bits 0 is lowest: 5 bits0 - 1 Start bit. */
                WakeupByteNumOfBits0 -= 1U;
            }
            /* Calculate wakeup value */
            WakeupByte = (uint8)((255U << WakeupByteNumOfBits0) & 255U);

            if (BitLengthInUs > LPUART_LIN_IP_AUTOBAUD_WAKEUP_TIME_MIN_IN_US)
            {
                /* Already have 1 bit0 of Start bit, wakeup byte received should have at least 0 bit0 (Ex:0xFF). */
                WakeupDetectByteNumOfBits0 = 0U;
            }
            else
            {
                /* Already have 1 bit0 of Start bit, so number of bit0 - 1 Start bit */
                WakeupDetectByteNumOfBits0 = (LPUART_LIN_IP_AUTOBAUD_WAKEUP_TIME_MIN_IN_US / BitLengthInUs) - 1U;
            }

            WakeupDetectByteInvert = (~(255U << WakeupDetectByteNumOfBits0)) & 255U;
            ...
            if ((0U != MasterBaudrateDivisor) && (MasterBaudrateDivisor <= 8191U))
            {
                ...
                Lpuart_Lin_Ip_au8WakeupDetectInvert[Instance] = WakeupDetectByteInvert;
...
Lpuart_Lin_Ip_StatusType Lpuart_Lin_Ip_Init(const uint8 Instance, const Lpuart_Lin_Ip_UserConfigType * UserConfig)
{
    ...
    */
    ...
    Lpuart_Lin_Ip_au8WakeupDetectInvert[Instance] = UserConfig->BaseWakeupByteDetectInverted;
#if (LPUART_LIN_IP_AUTO_BAUD == STD_ON)
...
Lpuart_Lin_Ip_StatusType Lpuart_Lin_Ip_GoToSleepMode(const uint8 Instance)
{
    ...
    else
    {
        if (LPUART_LIN_IP_NODE_STATE_SLEEP_MODE != LinCurrentState->CurrentNodeState)
        {
            ...
            /* Enable RX complete interrupt to check wakeup byte reception */
            Lpuart_Lin_Ip_HwSetIntMode(Base, LPUART_LIN_IP_INT_RX_DATA_REG_FULL, TRUE);
        }
    }
...
void Lpuart_Lin_Ip_IRQHandler(const uint8 Instance)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                /* if a spurious interrupt occured. Clear all interrupt flag */
                (void)Lpuart_Lin_Ip_HwClearStatusFlag(Base, LPUART_LIN_IP_ALL_INT_FLAGS);
            }
        }
    }
...
static void Lpuart_Lin_Ip_CheckWakeupSignal(const uint8 Instance, const uint8 TmpByte)
{
    ...
    if (0u == (Lpuart_Lin_Ip_au8WakeupDetectInvert[Instance] & TmpByte))
    {
