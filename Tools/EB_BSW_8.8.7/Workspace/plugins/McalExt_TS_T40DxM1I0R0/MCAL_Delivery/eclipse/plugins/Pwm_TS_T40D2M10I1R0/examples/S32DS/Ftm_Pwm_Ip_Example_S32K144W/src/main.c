...
#define FTM_CH_7          (uint8)7U /* Channel_7*/
...
int main(void)
{
    ...
    Ftm_Pwm_Ip_UpdatePwmChannel(FTM_INSTANCE_0, FTM_CH_7, 0x4000, 0x000, TRUE);

    /* Setup new duty cycle to decreasing LED brightness */
    Ftm_Pwm_Ip_UpdatePwmChannel(FTM_INSTANCE_0, FTM_CH_7, 0x200, 0x000, TRUE);

    /* Using duty 0% and 100% to blink led */
    while(num_blink < NUM_BLINK_LED)
    {
        /* Led on when duty cycle is 100% */
        Ftm_Pwm_Ip_UpdatePwmChannel(FTM_INSTANCE_0, FTM_CH_7, 0x5000, 0x000, TRUE);
        TestDelay(DELAY_TIMER);

        /* Led off when duty cycle is 0% */
        Ftm_Pwm_Ip_UpdatePwmChannel(FTM_INSTANCE_0, FTM_CH_7, 0x0000, 0x000, TRUE);
        TestDelay(DELAY_TIMER);
        num_blink++;
    }
