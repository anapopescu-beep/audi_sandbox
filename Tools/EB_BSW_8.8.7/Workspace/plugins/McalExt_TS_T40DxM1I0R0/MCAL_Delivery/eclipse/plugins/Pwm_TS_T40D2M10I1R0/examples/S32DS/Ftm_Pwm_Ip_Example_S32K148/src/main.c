...
#define FTM_INSTANCE_4    (uint8)4U /* FTM_4 */
#define FTM_CH_1          (uint8)1U /* Channel_1*/
...
int main(void)
{
    ...
    Ftm_Pwm_Ip_Init(FTM_INSTANCE_4, &Ftm_Pwm_Ip_VS_0_UserCfg4);

    /* Set new period for Edge-Align Pwm */
    Ftm_Pwm_Ip_UpdatePwmPeriod(FTM_INSTANCE_4, 0x5000, TRUE);

    /* Setup new duty cycle to increasing LED brightness */
    Ftm_Pwm_Ip_UpdatePwmChannel(FTM_INSTANCE_4, FTM_CH_1, 0x4000, 0x000, TRUE);

    /* Setup new duty cycle to decreasing LED brightness */
    Ftm_Pwm_Ip_UpdatePwmChannel(FTM_INSTANCE_4, FTM_CH_1, 0x200, 0x000, TRUE);

    /* Using duty 0% and 100% to blink led */
    while(num_blink < NUM_BLINK_LED)
    {
        /* Led on when duty cycle is 100% */
        Ftm_Pwm_Ip_UpdatePwmChannel(FTM_INSTANCE_4, FTM_CH_1, 0x5000, 0x000, TRUE);
        TestDelay(DELAY_TIMER);

        /* Led off when duty cycle is 0% */
        Ftm_Pwm_Ip_UpdatePwmChannel(FTM_INSTANCE_4, FTM_CH_1, 0x0000, 0x000, TRUE);
        TestDelay(DELAY_TIMER);
        num_blink++;
    }

    /* De-initialize PWM driver */
    Ftm_Pwm_Ip_DeInit(FTM_INSTANCE_4);
