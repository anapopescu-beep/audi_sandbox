...
{
    {DMA_IP_CH_SET_SOURCE_ADDRESS, (uint32)&Src_data[0U]},
    {DMA_IP_CH_SET_DESTINATION_ADDRESS, (uint32)&Dest_data[0U]},
};
...
* @details      Example uses Mpu_Ip_Init to initialize regions for the MPU and uses DMA to perform data transfers from the source array to the destination array.
...
{
    uint16 i;
    Mpu_Ip_ErrorDetailsType ErrorDetails;
    boolean Status = TRUE;
    ...
    /* Initialize Mpu, DMA has read/write permission */
    Mpu_Ip_Init(&Mpu_Config_VS_0);
    ...
    Mpu_Ip_EnableRegion(1, (boolean)FALSE);
    Mpu_Ip_EnableRegion(2, (boolean)FALSE);
    ...
    /* Get error details */
    (void)Mpu_Ip_GetErrorDetails(&ErrorDetails);
    ...
    Mpu_Ip_EnableRegion(1, (boolean)TRUE);
    ...
    Mpu_Ip_EnableRegion(2, (boolean)TRUE);
