...
* @details      Example uses RM_init to initialize regions for the MPU and uses DMA to perform data transfers from the source array to the destination array.
*               Depending on the permission of RM, the data transfer is successful or not.
*               The steps are as follows:
*                 - Declare variables
*                 - Call Mcu_Init, Mcu_InitClock, Mcu_SetMode to initialize Clock
*                 - Call Mcl_Init to initialize DMA
*                 - Call Rm_Init to initialize 3 regions, with 3 regions configured as follows:
...
*                 - Call Rm_Mpu_EnableRegion to disable 2 regions, DMA has not read/write permission
*                 - Perform DMA transfer.
*                 - Verification Point: DMA transfer failed
*                 - Call Rm_Mpu_GetErrorDetails to get error information
*                 - Verification Point: check correct error information
*                 - Call Rm_Mpu_EnableRegion to enable region 1, DMA has only read permission
*                 - Perform DMA transfer.
*                 - Verification Point: DMA transfer failed
*                 - Call Rm_Mpu_GetErrorDetails to get error information
*                 - Verification Point: check correct error information
*                 - Call Rm_Mpu_EnableRegion to enable region 2, DMA has read/write permission
*                 - Perform DMA transfer.
*                 - Verification Point: DMA transfer successfully
*                 - Exit example
*/
...
{
    uint16 i;
    Rm_Mpu_ErrorDetailsType ErrorDetails;
    boolean Status = TRUE;
    ...
    Rm_Mpu_EnableRegion(1, (boolean)FALSE);
    Rm_Mpu_EnableRegion(2, (boolean)FALSE);
    ...
    /* Get error details */
    (void)Rm_Mpu_GetErrorDetails(&ErrorDetails);
    ...
    Rm_Mpu_EnableRegion(1, (boolean)TRUE);
    ...
    Rm_Mpu_EnableRegion(2, (boolean)TRUE);
