        ...
        if (LPSPI_IP_STATUS_SUCCESS == Lpspi_spiStatus)
        {
            ...
            if (LPSPI_IP_STATUS_SUCCESS == Lpspi_spiStatus)
            {
                ...
                if (LPSPI_IP_STATUS_SUCCESS == Lpspi_spiStatus)
                {
                    ...
                    #endif
                    {
                        Lpspi_spiStatus = Lpspi_Ip_SyncTransmit(LpspiExternalDevice, TxBuffer, RxBuffer, NumberOfBytes, SPI_TIMEOUT_COUNTER_U32);
                    }
                }
            }
        }
    ...
    else
    {
        ...
        if (FLEXIO_SPI_IP_STATUS_SUCCESS == Flexio_spiStatus)
        {
            ...
            if (FLEXIO_SPI_IP_STATUS_SUCCESS == Flexio_spiStatus)
            {
                ...
                if (FLEXIO_SPI_IP_STATUS_SUCCESS == Flexio_spiStatus)
                {
                    Flexio_spiStatus = Flexio_Spi_Ip_SyncTransmit(Flexio_SpiExternalDevice, TxBuffer, RxBuffer, NumberOfBytes, SPI_TIMEOUT_COUNTER_U32);
                }
            }
        }
    }
