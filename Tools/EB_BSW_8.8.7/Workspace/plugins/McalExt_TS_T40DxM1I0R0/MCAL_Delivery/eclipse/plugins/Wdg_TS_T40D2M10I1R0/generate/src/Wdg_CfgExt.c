...
[!IF "num:i($isPreCompileOrLinkTimeEnabled) = 1"!]
    [!IF "variant:size()<=1"!]
        ...
        [!FOR "i" = "0" TO "num:i(ecu:get('Wdg.Num.Of.Instances'))-1"!]
            ...
            [!IF "node:exists(concat($xPathModule,'/ELEMENTS/Wdg'))"!]
                [!IF "node:ref(concat($xPathModule,'/ELEMENTS/Wdg/IMPLEMENTATION_CONFIG_VARIANT'))='VariantPreCompile' or node:ref(concat($xPathModule,'/ELEMENTS/Wdg/IMPLEMENTATION_CONFIG_VARIANT'))='VariantLinkTime'"!]
                    [!CODE!][!IF "num:i($i) > 0"!] ,[!ENDIF!]&Wdg_Config_[!"$i"!][!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!][!ENDCODE!][!CR!]
                [!ELSE!]
...
[!IF "num:i($WdgDisableDemReport) = 1"!]
        ...
        [!ENDIF!]
            [!IF "node:exists(concat($xPathModule,'/ELEMENTS/Wdg/WdgDemEventParameterRefs'))"!]
                [!IF "node:ref(concat($xPathModule,'/ELEMENTS/Wdg/WdgGeneral/AutosarExt/WdgDisableDemReportErrorStatus'))='false'"!]
                    ...
                    [!IF "node:exists(concat($xPathModule,'/ELEMENTS/Wdg/WdgDemEventParameterRefs/WDG_E_DISABLE_REJECTED'))"!]
                        [!VAR "isWdgDemEventAvailable" = "0"!]
                        [!CODE!]const Mcal_DemErrorType Wdg_E_Disable_Rejected = [!ENDCODE!][!CR!]
                        [!CODE!]{[!ENDCODE!][!CR!]
                        [!CODE!]    (uint32)STD_ON,[!ENDCODE!][!CR!]
                        [!CODE!]    (uint16)DemConf_DemEventParameter_[!"string(node:ref(concat($xPathModule,'/ELEMENTS/Wdg/WdgDemEventParameterRefs/WDG_E_DISABLE_REJECTED'))/@name)"!][!ENDCODE!][!CR!]
                        [!CODE!]};[!ENDCODE!][!CR!][!CR!]
                        [!BREAK!]
                    [!ENDIF!]
                [!ENDIF!]
    ...
    [!FOR "i" = "0" TO "num:i(ecu:get('Wdg.Num.Of.Instances'))-1"!]
        ...
        [!IF "node:exists(concat($xPathModule,'/ELEMENTS/Wdg/WdgDemEventParameterRefs'))"!]
            [!IF "node:ref(concat($xPathModule,'/ELEMENTS/Wdg/WdgGeneral/AutosarExt/WdgDisableDemReportErrorStatus'))='false'"!]
                [!VAR "isWdgDemEventAvailable"    = "1"!]
                [!IF "node:exists(concat($xPathModule,'/ELEMENTS/Wdg/WdgDemEventParameterRefs/WDG_E_MODE_FAILED'))"!]
                    [!VAR "isWdgDemEventAvailable" = "0"!]
                    [!CODE!]const Mcal_DemErrorType Wdg_E_Mode_Failed = [!ENDCODE!][!CR!]
                    [!CODE!]{[!ENDCODE!][!CR!]
                    [!CODE!]    (uint32)STD_ON,[!ENDCODE!][!CR!]
                    [!CODE!]    (uint16)DemConf_DemEventParameter_[!"string(node:ref(concat($xPathModule,'/ELEMENTS/Wdg/WdgDemEventParameterRefs/WDG_E_MODE_FAILED'))/@name)"!][!ENDCODE!][!CR!]
                    [!CODE!]};[!ENDCODE!][!CR!][!CR!]
                    [!BREAK!]
                [!ENDIF!]
            [!ENDIF!]
