...
#if (WDG_SUPPORT_IACR == STD_ON)
#if (WDG_ENABLE_SET_SERVICE_ADDRESS == STD_ON)
#if (WDG_DIRECT_SERVICE == STD_ON)
/**
* @brief     Set the Instruction Address Compare Register.
* @details   The function Wdg_SetServiceAddress shall set the Instruction Address
*            that will service the Wdg when executed.
* @param[in] ServiceAddress  Address of instruction which will service the Wdg 
* @param[in] Instance  Harwdware instance
* @return    Std_ReturnType
* @retval    E_OK        Set the Instruction Address Compare Register successfully.
* @retval    E_NOT_OK    Returned if watchdog set Instruction Address Compare Register failed.
* @implements      Wdg_ChannelSetServiceAddress_Activity
*
**/
Std_ReturnType Wdg_ChannelSetServiceAddress(const Wdg_Ipw_InstanceType Instance, uint32 ServiceAddress)
{
    Std_ReturnType Ret = (Std_ReturnType)E_NOT_OK;
    
    Ret =  Wdg_Ipw_SetServiceAddress(Instance, ServiceAddress);
    #if (WDG_VALIDATE_PARAMS == STD_ON)
    if (Ret == (Std_ReturnType)E_NOT_OK)
    {
        (void)Det_ReportError((uint16)WDG_CHANNEL_MODULE_ID, (uint8)Instance, (uint8)WDG_SET_SERVICE_ADDRESS_ID, (uint8)WDG_E_PARAM_MODE);
    }
    #endif
...
#endif /* WDG_ENABLE_SET_SERVICE_ADDRESS == STD_ON */
#endif /* (WDG_SUPPORT_IACR == STD_ON) */
