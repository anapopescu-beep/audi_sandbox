...
typedef FUNC( void,MEMACC_CODE )MemAcc_MemInitFuncType(MemAcc_MemInstanceIdType InstanceId, const Mem_ConfigType* ConfigPtr);

/**  \brief Defines the type for flash memory Erase function . */
typedef FUNC( Std_ReturnType,MEMACC_CODE)MemAcc_MemEraseFuncType(MemAcc_MemInstanceIdType InstanceId, Mem_AddressType TargetAddress, Mem_LengthType Length );

/**  \brief Defines the type for flash memory BlankCheck function . */
typedef FUNC( Std_ReturnType,MEMACC_CODE)MemAcc_MemBlankCheckFuncType(MemAcc_MemInstanceIdType InstanceId, Mem_AddressType TargetAddress, Mem_LengthType Length );

/**  \brief Defines the type for flash memory Write function . */
typedef FUNC( Std_ReturnType,MEMACC_CODE)MemAcc_MemWriteFuncType(MemAcc_MemInstanceIdType InstanceId, Mem_AddressType TargetAddress, const uint8* SourceDataPtr, Mem_LengthType Length );

/**  \brief Defines the type for flash memory Read function . */
typedef FUNC( Std_ReturnType,MEMACC_CODE)MemAcc_MemReadFuncType(MemAcc_MemInstanceIdType InstanceId, Mem_AddressType SourceAddress, uint8* DestinationDataPtr, Mem_LengthType Length);

/**  \brief Defines the type for flash memory GetJobResult function . */
typedef FUNC( Mem_JobResultType,MEMACC_CODE)MemAcc_MemGetJobResultFuncType(MemAcc_MemInstanceIdType InstanceId);
...
typedef struct
{
  /** \brief AddressArea Index in the configuration. */
  MemAcc_AddressAreaIdType AddressAreaCfgIdx;
}MemAcc_QueueItemType;
...
typedef struct
{
  ...
  Mem_JobResultType MemDrvJobResult;
...
#endif
  /** \brief This flag is set to TRUE when Cancel request is called for this address area. */
  boolean JobCancelRequested;
