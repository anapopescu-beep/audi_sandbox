...
#else
#define NVM_START_SEC_VAR_POWER_ON_INIT_MC_SHARED_UNSPECIFIED
#include <NvM_MemMap.h>
/** \brief Stores the status of the NvM blocks.
 **/
...
#include <NvM_MemMap.h>
#else
#define NVM_STOP_SEC_VAR_POWER_ON_INIT_MC_SHARED_UNSPECIFIED
#include <NvM_MemMap.h>
#endif
#else /*MULTICORE ENABLED*/
#if (NVM_CLEAR_RAMBLOCK_AFTER_RESET == STD_ON)
...
#if (NVM_BUFFER_ALIGNMENT_BITS != 8)
    /* !LINKSTO SWS_NvM_00985,1 */
    if( ((uint32)NvM_TempRamBlockAddress & (NVM_BUFFER_ALIGNMENT_BITS/8 - 1)) != 0)
    {
...
#if (NVM_BUFFER_ALIGNMENT_BITS != 8)
        /* !LINKSTO SWS_NvM_00985,1 */
        if( ((uint32)((uint8 *)NvM_CurrentBlockDescriptorPtr->ramBlockDataAddress) & (NVM_BUFFER_ALIGNMENT_BITS/8 - 1)) != 0)
        {
          /*force use of internal buffer for unaligned address*/
          NvM_UseInternalBufferAlignment = TRUE;
        }
