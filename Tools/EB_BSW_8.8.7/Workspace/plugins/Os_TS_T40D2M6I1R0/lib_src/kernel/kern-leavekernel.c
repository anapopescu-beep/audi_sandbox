 ...
 *
 * $Id: kern-leavekernel.c 1.1 2023/04/18 07:06:23CEST Dan Dustinta (dan.dustinta) in_work  $
*/
...
#if ((OS_KERNEL_TYPE == OS_FUNCTION_CALL) || (OS_ARCH == OS_WINDOWS) || (OS_ARCH == OS_LINUX))
...
#else /* (!(OS_KERNEL_TYPE == OS_FUNCTION_CALL)) */
const os_uint8_t OS_dummy_kern_leavekernel_c = 0xeb;
