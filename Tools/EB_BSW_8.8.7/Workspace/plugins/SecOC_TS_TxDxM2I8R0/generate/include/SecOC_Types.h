...
typedef struct
{
  /* Csm Job ID */
  uint32                             CsmJobId;
[!IF "(node:value(SecOCGeneral/SecOCQueryFreshnessValue) = 'RTE') and (num:i(count(SecOCMainFunctionRx/*)) > 0)"!][!//
...
[!ENDIF!][!//
  /* Csm Job ID */
  uint32                            CsmJobId;
[!IF "(node:value(SecOCGeneral/SecOCQueryFreshnessValue) = 'RTE') and (num:i(count(SecOCMainFunctionTx/*)) > 0)"!][!//
  uint32                            MainFunctionIndex;
[!ENDIF!][!//
[!IF "node:value(SecOCGeneral/SecOCQueryFreshnessValue) = 'CFUNC'"!][!//
[!IF "$UseGetTxFreshnessTruncData = 'true'"!][!//
