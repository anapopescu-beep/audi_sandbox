...
[!ENDIF!][!//
    /* CsmFuncType */
    [!IF "node:exists(as:ref(as:ref(./SecOCRxAuthServiceConfigRef)/CsmJobPrimitiveRef)/CsmMacVerify) = 'true'"!]SECOC_RX_MACVERIFY_FUNC_TYPE[!ELSE!]SECOC_RX_SIGNATUREVERIFY_FUNC_TYPE[!ENDIF!],
...
[!IF "$RxUseCsmAsync = 'true'"!][!//
    /* UseCsmAsync */
    [!IF "node:exists(as:ref(as:ref(./SecOCRxAuthServiceConfigRef)/CsmJobPrimitiveRef)/CsmMacVerify) = 'true'"!][!//
    ...
    [!ELSEIF "node:exists(as:ref(as:ref(./SecOCRxAuthServiceConfigRef)/CsmJobPrimitiveRef)/CsmSignatureVerify) = 'true'"!][!//
...
[!IF "count(../../SecOCRxPduProcessing/*[node:name(node:ref(./SecOCRxPduMainFunctionRef)) = $MainFuncRx and node:value(./SecOCUseAuthDataFreshness) = 'false']) > 0"!][!//
    case [!"num:i($RxMainFunctionIndex)"!]U:
      ...
      break;
[!ENDIF!][!//
[!VAR "RxMainFunctionIndex" = "$RxMainFunctionIndex + 1"!][!//
[!ENDLOOP!][!//
    default:
      /* do nothing */
      break;
