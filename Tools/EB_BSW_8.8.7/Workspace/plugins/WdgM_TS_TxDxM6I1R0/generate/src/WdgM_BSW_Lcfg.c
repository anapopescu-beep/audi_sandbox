...
static VAR(WdgM_EB_SatteliteWdgIfSetModeFailedType, WDGM_VAR) WdgM_EB_SatteliteWdgIfSetModeFailed_Core[!"num:i($index_i)"!] = WDGM_EB_FALSE;
  [!ENDIF!]
[!ENDIF!]
...
[!IF "$WdgMTriggerWdgDriverMulticore"!]
  [!IF "$index_i != $WDGM_MASTER_CORE_ID"!]
  &WdgM_EB_SatteliteWdgIfSetModeFailed_Core[!"num:i($index_i)"!],
  [!ELSE!][!//
  NULL_PTR,
  [!ENDIF!]
[!ENDIF!]
...
[!IF "$WdgMTriggerWdgDriverMulticore"!]
FUNC(WdgM_EB_SatteliteWdgIfSetModeFailedType, WDGM_CODE) WdgM_EB_GetSatteliteWdgIfSetModeFailed(void)
{
  uint8 LocalCounter = 0;

  WdgM_EB_SatteliteWdgIfSetModeFailedType LocalStatus = WDGM_EB_FALSE;
  ...
  [!IF "$SupEntExistPerCore"!]
    [!IF "$index_i != $WDGM_MASTER_CORE_ID"!]
    {
      LocalStatus = WdgM_EB_SatteliteWdgIfSetModeFailed_Core[!"num:i($index_i)"!];
#if (WDGM_MIXED_CRITICALITY == STD_ON)
  ...
  return (LocalCounter > 0)? WDGM_EB_TRUE : WDGM_EB_FALSE;
}
[!ENDIF!]
