...
typedef uint16 WdgM_EB_SatteliteWdgIfSetModeFailedType;
#else
typedef uint8 WdgM_EB_LocalStatusType;
typedef uint8 WdgM_EB_ModeType;
typedef uint8 WdgM_EB_SatteliteWdgIfSetModeFailedType;
#endif
...
#if (WDGM_TRIGGER_WDG_DRIVER_MULTICORE_ENABLED == STD_ON)
  CONSTP2VAR(WdgM_EB_SatteliteWdgIfSetModeFailedType, AUTOMATIC, AUTOMATIC) WdgM_EB_SatteliteWdgIfSetModeFailed;
  /**<local status for the SatteliteWdgIfSetMode for a specific core*/
#endif
