...
/* patch version check */
#if (WDGM_SW_PATCH_VERSION < 43U)
#error WDGM_SW_PATCH_VERSION wrong (< 43U)
#endif
...
FUNC(void, WDGM_CODE) WdgM_MainFunction(void)
{
  ...
  else
  {
    ...
    {
      if(WdgM_EB_GetSatteliteWdgIfSetModeFailed() == WDGM_EB_TRUE)    
      {
  ...
  for (TriggerId = 0U; TriggerId < ModeCfg->NumTriggers; TriggerId++)
  {
...
#endif
    {
      ...
      if (RetVal != E_OK)
      {
        ...
        {
#if (WDGM_MIXED_CRITICALITY == STD_ON)
          if ((*CoreInstance->WdgM_EB_SatteliteWdgIfSetModeFailed == WDGM_EB_FALSE) || (*CoreInstance->WdgM_EB_SatteliteWdgIfSetModeFailed == WDGM_EB_TRUE))
#endif
          {
            /* !LINKSTO WDGM.EB.TIMEPM.WDGM020473,1 */
            *CoreInstance->WdgM_EB_SatteliteWdgIfSetModeFailed = WDGM_EB_TRUE;
          }
#if (WDGM_MIXED_CRITICALITY == STD_ON)
          else
          {
