...
/* patch version check */
#if (WDGM_SW_PATCH_VERSION < 4U)
#error WDGM_SW_PATCH_VERSION wrong (< 4U)
#endif
...
#if (WDGM_MIXED_CRITICALITY == STD_ON)
STATIC VAR(uint16, WDGM_VAR) WdgM_EB_SatteliteWdgIfSetModeFailed = WDGM_EB_FALSE;
#else
STATIC VAR(boolean, WDGM_VAR) WdgM_EB_SatteliteWdgIfSetModeFailed = WDGM_EB_FALSE;
#endif
#endif
...
#if(WDGM_TRIGGER_WDG_DRIVER_MULTICORE_ENABLED == STD_ON)
#if (WDGM_MIXED_CRITICALITY == STD_ON)
    if ((WdgM_EB_SatteliteWdgIfSetModeFailed == WDGM_EB_FALSE) || (WdgM_EB_SatteliteWdgIfSetModeFailed == WDGM_EB_TRUE))
#endif
    {
      if((CoreInstance->CoreID == WDGM_EB_MASTER_INSTANCE) && (WdgM_EB_SatteliteWdgIfSetModeFailed == WDGM_EB_TRUE))
      {
...
)
{
  ...
  /* !LINKSTO WDGM.EB.TIMEPM.WDGM020473,1 */
  ...
  if (RetVal == E_OK)
  {
...
#endif
    {
      ...
      /* !LINKSTO WDGM.ASR40.WDGM351,1,WDGM.EB.TIMEPM.WDGM020473,1 */
...
#endif
    {
      ...
      if (RetVal != E_OK)
      {
        ...
        else
        {
          ...
          {
            /* !LINKSTO WDGM.EB.TIMEPM.WDGM020473,1 */
            WdgM_EB_SatteliteWdgIfSetModeFailed = WDGM_EB_TRUE;
          }
#if (WDGM_MIXED_CRITICALITY == STD_ON)
          else
          {
...
{
  Std_ReturnType RetVal;
  const WdgM_EB_SEIdType CheckedExpiredSEID = (WdgM_EB_SEIdType)(RedundantData);
  const WdgM_EB_SEIdType ExpiredSEIDRedundant = (WdgM_EB_SEIdType)
      (~((WdgM_EB_SEIdType)(((WdgM_EB_ExpiredSEIDType)(RedundantData >> 8U))
          & (uint8)0xFFU)));
...
#else
      WdgM_EB_ModeType LocalMode;
      LocalMode = WdgM_EB_GetCoreSwitchMode(WDGM_EB_MASTER_INSTANCE);
      /* We use cast to void to keep consistency when reading data for WdgM_EB_CoreSwitchMode_Core.
       * Since we check the value of the master core, there is no need to check for data corruption
       * because the master cannot be corrupted (satellites have only read data access)
       */
      (void)WdgM_EB_GetCheckedRedundantData8(LocalMode, &ExpectedWdgMMode);
#endif
