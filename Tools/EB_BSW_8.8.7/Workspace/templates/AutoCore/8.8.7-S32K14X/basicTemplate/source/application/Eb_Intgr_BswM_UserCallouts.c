...
#if defined(BASE_ETH_ENABLED)
#if(BASE_ETH_ENABLED==STD_ON)
#if defined(ETH_EB_MIIINIT)
  /* For EB Eth drivers: Initialization of only MII management so that EthTrcv can be accessed for initialization. */
  ETH_EB_MIIINIT();
#endif
#endif
#endif
/* If necessary add the activation of the EthTrcv via switching port pins.  */
#if defined(BASE_ETHTRCV_ENABLED)
#if(BASE_ETHTRCV_ENABLED==STD_ON)
#if defined(ETHTRCV_EB_TRANSEIVERINIT)
  ETHTRCV_EB_TRANSEIVERINIT();
  /* Remark: Alternative the function EthTrcv_1_T01_TransceiverInit() must be called manually.  */
#endif
#endif
#endif
#if defined(BASE_ETH_ENABLED)
  ...
  /* For EB Eth drivers: Initialization of Eth controller for Eth frame processing. */
