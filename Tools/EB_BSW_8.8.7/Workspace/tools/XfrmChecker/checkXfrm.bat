...
SET XFRM_CLASS_PATH=%CP%;
rem append all (jar) files found in hashes.txt to the classpath
for /F "tokens=2" %%F in (%CURRFOLDER%\XfrmCV\hashes.txt) do SET "XFRM_CLASS_PATH=!XFRM_CLASS_PATH!%CURRFOLDER%\XfrmCV\%%F;
...
rem if the value of STATUS is 0 and if the given folder contains file hashes.txt
rem
:checkIntegrity
if %STATUS% EQU 0 (
    pushd %1
    if exist .\hashes.txt (
        ...
        for /F "tokens=2" %%F in (.\hashes.txt) do call :checkIntegrityForFile .\hashes.txt .\setEnvVars.bat %%F
        popd
        if %errorlevel% NEQ 0 (
      ...
      @echo Error: Did not find hashes.txt in folder %1.
      SET STATUS=43
    )
)
exit /b %errorlevel%
