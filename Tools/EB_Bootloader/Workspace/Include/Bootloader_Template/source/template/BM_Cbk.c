...
void BM_SoftwareInit(void)
{
    u16 i;
    u16 j;
    uint8 num_test_block[8];
    
    /* NvM init (without FLS init) */
    BL_Nvm_Init();


    /* Read bootlader_Data NvM block */
    BL_Nvm_ReadSync((uint8*)&stNVMBootloaderData,NvMConf_NvMBlockDescriptor_NVM_BOOTLOADER_DATA);
    ...
    if (stNVMBootloaderData.ulInitPattern != NVM_INIT_PATTERN)
    {
        ...
        BL_Nvm_WriteSync((uint8*)&stNVMBootloaderData,NvMConf_NvMBlockDescriptor_NVM_BOOTLOADER_DATA);
