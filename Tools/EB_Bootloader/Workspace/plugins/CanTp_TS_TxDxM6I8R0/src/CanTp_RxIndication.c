...
/** \brief CanTp_CalculateNPduMaxLength()
 **
 ** Internal function to calculate the NPdu max. length
 **
 **
 ...
 ** \retval TRUE: NPduLength is valid.
 */
STATIC FUNC(PduLengthType, CANTP_CODE) CanTp_CalculateNPduMaxLength
(
  PduIdType NSduId,
    ...
    if(CanTp_FindNSduId(CanTpRxPduId, CanTpRxPduPtr->SduDataPtr, &NSduId, &FrameType))
    {
...
#endif /* CANTP_PARALLEL_CHANNELS_ENABLED == STD_ON */
      {
        ...
        if(FrameType == CANTP_N_PCI_TYPE_FC)
        {
          ...
          NPduMaxLength = CanTp_CalculateNPduMaxLength(NSduId, CanTpRxPduPtr->SduLength, AddressingFormat);
        }
        else
        {
        ...
        if(((FrameType == CANTP_N_PCI_TYPE_FC) && (CanTpRxPduPtr->SduLength == NPduMaxLength)) || ((FrameType != CANTP_N_PCI_TYPE_FC) && (CanTpRxPduPtr->SduLength <= NPduMaxLength)))
        {
  ...
  if(CANTP_CFG_GET_TXNSDU_CONFIG(NSduId).PaddingActivation == CANTP_OFF)
  {
    if(SduLength != CANTP_NPDU_LENGTH_8)
    {
      if(AddressingFormat == CANTP_STANDARD)
      {
        ...
        NPduMaxLength = CANTP_FC_NPCI_SIZE;
      }
      else
      {
        /* N-PDU length must be the address byte (if exist) + FC N_PCI header */
        NPduMaxLength = 1U + CANTP_FC_NPCI_SIZE;
      }
    }
  }
#if (CANTP_CANFD_ENABLED == STD_ON)
