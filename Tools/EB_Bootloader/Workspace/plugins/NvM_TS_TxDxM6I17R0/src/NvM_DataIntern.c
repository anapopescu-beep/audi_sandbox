...
#define NVM_START_SEC_MC_SHARED_VAR_POWER_ON_INIT_8
#include <NvM_MemMap.h>
#else
...
#if ( defined(NVM_MULTICORE_ENABLED) )
#define NVM_STOP_SEC_MC_SHARED_VAR_POWER_ON_INIT_8
#include <NvM_MemMap.h>
#else
...
FUNC(void,NVM_CODE) NvM_CryptoOperations_Async_Error(void)
{
  ...
  /* set CallLevel to corresponding levels, 1 for MultiBlock request so they continue and 0 for single block to terminate them */
  /* !LINKSTO NvM.CryptoHooks.CryptoFailDuringWrite,1 */
  /* !LINKSTO NvM.CryptoHooks.CryptoFailDuringRead,1 */
  if((NvM_CurrentServiceId == NVM_READ_ALL_API_ID) || (NvM_CurrentServiceId == NVM_WRITE_ALL_API_ID))
  {
    /* !LINKSTO NvM.CryptoHooks.CryptoFailDuringReadWriteAll,1 */
