...
(
   ...
   PduR_PduLengthType TpSduLength
); /* TODO: test what happens if the second buffer (queued) is requested with TpSduLength 0 */
...
(
   PduR_PduLengthType BufferSizeMin, /* TODO: check what happens if set to 0 */
   PduR_PduLengthType BufferSizeMax  /* TODO: check what happens if both are of the same */
);
...
(
   PduR_PduLengthType BufferSizeMin, /* TODO: check what happens if set to 0 */
   PduR_PduLengthType BufferSizeMax  /* TODO: check what happens if both are of the same */
) /* TODO: check what happens if BufferSizeMin >= BufferSizeMax */
{
...
)
{
   ...
   else
   {
      ...
      while((BufTpIndex < PduR_GConfigPtr->PduR_BufTpCount) &&                  /* TODO: test where smallest buffer holds BufferSizeSmallest already */
   ...
   do{
      ...
      if(BufferSizeMin > pBufTpConfig[SessionIndex].Length)
      {
         LowerSizeReached = TRUE;
          ...
          (ResultGetBitOccupiedSession > 0U) &&
          (LowerSizeReached == FALSE));     /* TODO: there should be a test case that the smallest configured buffer is taken */
