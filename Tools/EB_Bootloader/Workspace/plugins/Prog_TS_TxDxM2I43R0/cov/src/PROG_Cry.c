...
tProgStatus PROG_VerifySignature(const Csm_AsymPublicKeyType* pstAsymPublicKey, u8* pubData, u32 ulDataSize, const u8* pubSignature)
{
    ...
    ICOV(0);DBG_PROG_VERIFYSIGNATURE_ENTRY(pstAsymPublicKey, pubData, ulDataSize, pubSignature);
...
tProgStatus PROG_VerifySignatureStart(const Csm_AsymPublicKeyType* pstAsymPublicKey, const u8* pubSignature)
{
    ...
    ICOV(1);DBG_PROG_VERIFYSIGNATURESTART_ENTRY(pstAsymPublicKey, pubSignature);
    ...
    if ((eCsmStatus == CSM_E_OK)?BCOV(2):!BCOV(3))
    {
    ...
    if ((eCsmStatus == CRYPTO_E_JOB_CANCELED)?BCOV(4):!BCOV(5))
    {
...
#endif
    if((eCsmStatus != CSM_E_OK)?BCOV(6):!BCOV(7))
    {
...
{
    Csm_ReturnType eCsmStatus;
    ICOV(8);DBG_PROG_VERIFYSIGNATUREUPDATE_ENTRY(pubData, ulDataSize);

    if((m_eCryptoStatus != PROG_E_NOT_OK)?BCOV(9):!BCOV(10))
    {
...
{
    Csm_ReturnType eCsmStatus;
    ICOV(11);DBG_PROG_VERIFYSIGNATUREFINISH_ENTRY(pubSignature);

    if((m_eCryptoStatus != PROG_E_NOT_OK)?BCOV(12):!BCOV(13))
    {
        if ((pubSignature != NULL_PTR)?BCOV(14):!BCOV(15))
        {
        ...
        if ((m_eCryptoStateOnHold != PROG_SIGNATURE_CHECK_FINISH)?BCOV(16):!BCOV(17))
        {
            UDS_StopNRC78Timer();
...
#if (PROG_CSMASR43_USED == STD_ON)
#if (PROG_SIGNFIN_SENDFRP == STD_ON)
        if ((m_eCryptoStateOnHold == PROG_IDLE)?BCOV(18):!BCOV(19))
#endif
        {
            eCsmStatus = Csm_SignatureVerify(m_astProgCsmJobConf[m_ubActiveProgCsmTableId].ulCsmJobId,
...
tProgStatus PROG_GetCryptoStatus(void)
{
    ...
    ICOV(20);DBG_PROG_GETCRYPTOSTATUS_ENTRY();
    eStatus = m_eCryptoStatus;
...
tProgCryState PROG_GetCryptoState(void)
{
    ...
    ICOV(21);DBG_PROG_GETCRYPTOSTATE_ENTRY();
...
tProgStatus PROG_ComputeHashStart(u16 uwCsmConfigId, u8 const * pubData,u32 ulDataSize, u8 * pubHashResult)
{
    ...
    ICOV(22);OSC_PARAM_UNUSED(pubHashResult);
#endif

    ICOV(22);DBG_PROG_COMPUTEHASHSTART_ENTRY(uwCsmConfigId,pubData,ulDataSize,pubHashResult);

    m_eCryptoStatus = PROG_E_BUSY;
    m_ulDigestLength = PROG_DIGEST_LENGTH;

    if ((NULL_PTR != pubData)?BCOV(23):!BCOV(24))
    {
        m_pubCryptoData = pubData;
        m_ulCryptoDataSize = ulDataSize;
    ...
    if ((eCsmStatus == CSM_E_OK)?BCOV(25):!BCOV(26))
    {
...
#endif
#if(PROG_CSM_CANCELJOB_ENABLED == STD_ON)
    if ((eCsmStatus == CRYPTO_E_JOB_CANCELED)?BCOV(27):!BCOV(28))
    {
        /* Wait until the Crypto Cancellation operation is completed and a notification is received */
...
#endif
    if ((CSM_E_OK != eCsmStatus)?BCOV(29):!BCOV(30))
    {
        m_eCryptoStatus = PROG_E_NOT_OK;
...
void PROG_ComputeHashUpdate(u8 const * pubData, u32 ulDataSize)
{
    ...
    ICOV(31);DBG_PROG_COMPUTEHASHUPDATE_ENTRY(pubData, ulDataSize);

    if((m_eCryptoStatus != PROG_E_NOT_OK)?BCOV(32):!BCOV(33))
    {
...
void PROG_ComputeHashFinish(u8* pubHashValue)
{
    ...
    ICOV(34);DBG_PROG_COMPUTEHASHFINISH_ENTRY(pubHashValue);

    if(((m_eCryptoStatus != PROG_E_NOT_OK) && (m_eCryptoState == PROG_HASH_END))?BCOV(35):!BCOV(36))
    {
        m_eCryptoState = PROG_HASH_FINISH;
#if (PROG_HASHFIN_SENDFRP == STD_ON)
        /* Trigger to provide a response pending to prevent communication timeouts in
         * case the duration of crypto processing takes long time */
        if ((m_eCryptoStateOnHold != PROG_HASH_FINISH)?BCOV(37):!BCOV(38))
        {
            UDS_StopNRC78Timer();
            m_eCryptoStateOnHold = m_eCryptoState;
...
#if (PROG_CSMASR43_USED == STD_ON)
#if (PROG_HASHFIN_SENDFRP == STD_ON)
        if ((m_eCryptoStateOnHold == PROG_IDLE)?BCOV(39):!BCOV(40))
#endif
        {
            eCsmStatus = Csm_Hash(m_astProgCsmJobConf[m_ubActiveProgCsmTableId].ulCsmJobId,
                                CRYPTO_OPERATIONMODE_FINISH,
...
NCS_PRI_FCT void PROG_CsmMacGenerateNotification(void)
{
    ...
    Csm_ReturnType eCsmStatus;
#endif
    ICOV(41);switch (m_eCryptoState)
    {
        case PROG_MAC_GENERATE_START:
            ICOV(42);m_eCryptoStatus = PROG_E_OK;
            ...
            if ((PROG_TRUE == eProgBoolean)?BCOV(43):!BCOV(44))
            {
        ...
        case PROG_MAC_GENERATE_UPDATE:
            ICOV(45);m_eCryptoStatus = PROG_E_OK;
            ...
            if ((PROG_TRUE == eProgBoolean)?BCOV(46):!BCOV(47))
            {
        ...
        case PROG_MAC_GENERATE_FINISH:
            ICOV(48);m_eCryptoStatus = PROG_E_OK;
            ...
            if ((PROG_TRUE == eProgBoolean)?BCOV(49):!BCOV(50))
            {
...
void PROG_GetMacKey(Csm_SymKeyType* pstMacKey)
{
    ...
    ICOV(51);DBG_PROG_GETMACKEY_ENTRY(pstMacKey);
    ...
    for (ulIndex = 0U; (ulIndex < (ulMacKeyLength))?BCOV(52):!BCOV(53); ulIndex++)
    {
        paubKeyDataPtr[ulIndex]  = paubKeyData[ulIndex];
    }
#else
    for (ubIdx = 0U; (ubIdx < (ulMacKeyLength/sizeof(u32)))?BCOV(54):!BCOV(55); ubIdx++)
    {
...
tProgStatus PROG_MacGenerateStart(u16 uwCsmConfigId, u8 const * pubData, u32 ulDataSize, u8 * pubMacResult)
{
    ...
    ICOV(56);DBG_PROG_MACGENERATESTART_ENTRY(uwCsmConfigId,pubData,ulDataSize,pubMacResult);

    m_eCryptoStatus = PROG_E_BUSY;

    if ((NULL_PTR != pubData)?BCOV(57):!BCOV(58))
    {
        m_pubCryptoData = pubData;
        m_ulCryptoDataSize = ulDataSize;
#if (PROG_CSM_MAC_USED == STD_ON)
    ...
    if ((eCsmStatus == CSM_E_OK)?BCOV(59):!BCOV(60))
    {
...
#endif
#if(PROG_CSM_CANCELJOB_ENABLED == STD_ON)
    if ((eCsmStatus == CRYPTO_E_JOB_CANCELED)?BCOV(61):!BCOV(62))
    {
        /* Wait until the Crypto Cancellation operation is completed and a notification is received */
...
#endif
    if ((CSM_E_OK != eCsmStatus)?BCOV(63):!BCOV(64))
    {
        m_eCryptoStatus = PROG_E_NOT_OK;
        eProgStatus     = PROG_E_NOT_OK;
    }
...
Std_ReturnType PROG_CsmNotification(Csm_ReturnType eResult)
{
    ...
    ICOV(65);DBG_PROG_CSMNOTIFICATION_ENTRY(eResult);
    ...
    if ((CSM_E_OK == eResult)?BCOV(66):!BCOV(67))
    {
        switch (m_eCryptoState)
        {
            ...
            case PROG_SIGNATURE_CHECK_FINISH:
                ICOV(68);PROG_CsmSignatureCheckNotification();
                break;
            ...
            case PROG_HASH_FINISH :
                ICOV(69);PROG_CsmHashNotification();
                break;
#endif /* ((PROG_HASH_CALCULATION == STD_ON) || (PROG_CSM_HASH_USED == STD_ON)|| ((PROG_IMPL_VARIANT == PROG_IMPL_31) && (PROG_REPROG_TYPE == PROG_STANDARD_REPROG))) */
#if (PROG_CSM_MAC_USED == STD_ON)
            /* Management of the Hash */
...
NCS_PRI_FCT void PROG_CsmSignatureCheckNotification(void)
{
    ICOV(72);switch(m_eCryptoState)
    {
    case PROG_SIGNATURE_CHECK_START:
#if (PROG_SINGLE_STEP_SIGNATURE_CHECK == STD_OFF)
        ICOV(73);m_eCryptoStatus = PROG_E_OK;
...
#endif
#else
        ICOV(74);PROG_VerifySignatureUpdate(m_pubCryptoData, m_ulCryptoDataSize);
#endif
        ICOV(75);break;

    case PROG_SIGNATURE_CHECK_UPDATE:
#if (PROG_SINGLE_STEP_SIGNATURE_CHECK == STD_OFF)
        ICOV(76);m_eCryptoStatus = PROG_E_OK;
...
#endif
#else
        ICOV(77);PROG_VerifySignatureFinish(m_pubProgCryptoSignature);
#endif
        ICOV(78);break;

    case PROG_SIGNATURE_CHECK_FINISH:
        ICOV(79);m_eCryptoStatus = PROG_E_OK;
        m_eCryptoState = PROG_IDLE;

        if ((m_ubProgVerifyResult == CSM_E_VER_OK)?BCOV(80):!BCOV(81))
        {
            /* Emit event PROG_EVENT_CRYPTO_FINISH */
            PROG_HsmEmit (&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_CRYPTO_FINISH);
        }
...
void PROG_GetPublicKey(Csm_AsymPublicKeyType* pstPublicKey)
{
    ...
    ICOV(82);PROG_CustomGetAsymPublicKey(&paubPublicModulus, &ulPublicExponent);
    ...
    for(uwIdx = 0U; /* Modulus */
        (uwIdx < PROG_KEY_LENGTH)?BCOV(83):!BCOV(84);
        uwIdx++)
    {
    ...
    for(uwIdx = 0U; /* Modulus */
        (uwIdx < pstPublicKey->data[(PROG_KEY_LENGTH/sizeof(u32)) + 1])?BCOV(85):!BCOV(86);
        uwIdx++)
    {
    ...
    for(uwIdx = 0U; /* Modulus */
        (uwIdx < pstPublicKey->data[0])?BCOV(87):!BCOV(88);
        uwIdx++)
    {
...
NCS_PRI_FCT void PROG_CsmHashNotification(void)
{
    ICOV(89);switch (m_eCryptoState)
    {
        case PROG_HASH_START:
            ICOV(90);m_eCryptoStatus = PROG_E_OK;
            m_eCryptoState = PROG_HASH_START_END;

#if (PROG_CSM_HASH_USED == STD_ON)
            /* Check if hash computation is for Authentication. In positive case all crypto calculation triggering is performed */
            /* directly in not */
            if ((PROG_TRUE == PROG_SecurityPendingOperation())?BCOV(91):!BCOV(92))
            {
        ...
        case PROG_HASH_UPDATE:
            ICOV(93);m_eCryptoStatus = PROG_E_OK;
            m_eCryptoState = PROG_HASH_END;

#if (PROG_CSM_HASH_USED == STD_ON)
            /* Check if hash computation is for SecureChecksumComputation */
            if ((PROG_TRUE == PROG_SecurityPendingOperation())?BCOV(94):!BCOV(95))
            {
        ...
        case PROG_HASH_FINISH:
            ICOV(96);m_eCryptoStatus = PROG_E_OK;
            m_eCryptoState = PROG_IDLE;
            ...
            if ((PROG_TRUE == PROG_SecurityPendingOperation())?BCOV(97):!BCOV(98))
            {
                /* Notify security feature with hash value */
                PROG_SecurityChecksumNotification();
            }
...
void PROG_CsmCheckResult(Csm_ReturnType eCsmStatus)
{
    ...
    ICOV(99);if ((CSM_E_OK == eCsmStatus)?BCOV(100):!BCOV(101))
    {
        m_eCryptoStatus = PROG_E_BUSY;
    }
    else if ((eCsmStatus == CSM_E_BUSY)?BCOV(102):!BCOV(103))
    {
...
#endif
#if (PROG_SINGLE_STEP_SIGNATURE_CHECK == STD_ON)
        if(((PROG_SIGNATURE_CHECK_UPDATE == m_eCryptoState) || (PROG_SIGNATURE_CHECK_FINISH == m_eCryptoState))?BCOV(104):!BCOV(105))
        {
            eRejectBusy = PROG_TRUE;
        }
#endif
        if((PROG_TRUE == eRejectBusy)?BCOV(106):!BCOV(107))
        {
    ...
    else
    {
        m_eCryptoStatus = PROG_E_NOT_OK;
#if (PROG_IMPL_VARIANT == PROG_IMPL_20 )
        if((!((PROG_SIGNATURE_CHECK_UPDATE == m_eCryptoState) || (PROG_SIGNATURE_CHECK_FINISH == m_eCryptoState)))?BCOV(108):!BCOV(109))
#endif
        {
            /* Emit failed event to indicate that the Crypto Finish operation failed
               except for Signature verification of Impl20 */
...
{
    ICOV(110);switch (m_eCryptoState)
    {
    ...
    case PROG_SIGNATURE_CHECK_START_DONE:
        ...
        ICOV(111);(void)PROG_VerifySignatureStart(m_pstAsymPublicKey,  m_pubProgCryptoSignature);
        break;
    ...
    case PROG_SIGNATURE_CHECK_STREAMSTART_DONE:
        ...
        ICOV(112);m_eCryptoState = PROG_SIGNATURE_CHECK_START;
        PROG_CsmSignatureCheckNotification();
        break;
    ...
    case PROG_SIGNATURE_CHECK_START:
        ...
        ICOV(113);PROG_CsmSignatureCheckNotification();
        break;

    case PROG_SIGNATURE_CHECK_UPDATE_DONE:
        /* Callback received for the Crypto Job Update operation */
        ICOV(114);if (((m_ulCryptoTotalBlockDataLength > m_ulCryptoProcessedDataLength) &&
                (m_astProgCsmJobConf[m_ubActiveProgCsmTableId].ulBSsize != 0U))?BCOV(115):!BCOV(116))
        {
    ...
    case PROG_SIGNATURE_CHECK_FINISH_TXCONF:
        ...
        ICOV(117);PROG_VerifySignatureFinish(NULL_PTR);
        break;

    case PROG_SIGNATURE_CHECK_FINISH_DONE:
        /* Signature Verification is complete */
        ICOV(118);m_eCryptoState = PROG_SIGNATURE_CHECK_FINISH;
        PROG_CsmSignatureCheckNotification();
        break;
#endif /*#if (PROG_SIGNATURE_CALCULATION == STD_ON)*/
    ...
    case PROG_HASH_START_DONE:
        ICOV(119);(void)PROG_ComputeHashStart((u16)m_ubActiveProgCsmTableId, m_pubCryptoData ,m_ulCryptoDataSize, m_pubHashValue);
        break;
#endif/*(PROG_CSM_CANCELJOB_ENABLED == STD_ON)*/

    case PROG_HASH_START:
        /* Simulate the Hash Start Notification */
        ICOV(120);PROG_CsmHashNotification();
        break;

    case PROG_HASH_UPDATE_DONE:
        /* Callback received for the Crypto Job Update operation */
        ICOV(121);if (((m_ulCryptoTotalBlockDataLength != m_ulCryptoProcessedDataLength) &&
                (m_astProgCsmJobConf[m_ubActiveProgCsmTableId].ulBSsize != 0U))?BCOV(122):!BCOV(123))
        {
            /* Trigger to start to process the pending data */
    ...
    case PROG_HASH_FINISH_TXCONF:
        /* Received TxConf, resume the Crypto Finish operation */
        ICOV(124);m_eCryptoState = PROG_HASH_END;
        PROG_ComputeHashFinish(m_pubHashValue);
        break;

    case PROG_HASH_FINISH_DONE :
        ICOV(125);m_eCryptoState = PROG_HASH_FINISH;
        PROG_CsmHashNotification();
        break;
#endif /* ((PROG_HASH_CALCULATION == STD_ON)|| (PROG_CSM_HASH_USED == STD_ON) || ((PROG_IMPL_VARIANT == PROG_IMPL_31) && (PROG_REPROG_TYPE == PROG_STANDARD_REPROG))) */
    ...
    case PROG_MAC_GENERATE_START_DONE:
        ICOV(126);(void)PROG_MacGenerateStart((u16)m_ubActiveProgCsmTableId, m_pubCryptoData, m_ulCryptoDataSize, m_pubMacValue);
        break;
#endif/*(PROG_CSM_CANCELJOB_ENABLED == STD_ON)*/

    case PROG_MAC_GENERATE_START:
        /* Simulate the Mac Generate Notification */
        ICOV(127);PROG_CsmMacGenerateNotification();
        break;

    case PROG_MAC_GENERATE_UPDATE_DONE:
        /* Callback received for the Crypto Job Update operation */
        ICOV(128);if (((m_ulCryptoTotalBlockDataLength != m_ulCryptoProcessedDataLength) &&
                (m_astProgCsmJobConf[m_ubActiveProgCsmTableId].ulBSsize != 0U))?BCOV(129):!BCOV(130))
        {
            /* Trigger to start to process the pending data */
            m_eCryptoState = PROG_MAC_GENERATE_START;
        }
    ...
    case PROG_MAC_GENERATE_FINISH_DONE:
        ICOV(131);m_eCryptoState = PROG_MAC_GENERATE_FINISH;
        PROG_CsmMacGenerateNotification();
        break;
#endif /* PROG_CSM_MAC_USED == STD_ON  */
    ...
    case PROG_CSM_JOBCANCEL_DONE:
        ...
        ICOV(132);m_eCryptoState = m_eCryptoStateOnHold | PROG_E_NOT_BUSY;
        break;
...
Csm_ReturnType PROG_CsmSetPreConditions(const u8 * ubKeyPtr, u8 ubProgCsmJobTabId)
{
    ...
    ICOV(135);m_ulCryptoTotalBlockDataLength = 0U;
    ubPtr2Key = ubKeyPtr;
    eCsmStatus = E_OK;

#if(PROG_CSM_CANCELJOB_ENABLED == STD_ON)
    if((m_astProgCsmJobConf[ubProgCsmJobTabId].eAllowJobCancellation == PROG_TRUE)?BCOV(136):!BCOV(137))
    {
        eCsmStatus = PROG_CsmCancelActiveJobs();
    }
#endif /* (PROG_CSM_CANCELJOB_ENABLED == STD_ON) */
#if(PROG_CUSTOM_CSMSTARTPREPROCESS_ENABLED == STD_ON)
    if(((m_astProgCsmJobConf[ubProgCsmJobTabId].eAllowCustCsmPreProc == PROG_TRUE) && (eCsmStatus == E_OK))?BCOV(138):!BCOV(139))
    {
    ...
    if(((m_astProgCsmJobConf[ubProgCsmJobTabId].eAllowKeySet == PROG_TRUE) && \
            (ubPtr2Key != NULL_PTR) && (eCsmStatus == E_OK))?BCOV(140):!BCOV(141))
    {
        ...
        if ((eCsmStatus == E_OK)?BCOV(142):!BCOV(143))
        {
...
Csm_ReturnType PROG_CsmCancelActiveJobs(void)
{
    ...
    ICOV(144);eCsmCancelStatus = CSM_E_OK;

    /* Generic way to cancel all the configured jobs in Boot */
    for (ulCsmjobIndex = 0U; ((ulCsmjobIndex < PROG_CSM_ALLJOBS_COUNT) && (CRYPTO_E_JOB_CANCELED != eCsmCancelStatus))?BCOV(145):!BCOV(146); ulCsmjobIndex++)
    {
...
{
    ICOV(147);DBG_PROG_CSMNOTIFICATION_ENTRY(result);
    ...
    if((m_astProgCsmJobConf[PROG_CSMDECRYPTIONJOBINDEX].ulCsmJobId == job->jobId)?BCOV(148):!BCOV(149))
    {
...
#endif
    {
        ...
        if ((CRYPTO_E_JOB_CANCELED!= result)?BCOV(150):!BCOV(151))
        {
            ...
            if (((m_astProgCsmJobConf[m_ubActiveProgCsmTableId].ulBSsize != 0U) && (result == E_OK) && \
                    (m_ulCryptoProcessedDataLength < m_ulCryptoTotalBlockDataLength))?BCOV(152):!BCOV(153))
            {
...
{
    ICOV(154);if((m_astProgCsmJobConf[m_ubActiveProgCsmTableId].ulBSsize != 0U)?BCOV(155):!BCOV(156))
    {
        if ((m_ulCryptoTotalBlockDataLength <= m_ulCryptoProcessedDataLength)?BCOV(157):!BCOV(158))
        {
...
{
    ICOV(159);if (((m_eCryptoStateOnHold == PROG_HASH_FINISH) || (m_eCryptoStateOnHold == PROG_SIGNATURE_CHECK_FINISH))?BCOV(160):!BCOV(161))
    {
