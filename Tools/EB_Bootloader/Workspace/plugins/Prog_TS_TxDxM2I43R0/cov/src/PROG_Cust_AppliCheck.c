...
void PROG_Entry_AutoControl(void)
{
    ...
    ICOV(162);DBG_PROG_ENTRY_AUTOCONTROL_ENTRY();
    ...
    if ((PROG_E_OK == eProgStatus)?BCOV(163):!BCOV(164))
    {
...
#endif /*((PROG_IMPL_VARIANT == PROG_IMPL_10)||(PROG_IMPL_VARIANT == PROG_IMPL_11))*/
#else
    ICOV(165);DBG_PROG_ENTRY_AUTOCONTROL_ENTRY();
#endif
    ICOV(165);DBG_PROG_ENTRY_AUTOCONTROL_EXIT();
}
...
/* CHECK: PARSE */
#else
    ICOV(166);DBG_PROG_AUTOCONTROL_PROCESS_ENTRY();
    ubVerifySectionCrcReturn = verifySectionCrc();
#endif

    ICOV(166);if (( PROG_FRAMEWORK_E_NOT_OK == ubVerifySectionCrcReturn)?BCOV(167):!BCOV(168))
    {
        eProgCompStatus = PROG_APPL_INVALID;
    }
    else
    ...
    ICOV(169);DBG_PROG_AUTOCONTROL_PROCESS_ENTRY();
    /* Check if the address is correct */
    eProgCompStatus = PROG_VerifySectionCrc();
#else
    ICOV(170);DBG_PROG_AUTOCONTROL_PROCESS_ENTRY();
#endif

    ICOV(170);m_stDiagMessageInfo.pubData[4] = (u8)(PROG_ROUTINE_SELFTERM | PROG_ROUTINE_PREDIF | PROG_ROUTINE_RESAVAILABLE); /* Positive response */
...
NCS_PRI_FCT tProgCompleteStatus PROG_CompleteCompatibleCheck (void)
{
    ...
    ICOV(171);eProgCompStatus = PROG_APPL_INVALID;
    /* Deviation MISRAC2012-2 */
    ...
    if (((PROG_START_COMP_BLK_HIGH == pulStartCompAddr[0]) && (PROG_START_COMP_BLK_LOW == pulStartCompAddr[1]))?BCOV(172):!BCOV(173))
    {
        /* Get the 64 bits of the end complete and compatible signature */
        /* Deviation MISRAC2012-2 */
        pulEndCompAddr = (u32*)*pulEndCompAddrTemp;

        if (((PROG_END_COMP_BLK_HIGH == pulEndCompAddr[0]) && (PROG_END_COMP_BLK_LOW == pulEndCompAddr[1]))?BCOV(174):!BCOV(175))
        {
...
NCS_PRI_FCT tProgStatus PROG_ReadCompErrorStatus(void)
{
    ...
    ICOV(176);if((m_ulCompTimeoutFLag == PROG_COMP_NO_ERROR)?BCOV(177):!BCOV(178))
    {
...
NCS_PRI_FCT void PROG_CompleteCompatibleInd (tProgCompTimeoutStatus eProgCompTimeoutStatus)
{
    ICOV(179);switch(eProgCompTimeoutStatus)
    {
        case PROG_COMPLETECOMPATIBLE_START:
            ICOV(180);m_ulCompTimeoutFLag = PROG_COMP_ERROR;
        ...
        case PROG_COMPLETECOMPATIBLE_END:
            ...
            ICOV(181);Watchdog_SetTimeoutValue(PROG_INIT_TIMEOUT_VALUE);
        ...
        default:
            ...
            ICOV(182);m_ulCompTimeoutFLag = PROG_COMP_NO_ERROR;
            break;
    }
}
...
/* CHECK: PARSE */
#else
    ICOV(183);DBG_PROG_MEMSTARTERASING_ENTRY(ubBlockId,ErrorCode);
    /*Call InvalidateSection API in framework before erasing the section*/
    ubInvalidateSectionReturn = invalidateSection((uint32)ulStartAddress);
#endif
    ICOV(183);if ((PROG_FRAMEWORK_E_OK == ubInvalidateSectionReturn)?BCOV(184):!BCOV(185))
    {
        eProgStatus = PROG_E_OK;
    }
    else
...
/* #endif (PROG_IMPL_VARIANT == PROG_IMPL_2)*/
    ...
    ICOV(186);DBG_PROG_MEMSTARTERASING_ENTRY(ubBlockId,ErrorCode);
    ...
    ICOV(187);DBG_PROG_MEMSTARTERASING_ENTRY(ubBlockId,ErrorCode);
    /* Call applicative callback */
    ...
    ICOV(188);DBG_PROG_MEMSTARTERASING_ENTRY(ubBlockId,ErrorCode);
    /* Call applicative callback */
    ...
    ICOV(189);DBG_PROG_MEMSTARTERASING_ENTRY(ubBlockId,ErrorCode);
    /*Call InvalidateSection API  before erasing the section*/
    eProgStatus = PROG_InvalidateSection(ubBlockId);
    if ((PROG_E_OK != eProgStatus)?BCOV(190):!BCOV(191))
    {
    ...
    ICOV(192);DBG_PROG_MEMSTARTERASING_ENTRY(ubBlockId,ErrorCode);
    /*Call InvalidateSection API  before erasing the section*/
    ...
    if ((PROG_E_OK != eProgStatus)?BCOV(193):!BCOV(194))
    {
        *ErrorCode = UDS_NRC_72;
    }
#else
    ICOV(195);DBG_PROG_MEMSTARTERASING_ENTRY(ubBlockId,ErrorCode);
#endif /* #if (PROG_IMPL_VARIANT == PROG_IMPL_2)   */
    ICOV(195);DBG_PROG_MEMSTARTERASING_EXIT(eProgStatus);
    return eProgStatus;
}
...
tProgStatus PROG_InvalidateSection_BlockID(u8 ubBlockId)
{
    ...
    ICOV(196);ulAddress = ((stConfigSegment[stCfgBlock[ubBlockId].ubFirstSegmentId + stCfgBlock[ubBlockId].ubSegmentNbr - 1u].ulEraseEndAddress
    ...
    if ((eProgStatus == PROG_E_OK)?BCOV(197):!BCOV(198))
    {

        for(ubIdx = 0U; (ubIdx < PROG_MAGIC_NUM_SIZE)?BCOV(199):!BCOV(200); ubIdx++)
        {
        ...
        u32 ulEraseLength, tUdsStatus * ErrorCode)
{
    ...
    ICOV(201);DBG_PROG_INVALIDATESECTION_ENTRY(ulStartAddress,ulEraseLength,ErrorCode);
    eProgStatus = PROG_GetSegmentByAddress(ulStartAddress, ulEraseLength, PROG_MEM_OPERATION_TYPE_ERASE, &iSegment);

    if ((eProgStatus == PROG_E_OK)?BCOV(202):!BCOV(203))
    {
        ...
        if ((eProgStatus == PROG_E_OK)?BCOV(204):!BCOV(205))
        {

            for(ubIdx = 0U; (ubIdx < PROG_MAGIC_NUM_SIZE)?BCOV(206):!BCOV(207); ubIdx++)
            {
                m_aubMagicNumValue[ubIdx] = m_aCfgMagicNumRevokedValue[ubIdx];
            }
    ...
    if ((eProgStatus != PROG_E_OK)?BCOV(208):!BCOV(209))
    {
        *ErrorCode = UDS_NRC_72;
    }
...
tProgStatus PROG_Check_Prg_Dep_Check(void)
{
    ...
    ICOV(210);DBG_PROG_CHECK_PRG_DEP_CHECK_ENTRY();
#if !((PROG_IMPL_VARIANT == PROG_IMPL_90) || (PROG_IMPL_VARIANT == PROG_IMPL_91))
    eProgBoolean = PROG_CheckValidAppl();
#endif

#if !((PROG_IMPL_VARIANT == PROG_IMPL_90) || (PROG_IMPL_VARIANT == PROG_IMPL_91))
    if ((PROG_TRUE == eProgBoolean)?BCOV(211):!BCOV(212))
    {
#endif
    ...
    eProgStatus = PROG_GetSegmentByAddress(ulAddress, 1U, PROG_MEM_OPERATION_TYPE_READ, &ubLocalSegmentId);
        if((eProgStatus == PROG_E_OK)?BCOV(213):!BCOV(214))
        {
...
#endif
                if((eProgStatus == PROG_E_OK)?BCOV(215):!BCOV(216))
                {
...
void PROG_Entry_CheckDependenciesFinish(void)
{
    ...
    ICOV(217);DBG_PROG_ENTRY_CHECKDEPENDENCIESFINISH_ENTRY();
#if((PROG_CRC_CALCULATION != PROG_NO_CRC) || ((PROG_IMPL_VARIANT == PROG_IMPL_3) || (PROG_IMPL_VARIANT == PROG_IMPL_4)||\
    (PROG_IMPL_VARIANT == PROG_IMPL_5))||((PROG_IMPL_VARIANT == PROG_IMPL_90) || (PROG_IMPL_VARIANT == PROG_IMPL_91)))
...
void PROG_Do_CheckPrgDependencies(void)
{
    ...
    tProgStatus eProgStatus;
    ICOV(218);eProgStatus = PROG_CRC();

    DBG_PROG_DO_CHECKPRGDEPENDENCIES_ENTRY();

    if ((PROG_E_OK == eProgStatus)?BCOV(219):!BCOV(220))
    {
    ...
    else if ((PROG_E_NOT_OK == eProgStatus)?BCOV(221):!BCOV(222))
    {
...
/* CHECK: PARSE */
#else
    ICOV(223);DBG_PROG_CHECKVALIDAPPL_ENTRY();
    ubisValidApplicationRes = isValidApplication();
#endif

    ICOV(223);if ((PROG_FRAMEWORK_E_OK != ubisValidApplicationRes)?BCOV(224):!BCOV(225))
    {
    ...
    ICOV(226);DBG_PROG_CHECKVALIDAPPL_ENTRY();
    ...
    ICOV(227);DBG_PROG_CHECKVALIDAPPL_ENTRY();

    ulisValidApplicationRes = PROG_CompleteCompatibleCheck();

    if((PROG_APPL_VALID != ulisValidApplicationRes)?BCOV(228):!BCOV(229))
    {
        eProgCompStatus = PROG_FALSE;
    }
    else
...
#else
    ICOV(230);DBG_PROG_CHECKVALIDAPPL_ENTRY();
    eProgCompStatus = PROG_TRUE;
#endif /* (PROG_IMPL_VARIANT == PROG_IMPL_20) */
    ICOV(230);DBG_PROG_CHECKVALIDAPPL_EXIT(eProgCompStatus);
    return eProgCompStatus;
}
...
tProgBoolean PROG_IsValidApplication(void)
{
    ...
    ICOV(231);DBG_PROG_ISVALIDAPPLICATION_ENTRY();
    eProgBoolean = PROG_TRUE;
#if (PROG_ERASE_TYPE != PROG_ERASE_BY_BLOCK_ID)
    for(ubIdx = 0U; (((ubIdx < PROG_SEGMENT_NB) && (eProgBoolean == PROG_TRUE)))?BCOV(232):!BCOV(233); ubIdx++)
#else
    for(ubIdx = 0U; (((ubIdx < PROG_BLOCK_NB) && (eProgBoolean == PROG_TRUE)))?BCOV(234):!BCOV(235); ubIdx++)
#endif
    {
        if (( stConfigSegment[ubIdx].eValidityCheck == PROG_TRUE)?BCOV(236):!BCOV(237))
        {
#if (PROG_ERASE_TYPE != PROG_ERASE_BY_BLOCK_ID)
            /* Retrieve address of the Invalid flag */
            ...
            if (( eProgStatus == PROG_E_OK)?BCOV(238):!BCOV(239))
            {
                for(uwLoopIndex = 0U; (uwLoopIndex < PROG_MAGIC_NUM_SIZE)?BCOV(240):!BCOV(241); uwLoopIndex++)
                {
                    if((aubValidyValue[uwLoopIndex] != m_astCfgMemory[stConfigSegment[ubIdx].ubMemoryIdx].ubEraseValue)?BCOV(242):!BCOV(243))
                    {
            ...
            if (((eProgStatus == PROG_E_OK) && (eProgBoolean == PROG_TRUE))?BCOV(244):!BCOV(245))
            {
#if (PROG_ERASE_TYPE != PROG_ERASE_BY_BLOCK_ID)
                ...
                if ((PROG_E_OK == eProgStatus)?BCOV(246):!BCOV(247))
                {
                    for (uwLoopIndex = 0U; (uwLoopIndex < PROG_MAGIC_NUM_SIZE)?BCOV(248):!BCOV(249); uwLoopIndex++)
                    {
                        if((aubValidyValue[uwLoopIndex] != m_aCfgMagicNumProgrammedValue[uwLoopIndex])?BCOV(250):!BCOV(251))
                        {
                            eProgBoolean = PROG_FALSE;
                        }
                    }
...
tProgStatus PROG_CustomSetDownloadVerificationSuccess(u8 ubLogicalBlockId, u8 ubLogicalSegmentId, tProgBoolean ubCompareSuccess)
{
    ...
    ICOV(252);OSC_PARAM_UNUSED(ubLogicalBlockId);
...
#else
    /* Retrieve address of the validity flag */
    ICOV(253);ulAddress = ((stConfigSegment[stCfgBlock[ubLogicalBlockId].ubFirstSegmentId + stCfgBlock[ubLogicalBlockId].ubSegmentNbr - 1u].ulEraseEndAddress
...
#if (PROG_OTA_DUALBANK_USED == STD_ON)
    ICOV(253);ulAddress = PROG_CustomCalcInactiveBankWriteAddr(ulAddress);
#endif /*(PROG_OTA_DUALBANK_USED == STD_ON)*/


    ICOV(253);if ((PROG_TRUE == ubCompareSuccess)?BCOV(254):!BCOV(255))
    {
        for (ubIdx = 0U; (ubIdx < PROG_MAGIC_NUM_SIZE)?BCOV(256):!BCOV(257); ubIdx++)
        {
    ...
    {
        for(ubIdx = 0U; (ubIdx < PROG_MAGIC_NUM_SIZE)?BCOV(258):!BCOV(259); ubIdx++)
        {
            m_aubMagicNumValue[ubIdx] = m_aCfgMagicNumRevokedValue[ubIdx];
        }
    }
    ...
    if ((PROG_E_BUSY == eProgStatus)?BCOV(260):!BCOV(261))
    {
        eProgStatus = PROG_E_OK;
    }
#endif
...
tProgBoolean PROG_CheckProgrammingRequest(void)
{
    ...
    ICOV(262);DBG_PROG_CHECKPROGRAMMINGREQUEST_ENTRY();
    ...
    if ((PROG_TRUE == eStatus)?BCOV(263):!BCOV(264))
    {
#if ((PROG_IMPL_VARIANT != PROG_IMPL_2) && (PROG_IMPL_VARIANT != PROG_IMPL_4))
        eStatus = PROG_CheckProgRequest();

        if ((PROG_BOOT_REPROG == eStatus)?BCOV(265):!BCOV(266))
...
void PROG_SwitchApplicationMode(void)
{
    ...
    ICOV(267);DBG_PROG_SWITCHAPPLICATIONMODE_ENTRY();
...
void PROG_Entry_ChecksumByRange(void)
{
    ...
    ICOV(268);DBG_PROG_ENTRY_CHECKSUMBYRANGE_ENTRY();
...
tProgStatus PROG_RangeChecksumFinish(void)
{
    ...
    ICOV(273);DBG_PROG_RANGECHECKSUMFINISH_ENTRY();
    /*call custom checksum calculation callback*/
    eProgStatus = PROG_CustomChecksumCalc(&uwCalculatedCrc);
    if((PROG_E_OK == eProgStatus)?BCOV(274):!BCOV(275))
    {
        if((m_uwExpectedCks == uwCalculatedCrc)?BCOV(276):!BCOV(277))
        {
    ...
    else if((PROG_E_BUSY == eProgStatus)?BCOV(278):!BCOV(279))
    {
...
u32 PROG_RetrieveRequestData32(u8 * pubBufferIn)
{
    ...
    /* Initialization */
    ICOV(281);ulData = 0x00000000;
    uwEndIndex = 3U;
    ubShift = 3U;

    /* Copy each byte of the buffer */
    for (uwIdx = 0; (uwIdx <= uwEndIndex)?BCOV(282):!BCOV(283); uwIdx++)
    {
...
u16 PROG_RetrieveRequestData16(u8 * pubBufferIn)
{
    ...
    /* Initialization */
    ICOV(284);uwData = 0x0000;
    uwEndIndex = 1U;
    ubShift = 1U;

    /* Copy each byte of the buffer */
    for (uwIdx = 0; (uwIdx <= uwEndIndex)?BCOV(285):!BCOV(286); uwIdx++)
    {
...
{
    tUdsStatus eStatus;
    ICOV(287);DBG_PROG_REQUESTSEED_ENTRY(pulLen,aubUdsData );
...
void PROG_Entry_PartialVerificationCrc(void)
{
    ...
    ICOV(288);m_ubActivePartialVerifyCrc = TRUE;
#if (PROG_CVN_CHECK == STD_ON)
    u8 ubIdx;
#endif /* PROG_CVN_CHECK == STD_ON */
    eProgStatus = PROG_E_OK;
    ...
    if ((ubTempBlockIdLen == PROG_PART_VERIF_BLOCKID_LEN_1BYTE)?BCOV(289):!BCOV(290))
    {
        uwBlockIdentifier = (u16) m_stDiagMessageInfo.pubData[PROG_RC_PART_VERIF_BLOCKID_BASE_INDEX];
    }
    else if ((ubTempBlockIdLen == PROG_PART_VERIF_BLOCKID_LEN_2BYTES)?BCOV(291):!BCOV(292))
    {
    ...
    if ((PROG_E_OK == eProgStatus)?BCOV(293):!BCOV(294))
    {
        /* Find the index table from block identifier */
        for (ubLoopIndex = 0U;
            ((ubLoopIndex < PROG_BLOCK_NB) && (PROG_E_NOT_OK == eIndexFound))?BCOV(295):!BCOV(296);
            ubLoopIndex++)
        {
            /* Find the corresponding index of block in the table */
            if ((stCfgBlock[ubLoopIndex].uwBlockIdentifier == uwBlockIdentifier)?BCOV(297):!BCOV(298))
            {
        ...
        if ((PROG_E_OK == eIndexFound)?BCOV(299):!BCOV(300))
        {
            /* Get the stored data about the programmed segments */
            PROG_CustomGetPartProgSegList(ubBlockId, aulSegProgSize, &ubSegNbr);

            if ((0U != ubSegNbr)?BCOV(301):!BCOV(302))
            {
                m_stSegList.ubSegNbr = ubSegNbr;
                for (ubSegIndex = 0; (ubSegIndex < m_stSegList.ubSegNbr)?BCOV(303):!BCOV(304); ubSegIndex++)
                {
...
*/
                ...
                if (((ubCrcCvnLength == PROG_CRC32_LENGTH) && (m_stDiagMessageInfo.ulLength == ((PduLengthType)PROG_PARTIAL_SW_ROUTINE_CRC_LEN + ubTempBlockIdLen - 1U)))?BCOV(305):!BCOV(306))
                {
...
/* CHECK: PARSE */
                    ...
                    /* Extract expected CRC */
                    if ((m_ulExpectedCks == 0U)?BCOV(307):!BCOV(308))
                    {
                ...
                else if (((ubCrcCvnLength == PROG_CVN_LENGTH) && (m_stDiagMessageInfo.ulLength == ((PduLengthType)PROG_PARTIAL_SW_ROUTINE_CVN_LEN + ubTempBlockIdLen - 1U)))?BCOV(309):!BCOV(310))
                {
                    /* Set CVN check is in progress */
                    m_eCvnCheckInProgress = PROG_TRUE;
                    for (ubIdx=0; (ubIdx < PROG_CVN_LENGTH)?BCOV(311):!BCOV(312); ubIdx++)
                    {
...
#if ((PROG_PARTIAL_VERIFY_CRC == STD_ON) && (PROG_IMPL_VARIANT == PROG_IMPL_40))
    /* Store active state */
    ICOV(313);m_stSegList.ubSegNbr = 0;
...
{
#if ((PROG_PARTIAL_VERIFY_CRC == STD_ON) && (PROG_IMPL_VARIANT == PROG_IMPL_40))
    ICOV(314);if ((m_stSegList.ubSegListCrcIndex < (m_stSegList.ubSegNbr - 1U))?BCOV(315):!BCOV(316))
    {
...
#if (PROG_CRC_CALCULATION != PROG_NO_CRC)
        if ((PROG_VERIFY_CRC == m_stDataProgVerification.ubAlgo)?BCOV(317):!BCOV(318))
        {
#if (PROG_CRC_CALCULATION == PROG_TD_CRC32)
...
tProgStatus PROG_CheckPartialSwCvnStatus(tProgStatus eProgStatus)
{
    ...
    ICOV(319);if ((eProgStatus != PROG_E_BUSY)?BCOV(320):!BCOV(321))
    {
        if ((eProgStatus == PROG_E_OK)?BCOV(322):!BCOV(323))
        {
...
void PROG_Entry_LogicalBlockHash(void)
{
    ...
    ICOV(324);DBG_PROG_ENTRY_LOGICALBLOCKHASH_ENTRY();

    /* Extract block ID */
    uwBlockIdentifier = (u16) m_stDiagMessageInfo.pubData[PROG_RC_LOGICAL_BLOCK_NUMBER_INDEX];

    /* Find the index table from block identifier */
    for (ubLoopIndex = 0U; ((ubLoopIndex < PROG_BLOCK_NB) && (PROG_E_NOT_OK == eIndexFound))?BCOV(325):!BCOV(326); ubLoopIndex++)
    {
        /* Find the corresponding index of block in the table */
        if ((stCfgBlock[ubLoopIndex].uwBlockIdentifier == uwBlockIdentifier)?BCOV(327):!BCOV(328))
        {
            /* Index has been found, parsing of index will stop here */
            eIndexFound = PROG_E_OK;
    ...
    if ((PROG_E_OK == eIndexFound)?BCOV(329):!BCOV(330))
    {
        /* Get the stored data about the programmed segments */
        PROG_CustomGetPartProgSegList(ubBlockId, aulSegProgSize, &ubSegNbr);

        if ((0U != ubSegNbr)?BCOV(331):!BCOV(332))
        {
            m_stSegList.ubSegNbr = ubSegNbr;

            for (ubSegIndex = 0; (ubSegIndex < m_stSegList.ubSegNbr)?BCOV(333):!BCOV(334); ubSegIndex++)
            {
                m_stSegList.astSeg[ubSegIndex].ulSize = aulSegProgSize[ubSegIndex];
                m_stSegList.astSeg[ubSegIndex].ulStartAddress = stConfigSegment[stCfgBlock[ubBlockId].ubFirstSegmentId + ubSegIndex].ulStartAddress;
        ...
        else
        {
            ...
            for (ubLoopIndex = 0; ((ubLoopIndex < PROG_DIGEST_LENGTH))?BCOV(335):!BCOV(336);  ubLoopIndex++)
            {
...
tProgStatus PROG_ComputeBlockHash(void)
{
    ...
    ICOV(337);DBG_PROG_COMPUTEBLOCKHASH_ENTRY();
    ...
    if (((PROG_HASH_START != eCryState) && (PROG_HASH_UPDATE != eCryState) && (PROG_HASH_FINISH != eCryState))?BCOV(338):!BCOV(339))
#endif /*(PROG_CRY_PRESENT == STD_ON)*/
    {
        /* Is there still data to use for programming verification */
        if ((m_stDataProgVerification.ulCnt < ulWriteSizeOfData)?BCOV(340):!BCOV(341))
        {
            ...
            if ((0U == m_stDataProgVerification.ulCnt)?BCOV(342):!BCOV(343))
            {
#if PROG_COMPUTE_HASH_WITH_PHYS_ADDR == STD_ON
                /* Apply offset on address */
                if ((PROG_MEM_TYPE_FLASH == m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType)?BCOV(344):!BCOV(345))
                {
                    /* Internal Flash */
                    ulAddr = (u32)(m_stDataProgVerification.ulAdd + PROG_FLASH_ADDR_OFFSET);
                }
#if (PROG_FLASH_EXT_MEMORY_USE == PROG_TRUE)
                else if ((PROG_MEM_TYPE_FLASH_EXT == m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType)?BCOV(346):!BCOV(347))
                {
                    /* external Flash */
                ...
                else if ((PROG_MEM_TYPE_CUSTOM == m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType)?BCOV(348):!BCOV(349))
                {
                    /* Custom Memory */
            ...
            if (((ulWriteSizeOfData - m_stDataProgVerification.ulCnt) >= (PROG_CRC_CALC_SIZE - (u32) ubAddressShift))?BCOV(350):!BCOV(351))
            {
            ...
            if ((PROG_E_NOT_OK != eProgStatus)?BCOV(352):!BCOV(353))
            {
...
void PROG_LogicalBlockHashFinish(void)
{
    ...
    ICOV(354);DBG_PROG_LOGICALBLOCKHASHFINISH_ENTRY();
   ...
   eProgStatus = PROG_GetCryptoStatus();
    ...
    if ((PROG_E_OK == eProgStatus)?BCOV(355):!BCOV(356))
    {
        for (ubLoopIndex = 0; (((ubLoopIndex < PROG_DIGEST_LENGTH) && (PROG_E_OK == eProgStatus)))?BCOV(357):!BCOV(358);  ubLoopIndex++)
        {
            /* store hash */
