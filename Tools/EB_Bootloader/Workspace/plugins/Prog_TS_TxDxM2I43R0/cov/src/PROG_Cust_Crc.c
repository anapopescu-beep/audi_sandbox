...
void PROG_CalcCrc16(const u8 * aubCrcData, u32 ulReadLength, u16 * uwCrcValue)
{
    u32 ulLengthIdx;
    u16 uwTabIndex;
    ICOV(359);DBG_PROG_CALCCRC16_ENTRY(aubCrcData, ulReadLength, uwCrcValue);

    for (ulLengthIdx = 0U; (ulLengthIdx < ulReadLength)?BCOV(360):!BCOV(361); ulLengthIdx++)
    {
        /* XOR(Crc,data byte) */
...
void PROG_CalcCrc32(const u8 * aubCrcData, u32 ulReadLength, u32 * ulCrcValue)
{
    u32 ulLengthIdx;
    u32 ulTabIndex;
    ICOV(362);DBG_PROG_CALCCRC32_ENTRY(aubCrcData, ulReadLength, ulCrcValue);

    if((m_ubCrcStart != PROG_TRUE)?BCOV(363):!BCOV(364))
    {
        *ulCrcValue = *ulCrcValue ^ 0xFFFFFFFFU;
    }

    for (ulLengthIdx = 0U; (ulLengthIdx < ulReadLength)?BCOV(365):!BCOV(366); ulLengthIdx++)
    {
        /* XOR(Crc32,data byte) */
    ...
    *ulCrcValue = *ulCrcValue ^ 0xFFFFFFFFU;
    ...
    if((m_ubCrcStart == PROG_TRUE)?BCOV(367):!BCOV(368))
    {
