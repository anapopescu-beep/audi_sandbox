...
{
    ICOV(418);DBG_PROG_ENTRY_ACTIVESBL_ENTRY();
#if PROG_ACTIVE_SBL_ENABLE == STD_ON
    /*Send NRC78 response*/
    UDS_StopNRC78Timer();
#endif /* PROG_ACTIVE_SBL_ENABLE == STD_ON */
...
void PROG_JumpToSBL(void)
{
    ...
    ICOV(419);DBG_PROG_JUMPTOSBL_ENTRY();
...
#else
    ICOV(420);DBG_PROG_JUMPTOSBL_ENTRY();
#endif /* PROG_ACTIVE_SBL_ENABLE == STD_ON */

    ICOV(420);DBG_PROG_JUMPTOSBL_EXIT();
}
/*---------------------------{End PROG_JumpToSBL}-----------------------------*/
...
{
    ICOV(421);DBG_PROG_ENTRY_SBLSYNCH_ENTRY();
#if PROG_ACTIVE_SBL_RESPONSE_ENABLE == STD_ON
...
tProgStatus PROG_ActiveSBL_Check(void)
{
    ...
    ICOV(423);DBG_PROG_ACTIVESBL_CHECK_ENTRY();
    ...
    if ((PROG_E_NOT_OK == eProgStatus)?BCOV(424):!BCOV(425))
    {
    ...
    {
      if((PROG_E_NOT_OK == m_ubSblVerficationStatus)?BCOV(426):!BCOV(427))
      {
        m_stDiagMessageInfo.pubData[4] = (u8)(PROG_ROUTINE_FINISHED_NOK|PROG_ROUTINE_TYPE_1);
        UDS_LongRequestEnd(5U, m_stDiagMessageInfo.pubTxData, UDS_ACK);
...
NCS_PRI_FCT tProgStatus PROG_GetSBLInfo(void)
{
    ...
    ICOV(428);pulSBLValidityInfo = (u32 volatile const *)(&m_stSblHeaderAddr.ulSBLValidityFlagAddr);

    /* Check if the address to check the SBL validity flag is not NULL_PTR */
    if((*pulSBLValidityInfo != 0x00000000U)?BCOV(429):!BCOV(430))
    {
        /* Deviation MISRAC2012-2 */
        pulSBLValidityFlag = (u32*)*pulSBLValidityInfo;
        /* Check if the validity flag is set */
        if((*pulSBLValidityFlag == PROG_SBL_VALIDITY_FLAG)?BCOV(431):!BCOV(432))
        {
#if (PROG_IMPL_VARIANT == PROG_IMPL_1)
...
NCS_PRI_FCT tProgStatus PROG_CheckSBLCompatibility(const u8 *pubRefStringPBL)
{
    ...
    ICOV(433);eProgStatus = PROG_E_OK;

    /* Check µc compatibility */
    if ((m_stSblHeaderAddr.referenceString[0] != pubRefStringPBL[0])?BCOV(434):!BCOV(435))
    {
        eProgStatus = PROG_E_NOT_OK;
    }

    /* Check HW version compatibility */
    if ((m_stSblHeaderAddr.referenceString[1] != pubRefStringPBL[1])?BCOV(436):!BCOV(437))
    {
        eProgStatus = PROG_E_NOT_OK;
    }

    /* Check application compatibility */
    if ((m_stSblHeaderAddr.referenceString[2] != pubRefStringPBL[2])?BCOV(438):!BCOV(439))
    {
        eProgStatus = PROG_E_NOT_OK;
    }

    /* Check customer compatibility */
    if ((m_stSblHeaderAddr.referenceString[3] != pubRefStringPBL[3])?BCOV(440):!BCOV(441))
    {
        eProgStatus = PROG_E_NOT_OK;
    }

    /* Check OEM-ID compatibility */
    if ((m_stSblHeaderAddr.referenceString[4] != pubRefStringPBL[4])?BCOV(442):!BCOV(443))
    {
        eProgStatus = PROG_E_NOT_OK;
    }

    /* Check project number compatibility */
    if ((m_stSblHeaderAddr.referenceString[5] != pubRefStringPBL[5])?BCOV(444):!BCOV(445))
    {
        eProgStatus = PROG_E_NOT_OK;
    }
