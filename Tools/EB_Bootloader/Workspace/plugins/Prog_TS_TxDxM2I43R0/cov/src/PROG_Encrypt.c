...
tProgStatus PROG_GetDecryptionStatus(void)
{
    ...
    ICOV(530);DBG_PROG_GETDECRYPTIONSTATUS_ENTRY( );
...
tProgCryState PROG_GetDecryptionState(void)
{
    ...
    ICOV(531);DBG_PROG_GETDECRYPTIONSTATE_ENTRY( );
...
Std_ReturnType PROG_CsmEncryptDecryptNotification(Csm_ReturnType eResult)
{
    ...
    ICOV(532);DBG_PROG_CSMENCRYPTDECRYPTNOTIFICATION_ENTRY(eResult);

    if ((CSM_E_OK == eResult)?BCOV(533):!BCOV(534))
    {
        switch (m_eEncryptDecryptState)
        {
            ...
            case PROG_DECRYPTION_FINISH:
                ICOV(535);PROG_CsmDecryptionNotification();
                break;
            /* Shall not happened go back in stable state happened */
            default:
                ICOV(536);m_eEncryptDecryptStatus = PROG_E_NOT_OK;
...
void PROG_GetSymDecryptionKey(Csm_SymKeyType* pstDecryptionKey)
{
    ...
    ICOV(537);DBG_PROG_GETSYMDECRYPTIONKEY_ENTRY(pstDecryptionKey);
    PROG_CustomGetSymDecryptionKey(&paubKeyData, &ulDecryptionKeyLength);

    pstDecryptionKey->length = ulDecryptionKeyLength;
#if (PROG_CSMASR43_USED ==STD_ON)
    for(ulIdx = 0U; /* key */
        (ulIdx < ulDecryptionKeyLength)?BCOV(538):!BCOV(539);
        ulIdx++)
    {
        pstDecryptionKey->data[ulIdx] = paubKeyData[ulIdx];
    }
#else
    for (ulIdx = 0U; (ulIdx < (ulDecryptionKeyLength/sizeof(u32)))?BCOV(540):!BCOV(541); ulIdx++)
    {
...
void PROG_DecryptionStart(void)
{
    ...
    ICOV(542);DBG_PROG_DECRYPTIONSTART_ENTRY();
    ...
    if ((eCsmStatus == CSM_E_OK)?BCOV(543):!BCOV(544))
    {
    ...
    if((eCsmStatus == CSM_E_OK)?BCOV(545):!BCOV(546))
    {
#if (PROG_CSMASR43_USED ==STD_ON)
    ...
    else if((eCsmStatus == CRYPTO_E_JOB_CANCELED)?BCOV(547):!BCOV(548))
    {
...
void PROG_DecryptionUpdate(const u8 * pubCipherText , u32 ulCipherTextLength, u8 * pubPlainText , u32 * pulPlainTextLength)
{
    ...
    ICOV(549);DBG_PROG_DECRYPTIONUPDATE_ENTRY(pubCipherText, ulCipherTextLength, pubPlainText, pulPlainTextLength);
    if ((m_eEncryptDecryptStatus != PROG_E_NOT_OK)?BCOV(550):!BCOV(551))
    {
...
void PROG_DecryptionFinish(u8 * pubplainText , u32 * pulPlainTextLength)
{
    ...
    ICOV(552);DBG_PROG_DECRYPTIONFINISH_ENTRY(pubplainText, pulPlainTextLength);

    if((m_eEncryptDecryptStatus != PROG_E_NOT_OK)?BCOV(553):!BCOV(554))
    {
...
NCS_PRI_FCT void PROG_CsmDecryptionNotification(void)
{
    ICOV(555);DBG_PROG_CSMDECRYPTIONNOTIFICATION_ENTRY();

    switch (m_eEncryptDecryptState)
    {
        case PROG_DECRYPTION_START:
          ICOV(556);m_eEncryptDecryptState  = PROG_DECRYPTION_START_END;
          m_eEncryptDecryptStatus = PROG_E_OK;
          break;

        case PROG_DECRYPTION_UPDATE:
          ICOV(557);m_eEncryptDecryptState  = PROG_DECRYPTION_UPDATE_END;
          m_eEncryptDecryptStatus = PROG_E_OK;
          break;

        case PROG_DECRYPTION_FINISH:
          ICOV(558);m_eEncryptDecryptState  = PROG_DECRYPTION_FINISH_END;
          m_eEncryptDecryptStatus = PROG_E_OK;
          break;
/* CHECK: NOPARSE */
/* NOCOV-EMPTYDEFAULT */
...
{
    ICOV(559);DBG_PROG_COMPUTETOTALENCRYPTEDDATAWITHAESCBCPKCS7_ENTRY(ulTotalPlainDataSize, pulTotalCipheredDataSize);
...
{
    ICOV(560);if ((eCsmStatus == CSM_E_OK)?BCOV(561):!BCOV(562))
    {
        m_eEncryptDecryptStatus = PROG_E_BUSY;
#if (PROG_CSMASR43_USED ==STD_ON)
        /* Check if the callback for decryption received from the Crypto 4.3 stack*/
        if((PROG_E_NOT_BUSY == (m_eEncryptDecryptState & PROG_E_NOT_BUSY))?BCOV(563):!BCOV(564))
        {
...
#if(PROG_CSM_CANCELJOB_ENABLED == STD_ON)
    else if ((eCsmStatus == CRYPTO_E_JOB_CANCELED)?BCOV(565):!BCOV(566))
    {
        if((m_eEncryptDecryptState == PROG_DECRYPTION_START_DONE)?BCOV(567):!BCOV(568))
        {
...
#endif
    else if ((eCsmStatus == CSM_E_BUSY)?BCOV(569):!BCOV(570))
    {
        /* Nothing to do */
    }
    else
