...
void PROG_Init_Impl10(void)
{
  ...
  ICOV(617);for(ubIdx = 0; (ubIdx < PROG_MAX_RD_PER_BLOCK)?BCOV(618):!BCOV(619); ubIdx++)
  {
...
void PROG_Impl10_Entry_CheckMemoryFailed(void)
{
  ...
  ICOV(620);m_ubFailedCheckMemoryCount++;
  m_ubVerificationTablePresent = FALSE;
  ...
  m_ubSoftwarePartID = PROG_SOFTWARE_PART_FINISHED;
  if ((PROG_E_NOT_OK == eProgStatus)?BCOV(621):!BCOV(622))
  {
...
void PROG_Impl10_AdjustAddressForHash(u32*ulAddress)
{
  ...
  ICOV(623);PROG_GetStartAddressFromVBT(m_uwCurrentDataBlock , &ulStartAddress);
  /* The address in the RD is smaller than the one in the VBT, start the hash from the address mentioned in the VBT */
  if(((*ulAddress) < ulStartAddress)?BCOV(624):!BCOV(625))
  {
...
void PROG_Impl10_AdjustLengthForHash(u32*ulReadLength)
{
  ...
  ICOV(626);PROG_GetLengthFromVBT(m_uwCurrentDataBlock , &ulLength);
  /* Case to protect overflow of ulReadLength*/
  if((m_stDataProgVerification.ulCnt >= ulLength)?BCOV(627):!BCOV(628))
  {
    (*ulReadLength) = 0;
  }
  /* adjust length to match the size of the VBT */
  else if(( ((*ulReadLength) + m_stDataProgVerification.ulCnt) > ulLength)?BCOV(629):!BCOV(630))
  {
...
void PROG_Impl10_FinalizeHash(void)
{
  ...
  ICOV(631);PROG_GetLengthFromVBT(m_uwCurrentDataBlock , &ulLength);

  if((TRUE ==  m_ubVerificationTablePresent)?BCOV(632):!BCOV(633))
  {
    /* No more data to be hashed  */
    if((m_stDataProgVerification.ulCnt >= ulLength)?BCOV(634):!BCOV(635))
    {
    ...
    else
    {
      ...
      if((PROG_CRC_CALC_SIZE <= m_ulRemainingUnwrittenData)?BCOV(636):!BCOV(637))
      {
...
void PROG_Impl10_Do_HashMoreUnwrittenData(void)
{
  ...
  ICOV(638);PROG_GetLengthFromVBT(m_uwCurrentDataBlock , &ulLength);

  eCryptoState = PROG_GetCryptoState();
  eCryptoStatus = PROG_GetCryptoStatus();

  if(((PROG_E_OK == eCryptoStatus) &&  (PROG_HASH_END == eCryptoState))?BCOV(639):!BCOV(640))
  {
    /* No more data to be hashed  */
    if((m_stDataProgVerification.ulCnt >= ulLength)?BCOV(641):!BCOV(642))
    {
      PROG_ComputeHashFinish(&m_aubComputedDigest[0]);
    }
    /* Hash more data (data which are not sent in TDs) */
    else
    {
      ...
      if((PROG_CRC_CALC_SIZE <= m_ulRemainingUnwrittenData)?BCOV(643):!BCOV(644))
      {
        ulReadLength = PROG_CRC_CALC_SIZE;
      }
      else
...
void PROG_Impl10_CompareDataBlockHash(void)
{
  ...
  ICOV(645);PROG_GetDigestFromVBT(m_uwCurrentDataBlock , &aubDigest[0]);

  if((TRUE ==  m_ubVerificationTablePresent)?BCOV(646):!BCOV(647))
  {
      for (ubLoopIndex = 0; (((ubLoopIndex < PROG_DIGEST_LENGTH)&&(PROG_E_OK == ubDataBlockStatus)))?BCOV(648):!BCOV(649); ubLoopIndex++)
      {
           /* compare hash */
           if ((m_aubComputedDigest[ubLoopIndex] != aubDigest[ubLoopIndex])?BCOV(650):!BCOV(651))
           {
  ...
  else
  {
...
#if (PROG_COMPRESSION_ENABLE == STD_ON)
    if ((TRUE == m_stReprogInfo.ubCompRequired)?BCOV(652):!BCOV(653))
    {
    ...
    for(ubIdx = 0; (ubIdx < PROG_MAX_RD_PER_BLOCK)?BCOV(654):!BCOV(655); ubIdx++)
    {
      m_aubHashComparisonStatus[ubIdx] = PROG_E_NOT_OK;
    }
  }
...
void PROG_Impl10_CheckDataBlocksResult(void)
{
   ...
   ICOV(656);ubProgStatus = PROG_VerifyVBTFormatAndLength();

   /* Length and format are ok */
   if((PROG_E_OK == ubProgStatus)?BCOV(657):!BCOV(658))
   {
   ...
   if((PROG_E_OK == ubProgStatus)?BCOV(659):!BCOV(660))
   {
...
#if (PROG_IMPL_VARIANT == PROG_IMPL_11)
   if((PROG_E_OK == ubProgStatus)?BCOV(661):!BCOV(662))
   {
...
void PROG_Impl10_GenerateMac(void)
{
  ...
  ICOV(669);PROG_SecurityComputeAppChecksum();
#else
  /* Nothing to do in case of PBL, simulate crypto event finished */
  ICOV(670);PROG_HsmEmit (&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_FINISHED);
#endif
ICOV(670);}
...
#if (PROG_IMPL_VARIANT == PROG_IMPL_10)
  ICOV(671);if((ulAddress == PROG_SBL_VBT_START_ADDRESS)?BCOV(672):!BCOV(673))
#else
   /* SBL variant */
   /* Address of VBT is get from ESS */
   ICOV(674);(void)PROG_GetVBTAddress(&ulVBTAddress);
   if((ulAddress == ulVBTAddress)?BCOV(675):!BCOV(676))
#endif

  {
    ICOV(677);eProgBoolean = TRUE;
...
{
  ICOV(678);return m_ubVerificationTablePresent;
}
...
void PROG_Impl10_Entry_SignatureCheck(void)
{
...
#if (PROG_IMPL_VARIANT == PROG_IMPL_10)
    ICOV(679);ulVBTAddress = PROG_SBL_VBT_START_ADDRESS;
#else
    /* SBL variant */
    /* Address of VBT is get from ESS */
    ICOV(680);(void)PROG_GetVBTAddress(&ulVBTAddress);
    /* in case of ESS assume one continuous block */
    if((0 == m_stCurrentSegment.ubLogicalBlockId)?BCOV(681):!BCOV(682))
    {
        uwDataBlockNbr = 1;
    }
    else
    ...
    ICOV(680);PROG_GetVBTLength(&ulVBTLength);

    if((m_ulVBTWrittenData == ulVBTLength)?BCOV(683):!BCOV(684))
    {
...
tProgStatus PROG_Impl10_ValidateDataBlockAddress(tProgAddressType ulAddress)
{
  ...
  ICOV(685);if((TRUE == m_ubVerificationTablePresent)?BCOV(686):!BCOV(687))
  {
    ...
    if((ulStartAddressInVBT < ulAddress)?BCOV(688):!BCOV(689))
    {
...
tProgStatus PROG_ExtractEraseParameters_Impl11(tSegmentType * pstSegment)
{
    ...
    ICOV(690);eProgStatus = PROG_E_OK;

    /* ESS Management */
    /* Identify if it is ESS block that shall be erased */
    if ((stConfigSegment[pstSegment->ubSegmentId].ePartitionType == PROG_ESS_PARTITION)?BCOV(691):!BCOV(692))
    {
        ...
        if((PROG_E_ERASED == eEraseStatus)?BCOV(693):!BCOV(694))
        {
    ...
    else
    {
        ...
        if ((PROG_E_OK == eESSStatus)?BCOV(695):!BCOV(696))
        {
            /* Find if memory range to erase is defined in ESS */
            eESSStatus = Prog_GetEssLogicalBlockNbr(&ubBlockNbr);
            if ((PROG_E_OK == eESSStatus)?BCOV(697):!BCOV(698))
            {
                ubBlockId = 0xFFU;
                for (ubLoopIndex = 0; (((ubLoopIndex < ubBlockNbr) && (0xFFU == ubBlockId)))?BCOV(699):!BCOV(700); ubLoopIndex++)
                {
                    eESSStatus = Prog_GetEssLogicalBlockStartAddr(ubLoopIndex, &ulBlockAddr);
                    if (((PROG_E_OK == eESSStatus) && (pstSegment->ulStartAddress >= ulBlockAddr))?BCOV(701):!BCOV(702))
                    {
                        eESSStatus = Prog_GetEssLogicalBlockLength(ubLoopIndex, &ulBlockLength);
                        if (((PROG_E_OK == eESSStatus) && ((pstSegment->ulStartAddress + pstSegment->ulSize) <= (ulBlockAddr + ulBlockLength)))?BCOV(703):!BCOV(704))
                        {
                ...
                if ((0xFFU == ubBlockId)?BCOV(705):!BCOV(706))
                {
                ...
                else
                {
                    ...
                    if((PROG_E_ERASED == eEraseStatus)?BCOV(707):!BCOV(708))
                    {
                        eProgStatus = PROG_E_BYPASS;
                    }
#endif /*(PROG_ERASE_CHECK_TYPE == PROG_BLOCK_ERASE_CHECK)*/
...
tProgStatus PROG_RD_Check_Impl11(u32 ulStartAddress, u32 ulMemorySize, u8 ubLocalSegmentId, u8 * pubBlockIndex)
{
    ...
    ICOV(709);if ((stConfigSegment[ubLocalSegmentId].ePartitionType == PROG_ESS_PARTITION)?BCOV(710):!BCOV(711))
    {
       /* Another software part is being downloaded, abort */
       if(((m_ubSoftwarePartID !=PROG_SOFTWARE_PART_FINISHED) && (m_ubSoftwarePartID !=0))?BCOV(712):!BCOV(713))
       {
         eProgStatus = PROG_E_NOT_OK;
       }
       else
    ...
    else
    {
      ...
      if(( ((ulStartAddress > m_ulLastWrittenAddress) && (TRUE ==  m_ubVerificationTablePresent)) || (FALSE ==  m_ubVerificationTablePresent) )?BCOV(714):!BCOV(715))
      {
        /* Not ESS memory */
        /* Check if ESS is present in memory and valid */
        eESSStatus = Prog_GetEssValidityStatus();
        if ((PROG_E_OK == eESSStatus)?BCOV(716):!BCOV(717))
        {
            /* Find if memory range to erase is defined in ESS */
            eESSStatus = Prog_GetEssLogicalBlockNbr(&ubBlockNbr);
            if ((PROG_E_OK == eESSStatus)?BCOV(718):!BCOV(719))
            {
                ubBlockId = 0xFFU;
                for (ubLoopIndex = 0; (((ubLoopIndex < ubBlockNbr) && (0xFFU == ubBlockId)))?BCOV(720):!BCOV(721); ubLoopIndex++)
                {
                    eESSStatus = Prog_GetEssLogicalBlockStartAddr(ubLoopIndex, &ulBlockAddr);
                    if (((PROG_E_OK == eESSStatus) && (ulStartAddress >= ulBlockAddr))?BCOV(722):!BCOV(723))
                    {
                        eESSStatus = Prog_GetEssLogicalBlockLength(ubLoopIndex, &ulBlockLength);
                        if (((PROG_E_OK == eESSStatus) && ((ulStartAddress + ulMemorySize) <= (ulBlockAddr + ulBlockLength)))?BCOV(724):!BCOV(725))
                        {
                          /* Another sofwtare part is being downloaded */
                          if(( (m_ubSoftwarePartID !=PROG_SOFTWARE_PART_FINISHED) && (m_ubSoftwarePartID != (ubLoopIndex + 1)) )?BCOV(726):!BCOV(727))
                          {
                            eProgStatus = PROG_E_NOT_OK;
                          }
                          else
                ...
                if ((0xFFU == ubBlockId)?BCOV(728):!BCOV(729))
                {
                    /* No matching block found in ESS, send negative response */
                    eProgStatus = PROG_E_NOT_OK;
                }
...
{
    tUdsStatus eStatus;
    ICOV(730);eStatus = PROG_GenericRequest(pulLen, aubUdsData,
...
{
    tUdsStatus eStatus;
    ICOV(731);if(((pulLen != NULL_PTR) && (aubUdsData != NULL_PTR))?BCOV(732):!BCOV(733))
    {
        if((TRUE == Prog_CustomIsProdKeyPresent())?BCOV(734):!BCOV(735))
        {
          eStatus = UDS_ACK;
...
{
  ICOV(736);if((TRUE == Prog_CustomIsProdKeyPresent())?BCOV(737):!BCOV(738))
  {
...
void PROG_Impl10_Do_CheckHashOfKey(void)
{
  ...
  ICOV(739);eCryptoState = PROG_GetCryptoState();
  eCryptoStatus = PROG_GetCryptoStatus();

  if((PROG_E_NOT_OK != eCryptoStatus)?BCOV(740):!BCOV(741))
  {
    if((PROG_HASH_END == eCryptoState)?BCOV(742):!BCOV(743))
    {
      PROG_ComputeHashFinish(&m_aubComputedDigest[0]);
    }
    else if((PROG_HASH_START_END == eCryptoState)?BCOV(744):!BCOV(745))
    {
...
void PROG_Impl10_Entry_WriteKeyFinished(void)
{
  ...
  ICOV(746);for (ubLoopIndex = 0; (((ubLoopIndex < PROG_DIGEST_LENGTH)&&(PROG_E_OK == ubHashStatus)))?BCOV(747):!BCOV(748); ubLoopIndex++)
  {
       /* compare hash */
       if ((m_aubComputedDigest[ubLoopIndex] != m_stDiagMessageInfo.pubData[PROG_PROD_KEY_HASH_INDEX + ubLoopIndex])?BCOV(749):!BCOV(750))
       {
         ubHashStatus = PROG_E_NOT_OK;
       }
  }

  if((PROG_E_OK == ubHashStatus)?BCOV(751):!BCOV(752))
  {
...
{
  ICOV(753);m_ubSoftwarePartID = PROG_SOFTWARE_PART_FINISHED;
}
#endif
...
tProgStatus PROG_Impl10_CheckMemoryAllowed(void)
{
    ...
    ICOV(754);if ((m_ubFailedCheckMemoryCount < PROG_CHECK_MEMORY_MAX_ATTEMPTS)?BCOV(755):!BCOV(756))
    {
    ...
    if ((PROG_E_NOT_OK == eProgStatus)?BCOV(757):!BCOV(758))
#endif
    {
        /* Send negative response NRC 24 */
        ICOV(759);UDS_LongRequestEnd(PROG_CHK_MEMORY_RESPONSE_SIZE, m_stDiagMessageInfo.pubTxData, UDS_NRC_24);
    }
...
NCS_PRI_VAR void PROG_GetVBTLength(u32* pulVBTLength)
{
    ...
    ICOV(760);*pulVBTLength = PROG_SBL_VBT_LENGTH;
#else
    /* VBT Length for ESS */
    ICOV(761);if ((0U == m_stCurrentSegment.ubLogicalBlockId)?BCOV(762):!BCOV(763))
    {
...
NCS_PRI_VAR void PROG_GetVBTAddress(u32 * pulVBTAddress)
{
  ...
  ICOV(764);*pulVBTAddress = PROG_SBL_VBT_START_ADDRESS;
#else
   /* SBL variant */
    ICOV(765);if ((0U == m_stCurrentSegment.ubLogicalBlockId)?BCOV(766):!BCOV(767))
    {
...
NCS_PRI_VAR void PROG_GetStartAddressFromVBT(u16 uwDataBlockIndex,u32*ulStartAddress)
{
  ...
  ICOV(768);PROG_GetVBTAddress(&ulVBTAddress);
...
NCS_PRI_VAR void PROG_GetLengthFromVBT(u16 uwDataBlockIndex,u32*ulLength)
{
  ...
  ICOV(769);PROG_GetVBTAddress(&ulVBTAddress);
...
NCS_PRI_VAR void PROG_GetDigestFromVBT(u16 uwDataBlockIndex,u8*aubDigest)
{
  ...
  ICOV(770);PROG_GetVBTAddress(&ulVBTAddress);
  ...
  /* Hash value */
  for(ubIdx = 0; (ubIdx <PROG_DIGEST_LENGTH)?BCOV(771):!BCOV(772); ubIdx++)
  {
...
#endif
{
...
#endif
#if (PROG_IMPL_VARIANT == PROG_IMPL_11)
    ICOV(773);ulNbPages = (ulSegmentStartAddr - m_ulBlankCheckLastAddress) / m_astCfgMemory[stConfigSegment[ubSegmentId].ubMemoryIdx].ulMinValueToWrite;
    for (ulPage = 0U; (((ulPage < ulNbPages) && (ubProgStatus == PROG_E_OK)))?BCOV(774):!BCOV(775); ulPage++)
    {
        ...
        if((ubProgStatus == PROG_E_NOT_OK)?BCOV(776):!BCOV(777))
        {
    ...
    ICOV(778);ulBlankSize = ulSegmentStartAddr - m_ulBlankCheckLastAddress;
    ubBlankValue = PROG_SBL_BLANK_VALUE;
    /* Fill the table to be written for the blank space */
    for(ulLoop = 0U; (ulLoop < ulBlankSize)?BCOV(779):!BCOV(780); ulLoop++)
    {
        /* Write in RAM */
...
NCS_PRI_VAR tProgStatus PROG_CheckNonProgrammedAreas(void)
{
    ...
    ICOV(781);if((0 == m_stCurrentSegment.ubLogicalBlockId)?BCOV(782):!BCOV(783))
    {
      uwDataBlockNbr = 1;
    }
    else
    ...
    /* Plus VBT*/
    for (uwIdx = 0U; (uwIdx < (uwDataBlockNbr + 1))?BCOV(784):!BCOV(785); uwIdx++)
#else
    ICOV(786);for (uwIdx = 0U; (uwIdx < stCfgBlock[m_stCurrentSegment.ubLogicalBlockId].ubSegmentNbr)?BCOV(787):!BCOV(788); uwIdx++)
#endif
    {
        ICOV(789);ubSegmentIsNotProgrammed = PROG_TRUE;
        for (ubLoop = 0U; (((ubLoop < m_stSegList.ubSegNbr) && (ubSegmentIsNotProgrammed == PROG_TRUE)))?BCOV(790):!BCOV(791); ubLoop++)
        {
            if ((uwIdx == m_stSegList.astSeg[ubLoop].ubSegmentId)?BCOV(792):!BCOV(793))
            {
                ubSegmentIsNotProgrammed = PROG_FALSE;
            }
        }
        if ((PROG_TRUE == ubSegmentIsNotProgrammed)?BCOV(794):!BCOV(795))
        {
#if (PROG_IMPL_VARIANT == PROG_IMPL_11)
   ...
   /* Blank check on all non-programmed areas */
    ...
    for (ubIdx = 0U; (((ubIdx < m_stSegList.ubSegNbr) && (m_ubCheckMemoryStatus != PROG_BLANK_CHECK_FAILED)))?BCOV(796):!BCOV(797); ubIdx++)
    {
    ...
    if (((m_ulBlankCheckLastAddress < stConfigSegment[m_stCurrentSegment.ubSegmentId].ulEraseEndAddress)  && (m_ubCheckMemoryStatus != PROG_BLANK_CHECK_FAILED))?BCOV(798):!BCOV(799))
    {
    ...
    for (ubIdx = 0U; (((ubIdx < m_stSegList.ubSegNbr) && (m_ubCheckMemoryStatus != PROG_BLANK_CHECK_FAILED)))?BCOV(800):!BCOV(801); ubIdx++)
    {
    ...
    if ((m_ubCheckMemoryStatus == PROG_BLANK_CHECK_FAILED)?BCOV(802):!BCOV(803))
    {
...
NCS_PRI_VAR tProgStatus PROG_CheckESSCoverageOfProgrammableMemory(void)
{
    ...
    ICOV(804);eESSStatus = Prog_GetEssLogicalBlockNbr(&ubBlockNbr);

    if ((PROG_E_OK == eESSStatus)?BCOV(805):!BCOV(806))
    {
        /* Check that all configured segments are included in ESS */
        for (uwIdx = 1U; (((uwIdx < PROG_SEGMENT_NB) && (PROG_TRUE == ubAllBlocksFound)))?BCOV(807):!BCOV(808); uwIdx++) /* ESS is segment 0*/
        {
            ubBlockFound = PROG_FALSE;
            for (ubLoopIndex = 0U; (((ubLoopIndex < ubBlockNbr) && (PROG_FALSE == ubBlockFound)))?BCOV(809):!BCOV(810); ubLoopIndex++)
            {
                eESSStatus = Prog_GetEssLogicalBlockStartAddr(ubLoopIndex, &ulBlockAddr);
                if (((PROG_E_OK == eESSStatus) && (stConfigSegment[uwIdx].ulEraseStartAddress >= ulBlockAddr))?BCOV(811):!BCOV(812))
                {
                    eESSStatus = Prog_GetEssLogicalBlockLength(ubLoopIndex, &ulBlockLength);
                    if (((PROG_E_OK == eESSStatus) && ((stConfigSegment[uwIdx].ulEraseEndAddress - stConfigSegment[uwIdx].ulEraseStartAddress) <= ulBlockLength))?BCOV(813):!BCOV(814))
                    {
            ...
            if ((PROG_FALSE == ubBlockFound)?BCOV(815):!BCOV(816))
            {
...
NCS_PRI_VAR void PROG_GetDataBlockNbrFromVBT(u16 *uwBlockNumber)
{
  ...
  u32 ulVBTAddress;
   ...
   /* SBL variant */
   /* Address of VBT is get from ESS, subtract one because the ess data is not included in the ess   */
   ICOV(817);(void)Prog_GetEssLogicalBlockVerifTable(m_stCurrentSegment.ubLogicalBlockId - 1, &ulVBTAddress);
...
NCS_PRI_VAR void PROG_GetVBTFormat(u16 *uwFormatID)
{
  ...
  u32 ulVBTAddress;
   ...
   /* SBL variant */
   /* Address of VBT is get from ESS, subtract one because the ess data is not included in the ess   */
   ICOV(818);(void)Prog_GetEssLogicalBlockVerifTable(m_stCurrentSegment.ubLogicalBlockId - 1, &ulVBTAddress);
...
NCS_PRI_VAR tProgStatus PROG_VerifyVBTFormatAndLength(void)
{
  ...
  /* SBL variant */
  /* in case of ESS assume one continuous block */
  ICOV(819);if((0 == m_stCurrentSegment.ubLogicalBlockId)?BCOV(820):!BCOV(821))
  {
    ...
    if((PROG_E_OK == ubProgStatus)?BCOV(822):!BCOV(823))
    {
...
#else
  ICOV(824);ubProgStatus = PROG_MemDataAccess(PROG_MEM_TYPE_RAM ,PROG_MEM_OPERATION_TYPE_READ, PROG_SBL_VBT_START_ADDRESS, PROG_VBT_FORMAT_ID_SIZE, aubData);
  if((PROG_E_OK == ubProgStatus)?BCOV(825):!BCOV(826))
  {
    uwVBTFormat = (u16)((u16)aubData[0] << 8);
    uwVBTFormat |= (u16)aubData[1];
  }
#endif
  ICOV(824);if((PROG_E_OK == ubProgStatus)?BCOV(827):!BCOV(828))
  {
    if((uwVBTFormat != (u16)PROG_EXPECTED_VBT_FORMAT_ID)?BCOV(829):!BCOV(830))
    {
      ubProgStatus = PROG_E_NOT_OK;
  ...
  if((PROG_E_OK == ubProgStatus)?BCOV(831):!BCOV(832))
  {
    /* Check Length */
#if (PROG_IMPL_VARIANT == PROG_IMPL_11)
    /* SBL variant */
    /* in case of ESS assume one continuous block */
    if((0 == m_stCurrentSegment.ubLogicalBlockId)?BCOV(833):!BCOV(834))
    {
      uwDataBlockNbr = 1;
    }
    else
    ...
    if((PROG_E_OK == ubProgStatus)?BCOV(835):!BCOV(836))
    {
      uwDataBlockNbr = (u16)((u16)aubData[0] << 8);
      uwDataBlockNbr |= (u16)aubData[1];
    }
    if((PROG_E_OK == ubProgStatus)?BCOV(837):!BCOV(838))
#endif
    {
      ...
      if((m_ulVBTWrittenData != ulExpectedLength)?BCOV(839):!BCOV(840))
      {
        ubProgStatus = PROG_E_NOT_OK;
        m_ubCheckMemoryStatus = PROG_INVALID_VBT_FORMAT_LENGTH;
      }
...
NCS_PRI_VAR tProgStatus PROG_CheckHashResults(void)
{
   ...
   /* SBL variant */
   /* in case of ESS assume one continuous block */
   ICOV(841);if((0 == m_stCurrentSegment.ubLogicalBlockId)?BCOV(842):!BCOV(843))
   {
     uwDataBlockNbr = 1;
   }
   else
...
#else
   ICOV(844);ubProgStatus = PROG_MemDataAccess(PROG_MEM_TYPE_RAM ,PROG_MEM_OPERATION_TYPE_READ, PROG_SBL_VBT_START_ADDRESS + PROG_DATA_BLOCKS_COUNT_MSB_OFFSET, PROG_DATA_BLOCKS_COUNT_SIZE, aubData);
   if((PROG_E_OK == ubProgStatus)?BCOV(845):!BCOV(846))
   {
     uwDataBlockNbr = (u16)((u16)aubData[0] << 8);
     uwDataBlockNbr |= (u16)aubData[1];
   }
   ...
   if((PROG_E_OK == ubProgStatus)?BCOV(847):!BCOV(848))
#endif /*(PROG_IMPL_VARIANT == PROG_IMPL_11)*/
   {
     /* Check that all data blocks are downloaded and their hash is correct */
     ICOV(849);for(uwIdx = 0; (uwIdx < uwDataBlockNbr)?BCOV(850):!BCOV(851); uwIdx++)
     {
       ubProgStatus |= m_aubHashComparisonStatus[uwIdx];
     }

     if((PROG_E_NOT_OK == ubProgStatus)?BCOV(852):!BCOV(853))
     {
...
{
    ICOV(854);if ((SA_GetSecuredKeysStatus() == SA_E_OK)?BCOV(855):!BCOV(856))
    {
