...
{
    ICOV(857);PROG_GetPublicKey(&m_stProgAsymPublicKey_cbk);
}
...
tProgStatus PROG_CheckPSIValue(u8 ubPartitionId, tProgPsiValue ePsiValue)
{
    ...
    ICOV(858);DBG_PROG_CHECKPSIVALUE_ENTRY(ubPartitionId, ePsiValue);

    /* Get PSI Address */
    eProgStatus = PROG_GetPSIAddress( ubPartitionId, &ulPSIAddress);

    if((eProgStatus == PROG_E_OK)?BCOV(859):!BCOV(860))
    {
        ...
        for(ubLoopIndex = 0U; (ubLoopIndex < PROG_PSI_VAL_SIZE)?BCOV(861):!BCOV(862); ubLoopIndex++)
        {
            if((aubPSIAddress[ubLoopIndex] != pubValToCheck[ubLoopIndex])?BCOV(863):!BCOV(864))
            {
                eProgStatus = PROG_E_CHECK_FAILED;
            }
        }

        if((eProgStatus == PROG_E_CHECK_FAILED)?BCOV(865):!BCOV(866))
        {
            /* PSI is not revoked so check if it is programmed */
            if((ePsiValue == PROG_PSI_PROGRAMMED)?BCOV(867):!BCOV(868))
            {
                /*called to check if the page is erased or not */
                eProgStatus = PROG_MemCheckPageErasing(
                                    m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType,
                                    ulPSIAddress);
                if((eProgStatus == PROG_E_OK)?BCOV(869):!BCOV(870))
                {
                    ...
                    for(ubLoopIndex = 0U; (ubLoopIndex < PROG_PSI_VAL_SIZE)?BCOV(871):!BCOV(872); ubLoopIndex++)
                    {
                        if((aubPSIAddress[ubLoopIndex] != pubValToCheck[ubLoopIndex])?BCOV(873):!BCOV(874))
                        {
                            eProgStatus = PROG_E_CHECK_FAILED;
                        }
                    }
        ...
        {
            if((ePsiValue == PROG_PSI_PROGRAMMED)?BCOV(875):!BCOV(876))
            {
                eProgStatus = PROG_E_CHECK_FAILED;
            }
        }
...
#else
    ICOV(882);DBG_PROG_ENTRY_DECOMPHEADER_ENTRY();
#endif
    ICOV(882);DBG_PROG_ENTRY_DECOMPHEADER_EXIT();
}
...
void PROG_Entry_ValidateSBASignature(void)
{
    ...
    ICOV(883);DBG_PROG_ENTRY_VALIDATESBASIGNATURE_ENTRY( );
    ...
    for(ubIdx = 0U; /* Modulus */
        (ubIdx < m_stProgAsymPublicKey.data[(PROG_KEY_LENGTH/sizeof(u32)) + 1])?BCOV(884):!BCOV(885);
        ubIdx++)
    {
    ...
    if((eProgStatus == PROG_E_NOT_OK)?BCOV(886):!BCOV(887))
    {
...
void PROG_SBASignerInfoCheck(void)
{
    ...
    ICOV(888);DBG_PROG_SBASIGNERINFOCHECK_ENTRY();
    ...
    switch(eOperationStatus)
    {
        case PROG_E_OK:
            ...
            ICOV(889);PROG_HsmEmit(&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_VALIDATE_SIGNATURE);
            break;
        case PROG_E_BUSY:
            /* Nothing to do */
            ICOV(890);break;
        default:
            ICOV(891);m_stBootStatus.ubSignatureBypass |= PROG_ERR_SBA_CERT;
            /* Emit event PROG_EVENT_INIT*/
            PROG_HsmEmit(&PROG_HsmScPROG,
                PROG_HSM_PROG_EV_PROG_EVENT_INIT);
            break;
...
tProgStatus PROG_GetPSIAddress(u8 ubPartitionId, u32* pulPSIAddress)
{
    ...
    ICOV(892);eProgStatus = PROG_E_OK;

    /* Check if it is the application partition */
    if((ubPartitionId == PROG_SW_APP_ID)?BCOV(893):!BCOV(894))
    {
    ...
    else
    {
        ...
        if((ubPartitionId <= (uwCalPartitionNbr + 1U))?BCOV(895):!BCOV(896))
        {
...
tUdsStatus PROG_CheckProgrammingDependencies_Impl20(PduLengthType *pulLen, u8 *aubUdsData)
{
    ...
    ICOV(897);if (((NULL_PTR != pulLen) && (NULL_PTR != aubUdsData))?BCOV(898):!BCOV(899))
    {
        /* Set response length */
        *pulLen = 9U;
        ...
        if ((PROG_E_OK == eProgStatus)?BCOV(900):!BCOV(901))
        {
            ubPartitionId++;
            /* Get address where is set the number of calibration partitions */
            uwCalPartitionNbr = PROG_GET_CALIB_PARTITION_NB;
            while((eProgStatus == PROG_E_OK)?BCOV(902):!BCOV(903))
            {
                if((ubPartitionId <= (uwCalPartitionNbr + 1U))?BCOV(904):!BCOV(905))
                {
            ...
            if ((PROG_E_CHECK_FAILED == eProgStatus)?BCOV(906):!BCOV(907))
            {
...
tProgBoolean PROG_IsValidApplication(void)
{
    ...
    u32 ulSize;
    u16 uwCalPartitionNbr;
    ICOV(908);DBG_PROG_ISVALIDAPPLICATION_ENTRY();
    ...
    if ((PROG_E_OK == eProgStatus)?BCOV(909):!BCOV(910))
    {
        ...
        while((eProgStatus == PROG_E_OK)?BCOV(911):!BCOV(912))
        {
            if((ubPartitionId <= (uwCalPartitionNbr + 1U))?BCOV(913):!BCOV(914))
            {
                /* When application is valid, check the next partition which are calibration */
    ...
    if ((PROG_E_CHECK_FAILED == eProgStatus)?BCOV(915):!BCOV(916))
    {
...
tProgStatus PROG_GetModulePartition(u16 uwModuleId, u8 * pubPartitionId, u8 * pubModuleIndex)
{
    ...
    ICOV(917);eProgStatus = PROG_E_NOT_OK;

    for(ubPartitionId = 2U; ((ubPartitionId < (PROG_GET_CALIB_PARTITION_NB + 2U))
                && (eProgStatus != PROG_E_OK))?BCOV(918):!BCOV(919); ubPartitionId++)
    {
        for(ubModuleIndex = 0U; (((ubModuleIndex < (PROG_GET_CALIB_SW_MODULE_NBR(ubPartitionId)))
                && (eProgStatus != PROG_E_OK)))?BCOV(920):!BCOV(921); ubModuleIndex++)
        {
            if((uwModuleId == PROG_GET_CALIB_SW_MODULE_ID(ubPartitionId,ubModuleIndex))?BCOV(922):!BCOV(923))
            {
                /* Module found */
...
void PROG_SBASignatureCheck(void)
{
    tProgStatus eOperationStatus;
    u8 ubSBIFlagToStore;
    ICOV(924);DBG_PROG_SBASIGNATURECHECK_ENTRY();
    ...
    switch(eOperationStatus)
    {
        case PROG_E_OK:
            ...
            ICOV(925);PROG_SetSBIFlagValue(ubSBIFlagToStore);
        ...
        case PROG_E_BUSY:
            ...
            ICOV(926);break;
        default:
            ICOV(927);m_stBootStatus.ubSignatureBypass |= PROG_ERR_SBA_SIGNATURE;
            /* Emit event PROG_EVENT_INIT*/
            PROG_HsmEmit(&PROG_HsmScPROG,
                PROG_HSM_PROG_EV_PROG_EVENT_INIT);
            break;
...
u8 PROG_ValidateSBATicket(void)
{
    ...
    u16 uwModuleId;
    u8 ubSBIFlagToStore;
    ICOV(928);DBG_PROG_VALIDATESBATICKET_ENTRY();
    ...
    if (((uwModuleId == PROG_SBA_MODULE_ID) && (PROG_E_OK == eProgStatus))?BCOV(929):!BCOV(930))
    {
        /* Check ECU Name */
        eProgStatus = PROG_CheckEcuName(&m_aubSwInfo[4]);

        if ((eProgStatus == PROG_E_OK)?BCOV(931):!BCOV(932))
        {
            /* Check ECU Id */
            eProgStatus = PROG_CheckEcuId(&m_aubSwInfo[4U + PROG_ECU_NAME_SIZE]);

            if ((eProgStatus == PROG_E_OK)?BCOV(933):!BCOV(934))
            {
                if ((m_ubSimulateProgSessionWithResponse == FALSE)?BCOV(935):!BCOV(936))
                {
...
void PROG_Entry_ValidateSBASignerInfo(void)
{
    ...
    ICOV(937);DBG_PROG_ENTRY_VALIDATESBASIGNERINFO_ENTRY();
    ...
    if((PROG_E_OK == eProgStatus)?BCOV(938):!BCOV(939))
    {
        uwStoredNBID = PROG_GetKeyNBIDValue();

        if((uwNBID >= uwStoredNBID)?BCOV(940):!BCOV(941))
        {
            /* Call crypto library to verify the root signature */
            ...
            if((eProgStatus == PROG_E_NOT_OK)?BCOV(942):!BCOV(943))
            {
...
void PROG_CheckDecompHeaderStatus(void)
{
    ...
    ICOV(944);DBG_PROG_CHECKDECOMPHEADERSTATUS_ENTRY();

    ullength = 0U;

    eCompStatus = PROG_GetDecompressedData(&pubdata, &ullength);

    if((ullength > 4U)?BCOV(945):!BCOV(946))
    {
        /* Get Data Type */
        uwDataType =    (u16)((u16)((u16)((u16)(pubdata[0]) << 8U) & 0xFF00U) |
                        (u16)((u16)(pubdata[1]) & 0x00FFU));

       if((uwDataType == PROG_DATATYPE_SIGNED)?BCOV(947):!BCOV(948))
       {
#if (PROG_HSM_UPDATE_ENABLE == STD_ON)
        if((stConfigSegment[m_stCurrentSegment.ubSegmentId].ePartitionType == PROG_HSM_PARTITION)?BCOV(949):!BCOV(950))
        {
...
#endif
        {
            ...
            {
                if ((eCompStatus == PROG_DECOMP_IN_PROGRESS )?BCOV(955):!BCOV(956))
                {
                    /* Not enough data decompressed, wait next call */
                    eProgStatus = PROG_DecompressData (NULL_PTR,0);
                    if((eProgStatus != PROG_E_OK)?BCOV(957):!BCOV(958))
                    {
...
#else
    ICOV(959);DBG_PROG_CHECKDECOMPHEADERSTATUS_ENTRY();
#endif
    ICOV(959);DBG_PROG_CHECKDECOMPHEADERSTATUS_EXIT();
}
...
u8* PROG_GetPartitionAddrInAppSWInfo(u8* pubHeaderAddr, u8 ubPartitionId, u16 uwRamAccess)
{
    ...
    for (ubLoopIndex = 2U; (ubLoopIndex < ubPartitionId)?BCOV(963):!BCOV(964); ubLoopIndex++)
    {
...
void PROG_Entry_TD_Header(void)
{
    tProgStatus eProgStatus;
    u16 uwDataType;
    ICOV(965);DBG_PROG_ENTRY_TD_HEADER_ENTRY();
    ...
    switch(uwDataType)
    {
        case PROG_DATATYPE_SIGNED:
#if (PROG_COMPRESSION_ENABLE == STD_ON)
            /* Check if DFI = 0x00 */
            ICOV(966);if((m_stReprogInfo.ubCompRequired == FALSE)?BCOV(967):!BCOV(968))
            {
#endif
#if(PROG_HSM_UPDATE_ENABLE == STD_ON)
            ICOV(969);if((stConfigSegment[m_stCurrentSegment.ubSegmentId].ePartitionType == PROG_HSM_PARTITION)?BCOV(970):!BCOV(971))
            {
                /* Emit event PROG_EVENT_REQ_TD*/
                PROG_HsmEmit (&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_REQ_TD);
            }
 ...
 #endif
            {
                ...
                ICOV(972);eProgStatus=PROG_CheckCompleteHeader(&m_stDiagMessageInfo.pubData[4], (m_stDiagMessageInfo.ulLength-4));

                if ((PROG_E_OK == eProgStatus)?BCOV(973):!BCOV(974))
                {
                    /* Check Signed Header */
                    eProgStatus = PROG_CheckSignedHeader(&m_stDiagMessageInfo.pubData[4]);

                    if((eProgStatus == PROG_E_OK)?BCOV(975):!BCOV(976))
                    {
        ...
        case PROG_DATATYPE_COMPRESSED:
            /* Check if DFI = 0x10 */
            ICOV(977);if((m_stReprogInfo.ubCompRequired == TRUE)?BCOV(978):!BCOV(979))
            {
                /* Data are compressed, check data length*/
                if((m_stDiagMessageInfo.ulLength > 10U)?BCOV(980):!BCOV(981))
                {
...
#if PROG_PEC_ENABLE == STD_ON
            /* Set error in PEC */
            ICOV(982);m_uwPEC = PROG_ERR_DATA_TYPE;
#endif
            ICOV(983);if((m_stDiagMessageInfo.pubData[1] != m_stReprogInfo.ubBlockSequenceCounter)?BCOV(984):!BCOV(985))
            {
...
void PROG_MessageDigestCheck(void)
{
    u8 ubIdx;
    tProgStatus eProgStatus;
    ICOV(986);DBG_PROG_MESSAGEDIGESTCHECK_ENTRY();

    eProgStatus = PROG_E_OK;

    for(ubIdx = 0U; (ubIdx < PROG_DIGEST_LENGTH)?BCOV(987):!BCOV(988); ubIdx++)
    {
        /* Compare digest */
        if((m_aubComputedMessageDigest[ubIdx] != m_aubMessageDigest[ubIdx])?BCOV(989):!BCOV(990))
        {
            eProgStatus = PROG_E_NOT_OK;
        }
    }

    if((eProgStatus == PROG_E_OK)?BCOV(991):!BCOV(992))
    {
...
void PROG_ComputeMessageDigest(void)
{
    ...
    ICOV(993);DBG_PROG_COMPUTEMESSAGEDIGEST_ENTRY();

    eCrcStatus = PROG_CRC();
    if (( PROG_E_OK == eCrcStatus)?BCOV(994):!BCOV(995))
    {
...
void PROG_SignatureCheck(void)
{
    ...
    ICOV(996);DBG_PROG_SIGNATURECHECK_ENTRY();
    ...
    switch(eProgStatus)
    {
        case PROG_E_OK:
            ...
            ICOV(997);eProgStatus = PROG_FinalizeCheckSignedHeader(m_pubHeaderAddress);

            if((eProgStatus == PROG_E_OK)?BCOV(998):!BCOV(999))
            {
                if((m_stCurrentSegment.uwModuleId == PROG_SW_APP_ID)?BCOV(1000):!BCOV(1001))
                {
                ...
                for(ubIdx = 0U; (ubIdx < PROG_DIGEST_LENGTH)?BCOV(1002):!BCOV(1003); ubIdx++)
                {
...
#if (PROG_PEC_ENABLE == STD_ON)
            /* Set error in PEC */
            ICOV(1005);m_uwPEC = PROG_ERR_SIGNATURE;
#endif
            /* Send negative response NRC31 */
            ICOV(1006);m_stDiagMessageInfo.ubDiagStatus = UDS_NRC_31;
            /* Emit event PROG_EVENT_FAILED*/
            PROG_HsmEmit (&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_FAILED);
            break;
    }
...
void PROG_Entry_ValidateSignature(void)
{
    ...
    ICOV(1007);DBG_PROG_ENTRY_VALIDATESIGNATURE_ENTRY();
    ...
    if((uwModuleId == PROG_SW_APP_ID)?BCOV(1008):!BCOV(1009))
    {
    ...
    if((uwInnerEnvSign != PROG_DATATYPE_PLAIN)?BCOV(1010):!BCOV(1011))
    {
#if PROG_PEC_ENABLE == STD_ON
      /* Set error in PEC */
      m_uwPEC = PROG_ERR_DATA_TYPE;
    ...
    if((eProgStatus == PROG_E_OK)?BCOV(1012):!BCOV(1013))
    {
      ubStoredSBIFlag = PROG_GetSBIFlagValue();

        if((ubStoredSBIFlag == TRUE)?BCOV(1014):!BCOV(1015))
        {
        ...
        {
            if(((m_pubHeaderAddress[0] == 0U) && (m_pubHeaderAddress[1] == 1U))?BCOV(1016):!BCOV(1017))
            {
                ...
                m_stProgAsymPublicKey.data[(PROG_KEY_LENGTH/sizeof(u32)) + 1] = (PROG_KEY_LENGTH/sizeof(u32)); /* Modulus size */
                for(ubIdx = 0U; /* Modulus */
                    (ubIdx < m_stProgAsymPublicKey.data[(PROG_KEY_LENGTH/sizeof(u32)) + 1])?BCOV(1018):!BCOV(1019);
                    ubIdx++)
                {
                    u8 ubLocIdx = ubIdx + ((u8)(PROG_KEY_LENGTH/sizeof(u32))) + 2U;
                    m_stProgAsymPublicKey.data[ubLocIdx] = (u32) paubSwInfoPublicModulus[((u16)(PROG_KEY_LENGTH - 1U) - ((u16)ubIdx*4U))];
            ...
            else
            {
                ...
                m_stProgAsymPublicKey.data[(PROG_KEY_LENGTH/sizeof(u32)) + 1] = (PROG_KEY_LENGTH/sizeof(u32));/* Modulus size */
                for(ubIdx = 0U; /* Modulus */
                    (ubIdx < m_stProgAsymPublicKey.data[(PROG_KEY_LENGTH/sizeof(u32)) + 1])?BCOV(1020):!BCOV(1021);
                    ubIdx++)
                {
                    u8 ubLocIdx = ubIdx + ((u8)(PROG_KEY_LENGTH/sizeof(u32))) + 2U;
                    m_stProgAsymPublicKey.data[ubLocIdx] = (u32) paubSwInfoPublicModulus[((u16)(PROG_KEY_LENGTH - 1U) - ((u16)ubIdx*4U))];
            ...
            if((eProgStatus == PROG_E_NOT_OK)?BCOV(1022):!BCOV(1023))
            {
#if (PROG_PEC_ENABLE == STD_ON)
                /* Set error in PEC */
...
void PROG_Entry_ValidateSignerInfo(void)
{
    ...
    ICOV(1024);DBG_PROG_ENTRY_VALIDATESIGNERINFO_ENTRY();

    /* Check Module Id */
    uwModuleId = ((u16)((u16)((u16)m_pubHeaderAddress[0] << 8U) & 0xFF00U) | (u16)(((u16)m_pubHeaderAddress[1]) & 0x00FFU));

    if((uwModuleId == PROG_SW_APP_ID)?BCOV(1025):!BCOV(1026))
    {
        uwRegionNbr = ((u16)((u16)((u16)m_pubHeaderAddress[30] << 8U) & 0xFF00U) | (u16)(((u16)m_pubHeaderAddress[31]) & 0x00FFU));
    ...
    if((eProgStatus == PROG_E_OK)?BCOV(1027):!BCOV(1028))
    {
        /* Get the Key NBID */
        ...
        if ((uwKeyNBID >= uwStoreKeyNBID)?BCOV(1029):!BCOV(1030))
        {
            ...
            /* In case of error */
            if((eProgStatus == PROG_E_NOT_OK)?BCOV(1031):!BCOV(1032))
            {
#if (PROG_PEC_ENABLE == STD_ON)
                /* Set error in PEC */
...
void PROG_SignerInfoCheck(void)
{
    ...
    ICOV(1033);DBG_PROG_SIGNERINFOCHECK_ENTRY();
    ...
    switch(eOperationStatus)
    {
        case PROG_E_OK:
            /* Emit event PROG_EVENT_VALIDATE_SIGNATURE*/
            ICOV(1034);PROG_HsmEmit (&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_VALIDATE_SIGNATURE);
            break;
        case PROG_E_BUSY:
            /* Wait for crypto to finish its treatment */
            ICOV(1035);break;
...
#if (PROG_PEC_ENABLE == STD_ON)
            /* Set error in PEC */
            ICOV(1036);m_uwPEC=PROG_ERR_ROOT_SIGNATURE;
#endif
            /* Send negative response NRC31 */
            ICOV(1037);m_stDiagMessageInfo.ubDiagStatus = UDS_NRC_31;
            /* Emit event PROG_EVENT_FAILED*/
            PROG_HsmEmit (&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_FAILED);
            break;
    }
...
tProgStatus PROG_InvalidateSection(u8 ubPartitionId)
{
    ...
    ICOV(1038);DBG_PROG_INVALIDATESECTION_ENTRY(ubPartitionId );

    if((ubPartitionId == PROG_SW_APP_ID)?BCOV(1039):!BCOV(1040))
    {
        ...
        if((eProgStatus == PROG_E_OK)?BCOV(1041):!BCOV(1042))
        {

            /* Revoke all calibration partition starting by the first one (partion number 1 is the application) */
            for(ublocalPartitionId = 2U;(((ublocalPartitionId <= (PROG_GET_CALIB_PARTITION_NB + 1U)) && (eUdsStatus == UDS_ACK) ))?BCOV(1043):!BCOV(1044); ublocalPartitionId++)
            {
              ...
              if(( (PROG_PROT_CALIBRATION_PARTITION != stConfigSegment[ubcurrentSegmentId].ePartitionType) &&
                  (eProgStatus == PROG_E_OK))?BCOV(1045):!BCOV(1046))
              {
    ...
    else
    {
        ...
        if((UDS_ACK == eUdsStatus)?BCOV(1047):!BCOV(1048))
        {
            /* Reset expectedModuleId for calibration programming */
    ...
    if((eUdsStatus != UDS_ACK)?BCOV(1049):!BCOV(1050))
    {
#if PROG_PEC_ENABLE == STD_ON
        if(((ublocalPartitionId - 1U) == PROG_SW_APP_ID)?BCOV(1051):!BCOV(1052))
        {
            /* Set error in PEC */
...
)
{
    ...
    ICOV(1053);ulRemaningDataToWrite = *pulLength;
    ...
    while (((ubRegionIndex < m_stCurrentSegment.ubRegionNbr) && (ulOffsetInTD >= (ulSize + m_stCurrentSegment.astRegion[ubRegionIndex].ulSize)))?BCOV(1054):!BCOV(1055))
    {
    ...
    if((ubRegionIndex < m_stCurrentSegment.ubRegionNbr)?BCOV(1056):!BCOV(1057))
    {
        ...
        do
        {
            ...
            if((ulRemaningDataToWrite > (m_stCurrentSegment.astRegion[ubRegionIndex].ulSize - ulWriteOffsetInRegion))?BCOV(1058):!BCOV(1059))
            {
            ...
            if(((ulRemaningDataToWrite > 0) && ((eProgStatus == PROG_E_OK) || (eProgStatus == PROG_E_BUSY)))?BCOV(1060):!BCOV(1061))
            {

                ubMemoryType = m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType;

                if((ubMemoryType == PROG_MEM_TYPE_FLASH)?BCOV(1062):!BCOV(1063))
                {
#if (PROG_FLASH_MODE == PROG_MEMORY_ASYNCHRONOUS)
                    if(((ubRegionIndex == 0U)||(eProgStatus == PROG_E_BUSY))?BCOV(1064):!BCOV(1065))
#else
                    if((ubRegionIndex == 0U)?BCOV(1066):!BCOV(1067))
#endif
                    {
                    ...
                    else
                    {
                        ...
                        if((ubRegionIndex >= m_stCurrentSegment.ubRegionNbr)?BCOV(1068):!BCOV(1069))
                        {
                            eProgStatus = PROG_E_NOT_OK;
                            ubExitLoop = TRUE;
                        }
                ...
                else if((ubMemoryType == PROG_MEM_TYPE_FLASH_EXT)?BCOV(1070):!BCOV(1071))
                {
#if (PROG_FLASH_EXT_MODE == PROG_MEMORY_ASYNCHRONOUS)
                    if(((ubRegionIndex == 0U)||(eProgStatus == PROG_E_BUSY))?BCOV(1072):!BCOV(1073))
#else
                    if((ubRegionIndex == 0U)?BCOV(1074):!BCOV(1075))
#endif
                    {
                        *pulLength -= ulRemaningDataToWrite;
                        ubExitLoop = TRUE;
                    ...
                    else
                    {
                        ...
                        if((ubRegionIndex >= m_stCurrentSegment.ubRegionNbr)?BCOV(1076):!BCOV(1077))
                        {
                            eProgStatus = PROG_E_NOT_OK;
                            ubExitLoop = TRUE;
                        }
                ...
                else if((ubMemoryType == PROG_MEM_TYPE_CUSTOM)?BCOV(1078):!BCOV(1079))
                {
#if (PROG_CUSTOM_MEMORY_MODE == PROG_MEMORY_ASYNCHRONOUS)
                    if(((ubRegionIndex == 0U) || (eProgStatus == PROG_E_BUSY))?BCOV(1080):!BCOV(1081))
#else
                    if((ubRegionIndex == 0U)?BCOV(1082):!BCOV(1083))
#endif
                    {
                        *pulLength -= ulRemaningDataToWrite;
                        ubExitLoop = TRUE;
                    ...
                    else
                    {
                        ...
                        if((ubRegionIndex >= m_stCurrentSegment.ubRegionNbr)?BCOV(1084):!BCOV(1085))
                        {
                            eProgStatus = PROG_E_NOT_OK;
                            ubExitLoop = TRUE;
                        }
        ...
        while((ubExitLoop == FALSE)?BCOV(1086):!BCOV(1087));
    }
    else
    {
        eProgStatus = PROG_E_NOT_OK;
...
NCS_PRI_FCT tProgStatus PROG_CheckSignedHeader(u8 * pubHeaderAddr)
{
    ...
    /* Get Module Id */
    ICOV(1088);uwModuleId = (u16)((u16)((u16)((u16)pubHeaderAddr[0] << 8U) & 0xFF00U) | (u16)((u16)pubHeaderAddr[1] & 0x00FFU));
    /* Get BCID/CCID Id */
    uwXCID = (u16)((u16)((u16)((u16)pubHeaderAddr[2] << 8U) & 0xFF00U) | (u16)((u16)pubHeaderAddr[3] & 0x00FFU));

    /* Check if it is application module */
    if((uwModuleId == PROG_SW_APP_ID)?BCOV(1089):!BCOV(1090))
    {
    ...
    if ((eProgStatus == PROG_E_OK)?BCOV(1091):!BCOV(1092))
    {
...
#if PROG_PEC_ENABLE == STD_ON
        if ((eProgStatus != PROG_E_OK)?BCOV(1093):!BCOV(1094))
        {
            /* Set error in PEC */
    ...
    if ((eProgStatus == PROG_E_OK)?BCOV(1095):!BCOV(1096))
    {
        /* Check ECU ID */
        eProgStatus = PROG_CheckEcuId(&pubHeaderAddr[12]);

#if PROG_PEC_ENABLE == STD_ON
        if ((eProgStatus != PROG_E_OK)?BCOV(1097):!BCOV(1098))
        {
            /* Set error in PEC */
...
NCS_PRI_FCT tProgStatus PROG_CheckApplSignedHeader(u16 uwXCID, u16 uwRegionNbr, const u8 * pubHeaderAddr)
{
    ...
    ICOV(1099);eProgStatus = PROG_E_OK;

    if(((uwXCID == m_stBootInfoBlock.uwBCID)
            || ( m_stBootInfoBlock.uwBCID == 0xFFFFU)
            || (uwXCID == 0xFFFFU))?BCOV(1100):!BCOV(1101))
    {
        /* Check address/size of every application region */
        for(ubRegionIndex = 0U; (((ubRegionIndex < uwRegionNbr) && (eProgStatus == PROG_E_OK)))?BCOV(1102):!BCOV(1103); ubRegionIndex++)
        {
            ...
            if((stConfigSegment[ubLocalSegmentId].ePartitionType != PROG_APPLICATION_PARTITION)?BCOV(1104):!BCOV(1105))
            {
                eProgStatus = PROG_E_NOT_OK;
            }

#if (PROG_COMPRESSION_ENABLE == STD_ON)
            if ((m_stReprogInfo.ubCompRequired == FALSE)?BCOV(1106):!BCOV(1107))
#endif
            {
...
#if PROG_PEC_ENABLE == STD_ON
            if((eProgStatus != PROG_E_OK)?BCOV(1108):!BCOV(1109))
            {
                /* Set error in PEC */
...
NCS_PRI_FCT tProgStatus PROG_CheckCalSignedHeader(u16 uwModuleId, u16 uwXCID)
{
    ...
    ICOV(1110);ubPartitionId = 0U;
    ubModuleIndex = 0U;

    eProgStatus = PROG_GetModulePartition(uwModuleId, &ubPartitionId, &ubModuleIndex);

    if((eProgStatus == PROG_E_OK)?BCOV(1111):!BCOV(1112))
    {
        if((m_stReprogInfo.uwExpectedModuleId[ubPartitionId - 1U] == uwModuleId)?BCOV(1113):!BCOV(1114))
        {
            uwModuleCCID = PROG_GET_CALIB_SW_MODULE_CCID(ubPartitionId,ubModuleIndex);
            if(((uwXCID == 0xFFFFU)
                    || ( 0xFFFFU == uwModuleCCID)
                    || (uwXCID == uwModuleCCID))?BCOV(1115):!BCOV(1116))
            {
                ...
                if(((PROG_GET_CALIB_SW_MODULE_ADDR(ubPartitionId,ubModuleIndex)
                        ...
                        <= (((PROG_GET_CALIB_SW_LAST_REGION_ADDR_START(ubPartitionId)
                                + PROG_GET_CALIB_SW_LAST_REGION_SIZE(ubPartitionId))) - PROG_PSI_SIZE))?BCOV(1117):!BCOV(1118))
                {
                    ...
                    m_stDataProgVerification.ubAlgo = PROG_VERIFY_HASH;
#if (PROG_COMPRESSION_ENABLE == STD_ON)
            if ((m_stReprogInfo.ubCompRequired == FALSE)?BCOV(1119):!BCOV(1120))
#endif
            {
                m_stReprogInfo.ulMemorySizeExpected = m_stCurrentSegment.ulSize;
            }
                    ...
                    if(((stConfigSegment[ubLocalSegmentId].ePartitionType != PROG_CALIBRATION_PARTITION)&&(stConfigSegment[ubLocalSegmentId].ePartitionType != PROG_PROT_CALIBRATION_PARTITION))?BCOV(1121):!BCOV(1122))
                    {
                        eProgStatus = PROG_E_NOT_OK;
                    }

#if PROG_PEC_ENABLE == STD_ON
                    if((eProgStatus != PROG_E_OK)?BCOV(1123):!BCOV(1124))
                    {
                        /* Set error in PEC */
...
NCS_PRI_FCT tProgStatus PROG_CheckEcuId(const u8* pubEcuId)
{
    ...
    ICOV(1125);eProgStatus = PROG_E_OK;
    ubIDCheckResult = TRUE;

    for(ubLoopIndex = 0U; (((ubLoopIndex < PROG_ECU_ID_SIZE ) && (ubIDCheckResult == TRUE)))?BCOV(1126):!BCOV(1127); ubLoopIndex++)
    {
        if((pubEcuId[ubLoopIndex] != 0U)?BCOV(1128):!BCOV(1129))
        {
            ubIDCheckResult = FALSE;
        }
    }

    if((ubIDCheckResult == FALSE)?BCOV(1130):!BCOV(1131))
    {
#if (PROG_USE_ECUID_CALLBACK == STD_ON)
        PROG_CustomGetEcuId(&aubEcuId[0]);
#endif /*(PROG_USE_ECUID_CALLBACK == STD_ON)*/
        for(ubLoopIndex = 0U; (((ubLoopIndex < PROG_ECU_ID_SIZE) && (eProgStatus == PROG_E_OK)))?BCOV(1132):!BCOV(1133); ubLoopIndex++)
        {
#if (PROG_USE_ECUID_CALLBACK == STD_OFF)
            if((pubEcuId[ubLoopIndex] != m_stBootInfoBlock.aubEcuId[ubLoopIndex])?BCOV(1134):!BCOV(1135))
            {
                eProgStatus = PROG_E_NOT_OK;
            }
#else
            if((pubEcuId[ubLoopIndex] != aubEcuId[ubLoopIndex])?BCOV(1136):!BCOV(1137))
            {
                eProgStatus = PROG_E_NOT_OK;
            }
...
NCS_PRI_FCT tProgStatus PROG_CheckEcuName(const u8* pubEcuName)
{
    ...
    ICOV(1138);eProgStatus = PROG_E_OK;

    for(ubLoopIndex = 0U; (((ubLoopIndex < PROG_ECU_NAME_SIZE) && (eProgStatus == PROG_E_OK)))?BCOV(1139):!BCOV(1140); ubLoopIndex++)
    {
        if((pubEcuName[ubLoopIndex] != m_stBootInfoBlock.aubEcuName[ubLoopIndex])?BCOV(1141):!BCOV(1142))
        {
            eProgStatus = PROG_E_NOT_OK;
        }
    }
...
NCS_PRI_FCT tProgStatus PROG_FinalizeCheckSignedHeader(const u8 * pubHeaderAddr)
{
    ...
    ICOV(1143);eProgStatus = PROG_E_OK;
    ubStoredSBIFlag = PROG_GetSBIFlagValue();
    ...
    if((uwModuleId == PROG_SW_APP_ID)?BCOV(1144):!BCOV(1145))
    {
        uwNBID = ((u16)((u16)((u16)pubHeaderAddr[28] << 8U) & 0xFF00U) | (u16)(((u16)pubHeaderAddr[29]) & 0x00FFU));

        uwStoredNBID = PROG_GetNBIDValue();

        if(((uwNBID >= uwStoredNBID) || (ubStoredSBIFlag == TRUE))?BCOV(1146):!BCOV(1147))
        {
            uwHeaderSize = 862U + (8U*uwRegionNbr);


#if (PROG_COMPRESSION_ENABLE == STD_ON)
            if ((FALSE == m_stReprogInfo.ubCompRequired)?BCOV(1148):!BCOV(1149))
#endif
            {
    ...
    else
    {
        uwHeaderSize = 858U;
#if (PROG_COMPRESSION_ENABLE == STD_ON)
            if ((FALSE == m_stReprogInfo.ubCompRequired)?BCOV(1150):!BCOV(1151))
#endif
            {
                /* Remove Signed Header from request */
                /*Move SID*/
    ...
    if(((ubStoredSBIFlag == FALSE) && (eProgStatus == PROG_E_OK))?BCOV(1152):!BCOV(1153))
    {
...
NCS_PRI_FCT tProgStatus PROG_CheckCompleteHeader(u8 * pubHeaderAddr,PduLengthType ulLength)
{
    ...
    ICOV(1154);eProgStatus = PROG_E_NOT_OK;

    if ((ulLength >= PROG_MIN_HEADER_SIZE)?BCOV(1155):!BCOV(1156))
    {
        /* Get Module Id */
        uwModuleId = (u16)((u16)((u16)((u16)pubHeaderAddr[0] << 8U) & 0xFF00U) | (u16)((u16)pubHeaderAddr[1] & 0x00FFU));

        if ((PROG_SW_APP_ID == uwModuleId)?BCOV(1157):!BCOV(1158))
        {
            /* Application header */
            if ((ulLength >= PROG_MIN_APP_HEADER_SIZE)?BCOV(1159):!BCOV(1160))
            {
                /* Get region number */
                uwRegionNbr = (u16)((u16)((u16)((u16)pubHeaderAddr[30] << 8U) & 0xFF00U) | (u16)((u16)pubHeaderAddr[31] & 0x00FFU));

                if ((ulLength >= (PROG_MIN_APP_HEADER_SIZE + (((PduLengthType)uwRegionNbr-1U)*16U)))?BCOV(1161):!BCOV(1162))
                {
                    /* Get AppSwInfo size */
                    ulAppSwInfoSize = PROG_GetAppSwSize(pubHeaderAddr,uwRegionNbr);
                    if ((ulLength >= (PROG_MIN_APP_SIGNED_HEADER_SIZE + (((PduLengthType)uwRegionNbr-1U)*8U) + (PduLengthType)ulAppSwInfoSize))?BCOV(1163):!BCOV(1164))
                    {
                        /* Header is complete */
                        eProgStatus = PROG_E_OK;
                    }
...
NCS_PRI_FCT u32 PROG_GetAppSwSize(u8 * pubHeaderAddr, u16 uwRegionNbr)
{
    ...
    ICOV(1165);ubCalNum = (u8)(PROG_GET_CALIB_PARTITION_NB_FROM_REQ(pubHeaderAddr,uwRegionNbr)+1U);
...
NCS_PRI_FCT tProgStatus PROG_CheckSubjectName(const u8* pubSubjectName)
{
    ...
    ICOV(1166);eProgStatus = PROG_E_OK;

    for(ubLoopIndex = 0U; (((ubLoopIndex < PROG_SUBJECT_NAME_SIZE) && (eProgStatus == PROG_E_OK)))?BCOV(1167):!BCOV(1168); ubLoopIndex++)
    {
        if((pubSubjectName[ubLoopIndex] != m_stBootInfoBlock.aubSubjectName[ubLoopIndex])?BCOV(1169):!BCOV(1170))
        {
            eProgStatus = PROG_E_NOT_OK;
        }
    }
...
void PROG_HSMUpdate_TD(void)
{
...
#if(PROG_COMPRESSION_ENABLE == STD_ON)
    tProgDecompressStateType eCompStatus;
    ICOV(1171);eCompStatus = PROG_DECOMP_FINISH;
    ullength = 0U;
#else
    ICOV(1172);OSC_PARAM_UNUSED(ullength);
    OSC_PARAM_UNUSED(pubdata);
#endif

    ICOV(1172);m_stDiagMessageInfo.ubDiagStatus = UDS_ACK;

#if(PROG_COMPRESSION_ENABLE == STD_ON)
    if((m_stReprogInfo.ubCompRequired == TRUE)?BCOV(1173):!BCOV(1174))
    {
        eCompStatus = PROG_GetDecompressedData(&pubdata, &ullength);
        if(((eCompStatus == PROG_DECOMP_FINISH) || (eCompStatus == PROG_DECOMP_IN_PROGRESS ))?BCOV(1175):!BCOV(1176))
        {
            /*Copy the all Decompressed data from TD request to HSM Buffer*/
            m_ulHsmbufferlength = ullength;
            for(uwIdx = 0U; (uwIdx < ullength)?BCOV(1177):!BCOV(1178); uwIdx++)
            {
...
#endif
    {
       ...
       for(uwIdx = 0U; (uwIdx < m_ulHsmbufferlength)?BCOV(1179):!BCOV(1180); uwIdx++)
       {
    ...
    {
        if((m_uwHSMTimeoutCnt <= PROG_HSM_TIMEOUT)?BCOV(1183):!BCOV(1184))
        {
          ...
          if((eProgStatus != PROG_E_BUSY)?BCOV(1185):!BCOV(1186))
          {
             m_uwHSMTimeoutCnt = 0U;
             if(( uwHsmPEC == PROG_PEC_NO_ERROR)?BCOV(1187):!BCOV(1188))
             {
#if(PROG_COMPRESSION_ENABLE == STD_ON)
                if((eCompStatus == PROG_DECOMP_FINISH)?BCOV(1189):!BCOV(1190))
                {
                ...
                else /*(eCompStatus == PROG_DECOMP_IN_PROGRESS)*/
                {
                    ...
                    eProgStatus = PROG_DecompressData (NULL_PTR,0);
                    if((eProgStatus != PROG_E_OK)?BCOV(1191):!BCOV(1192))
                    {
                        /*Send NRC 72 and assign the Returned Error code to PEC*/
                        m_stDiagMessageInfo.ubDiagStatus = UDS_NRC_72;
                        /*Update the Error code to PEC*/
...
{
  ICOV(1193);if((m_uwHSMTimeoutCnt >= PROG_HSM_MAX_TIMEOUT)?BCOV(1194):!BCOV(1195))
  {
...
void PROG_Entry_HSMUpdate_TDFinish(void)
{
    ...
    ICOV(1196);DBG_PROG_EXIT_TD_WRITE_ENTRY();

    if (((PROG_TRUE == m_stWriteInfo.ubTDReceived) && (m_stDiagMessageInfo.ubDiagStatus == UDS_ACK))?BCOV(1197):!BCOV(1198))
    {
#if(PROG_MULTIPLE_BUFFERS_ENABLE == STD_ON)
        if((PROG_TRUE == m_stMultipleBuffers.eResponsePending)?BCOV(1199):!BCOV(1200))
        {
        ...
        if ((m_stDiagMessageInfo.pubData[1] == m_stReprogInfo.ubBlockSequenceCounter)?BCOV(1201):!BCOV(1202))
        {
    ...
    if((paubData != NULL_PTR)?BCOV(1203):!BCOV(1204))
    {
    ...
    else
    {
        ...
        if ((m_stEventInfo.Event != PROG_NO_EVENT)?BCOV(1205):!BCOV(1206))
        {
