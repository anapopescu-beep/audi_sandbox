...
void PROG_Init_Impl80(void)
{
    ...
    ICOV(1244);m_uwToolSignature = 0x0000U;
    m_ubErrorType     = 0x00U;
    ...
    for (uwIdx = 0U; (uwIdx < PROG_IMPL80_MAX_WRITTEN_DATA_NBR)?BCOV(1245):!BCOV(1246); uwIdx++)
    {
        m_aubWriteDataBuffer[uwIdx] = 0xFF;
    }
    /* Initialize segment's size */
    for (uwIdx = 0U; (uwIdx < PROG_SEGMENT_NB)?BCOV(1247):!BCOV(1248); uwIdx++)
    {
        m_aulDownloadedSegSize[uwIdx] = 0U;
    }
    /* Initialize status of routine request */
    for (uwIdx = 0U; (uwIdx < PROG_ROUTINE_RESULT_NB)?BCOV(1249):!BCOV(1250); uwIdx++)
    {
...
void PROG_Entry_RD_Impl80(void)
{
    ...
    ICOV(1251);m_stWriteInfo.ulDataToBeWritten   = 0U;
    ...
    if ((PROG_E_OK == eProgStatus)?BCOV(1252):!BCOV(1253))
    {
...
tProgStatus PROG_ControlRDParameters_Impl80(const u8 * pubUdsData, tUdsStatus * eUdsStatus)
{
    ...
    ICOV(1254);eProgStatus = PROG_E_OK;
    ...
    if ((PROG_E_OK == eProgStatus)?BCOV(1257):!BCOV(1258))
    {
        ...
        switch (m_ubLogicalMarker)
        {
            ...
            case PROG_IMPL80_LOGICAL_MARKER_KEY_APPLI:
                ICOV(1259);m_stProgrammingFlags.ubRDKeyAppliOk = PROG_TRUE;
                *eUdsStatus = UDS_ACK;
                break;
            ...
            case PROG_IMPL80_LOGICAL_MARKER_APP_CALIB:
                ICOV(1260);eProgStatus = PROG_E_NOT_OK;
                if ((PROG_IMPL80_TOOL_SIGNATURE == m_uwToolSignature)?BCOV(1261):!BCOV(1262))
                {
                    if ((PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)?BCOV(1263):!BCOV(1264))
                    {
                        for (ubIndex = 0U; (ubIndex < (PROG_BLOCK_NB-1))?BCOV(1265):!BCOV(1266); ubIndex++)
                        {
                        ...
                        if ((PROG_TRUE == m_stProgrammingFlags.ubGlobalBlank)?BCOV(1267):!BCOV(1268))
                        {
                    ...
                    else
                    {
                        ...
                        if ((PROG_TRUE == m_stProgrammingFlags.ubTunBlank)?BCOV(1269):!BCOV(1270))
                        {
                            eProgStatus = PROG_E_OK;
                        }
                    }
                ...
                if ((PROG_E_OK == eProgStatus)?BCOV(1271):!BCOV(1272))
                {
            ...
            case PROG_IMPL80_LOGICAL_MARKER_ZI:
                ICOV(1273);if ((PROG_TRUE == m_stProgrammingFlags.ubAuthorizeLog)?BCOV(1274):!BCOV(1275))
                {
            ...
            default:
                ICOV(1276);eProgStatus = PROG_E_NOT_OK;
                *eUdsStatus = UDS_NRC_31;
                break;
        }
    }
...
{
    ICOV(1278);switch (m_ubLogicalMarker)
    {
        case PROG_IMPL80_LOGICAL_MARKER_KEY_APPLI:
            ICOV(1279);PROG_ControlKeyAppliWritingParameters_Impl80(&m_stDiagMessageInfo.pubData[0U], m_stDiagMessageInfo.ulLength);
            break;

        case PROG_IMPL80_LOGICAL_MARKER_CALIB:
        case PROG_IMPL80_LOGICAL_MARKER_APP_CALIB:
            ICOV(1280);PROG_ControlDataWritingParameters_Impl80(&m_stDiagMessageInfo.pubData[0U], m_stDiagMessageInfo.ulLength);
            break;

        case PROG_IMPL80_LOGICAL_MARKER_ZI:
            ICOV(1281);PROG_ControlZIWritingParameters_Impl80(&m_stDiagMessageInfo.pubData[0U], m_stDiagMessageInfo.ulLength);
            break;

        default:
            ICOV(1282);m_stDiagMessageInfo.ubDiagStatus = UDS_NRC_31;
...
NCS_PRI_FCT void PROG_ControlKeyAppliWritingParameters_Impl80(const u8 * pubUdsData, u16 ulLength)
{
    ...
    ICOV(1283);if ((PROG_TRUE == m_stProgrammingFlags.ubRDKeyAppliOk)?BCOV(1284):!BCOV(1285))
    {
        /* Set TD received */
        m_stWriteInfo.ubTDReceived = PROG_TRUE;
        if ((PROG_IMPL80_BLOCK_SEQ_COUNT_APPLI_KEY == m_stDiagMessageInfo.pubData[1U])?BCOV(1286):!BCOV(1287))
        {
            uwCalculatedCrc = PROG_CalculateInversedCrc16_Impl80(pubUdsData, ulLength, PROG_IMPL80_CRC_INIT_VALUE);
            /* Check calculated CRC */
            if ((PROG_IMPL80_CRC_REVERSED_CCITT == uwCalculatedCrc)?BCOV(1288):!BCOV(1289))
            {
...
NCS_PRI_FCT void PROG_ControlDataWritingParameters_Impl80(const u8 * pubUdsData, u16 ulLength)
{
    ...
    ICOV(1290);eProgFirstCtrlOk = PROG_FALSE;

    /* Check RD flag */
    if ((PROG_TRUE == m_stProgrammingFlags.ubRDOk)?BCOV(1291):!BCOV(1292))
    {
        /* Set TD received */
        m_stWriteInfo.ubTDReceived = PROG_TRUE;
        ...
        if ((PROG_E_OK == eProgStatus)?BCOV(1293):!BCOV(1294))
#endif /* (PROG_PRECOND_CHECK_ENABLE == STD_ON) */
        {
            if ((m_stReprogInfo.ubBlockSequenceCounter == m_stDiagMessageInfo.pubData[1U])?BCOV(1295):!BCOV(1296))
            {
                uwCalculatedCrc = PROG_CalculateInversedCrc16_Impl80(pubUdsData, ulLength, PROG_IMPL80_CRC_INIT_VALUE);
                /* Check calculated CRC */
                if ((PROG_IMPL80_CRC_REVERSED_CCITT == uwCalculatedCrc)?BCOV(1297):!BCOV(1298))
                {
    ...
    if ((PROG_TRUE == eProgFirstCtrlOk)?BCOV(1299):!BCOV(1300))
    {
        ...
        if ((PROG_E_OK == eProgStatus)?BCOV(1301):!BCOV(1302))
        {
            if ((PROG_TRUE == m_stProgrammingFlags.ubWriteOk)?BCOV(1303):!BCOV(1304))
            {
                m_stDiagMessageInfo.ubDiagStatus = UDS_ACK;
                /* Push event PROG_EVENT_FAILED*/
                PROG_HsmEmit(&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_WRITE_CHECK);
...
NCS_PRI_FCT void PROG_ControlZIWritingParameters_Impl80(const u8 * pubUdsData, u16 ulLength)
{
    ...
    /* Check RD flag */
    ICOV(1305);if ((PROG_TRUE == m_stProgrammingFlags.ubRDLogOk)?BCOV(1306):!BCOV(1307))
    {
        /* Set TD received */
        m_stWriteInfo.ubTDReceived = PROG_TRUE;

        if ((PROG_IMPL80_BLOCK_SEQ_COUNT_ZI == m_stDiagMessageInfo.pubData[1U])?BCOV(1308):!BCOV(1309))
        {
            uwCalculatedCrc = PROG_CalculateInversedCrc16_Impl80(pubUdsData, ulLength, PROG_IMPL80_CRC_INIT_VALUE);
            /* Check calculated CRC */
            if ((PROG_IMPL80_CRC_REVERSED_CCITT == uwCalculatedCrc)?BCOV(1310):!BCOV(1311))
            {
                /* Check if there is enough space for ZI writing */
                eProgStatus = PROG_CustomCheckZIAvailableSpace();
                if ((PROG_E_OK == eProgStatus)?BCOV(1312):!BCOV(1313))
                {
                    m_stDiagMessageInfo.ubDiagStatus  = UDS_ACK;
                    /* Push write event PROG_EVENT_WRITE_CHECK */
                    PROG_HsmEmit(&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_WRITE_CHECK);
...
NCS_PRI_FCT tProgStatus PROG_AddressSizeConsistencyCheck(tProgAddressType ulStartAddress, u32 ulDataSize)
{
    ...
    ICOV(1314);eProgStatus = PROG_E_NOT_OK;
    ...
    if ((m_stDiagMessageInfo.ulLength == ulExpectSize)?BCOV(1315):!BCOV(1316))
    {
        ubCalibIdx    = 0U;
        ...
        if ((PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == stCfgBlock[0U].uwBlockIdentifier)?BCOV(1317):!BCOV(1318))
        {
            ubCalibIdx = 1U;
        }

        if ((PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)?BCOV(1319):!BCOV(1320))
        {
        ...
        for (ubSegmentIdx = 0U; (ubSegmentIdx < ubNbSegments)?BCOV(1321):!BCOV(1322); ubSegmentIdx++)
        {
            if ((  ((u32)ulStartAddress >= stConfigSegment[ubSegmentIdx + ubFirstSegment].ulStartAddress)
                &&( ((u32)ulStartAddress + ulDataSize) <= (stConfigSegment[ubSegmentIdx + ubFirstSegment].ulEndAddress + 1U)) )?BCOV(1323):!BCOV(1324))
            {
...
void PROG_GetBlockInfos_Impl80(u8 ubLogicMarker, tSegmentType * pstSegment)
{
    ...
    ICOV(1325);ubBlockIndex = 1U;

    /* Identify the index of block */
    if ((ubLogicMarker == stCfgBlock[0U].uwBlockIdentifier)?BCOV(1326):!BCOV(1327))
    {
        ubBlockIndex = 0U;
    }
...
void PROG_Entry_RTE_Impl80(void)
{
    ...
    ICOV(1328);if (( ((PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)
        ||(PROG_IMPL80_LOGICAL_MARKER_CALIB     == m_ubLogicalMarker))
       &&(m_stPageBuffer.ulOldDataLength != 0U) )?BCOV(1329):!BCOV(1330))
    {
    ...
    else
    {
        ...
        {
            case PROG_IMPL80_LOGICAL_MARKER_KEY_APPLI:
                ICOV(1331);if ((PROG_TRUE == m_stProgrammingFlags.ubRDKeyAppliOk)?BCOV(1332):!BCOV(1333))
                {
                    m_stProgrammingFlags.ubRDKeyAppliOk = PROG_FALSE;
                    eProgStatus                         = PROG_E_OK;
                }
            ...
            case PROG_IMPL80_LOGICAL_MARKER_APP_CALIB:
            case PROG_IMPL80_LOGICAL_MARKER_CALIB:
                ICOV(1334);if ((PROG_TRUE == m_stProgrammingFlags.ubRDOk)?BCOV(1335):!BCOV(1336))
                {
                    if((PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)?BCOV(1337):!BCOV(1338))
                    {
                    ...
                    for (/* No init */; (ubIndex < ubMaxSegments)?BCOV(1339):!BCOV(1340); ubIndex++)
                    {
            ...
            case PROG_IMPL80_LOGICAL_MARKER_ZI:
                ICOV(1341);if ((PROG_TRUE == m_stProgrammingFlags.ubRDLogOk)?BCOV(1342):!BCOV(1343))
                {
                    m_stProgrammingFlags.ubRDLogOk = PROG_FALSE;
                    eProgStatus                    = PROG_E_OK;
                }
            ...
            default:
                ICOV(1344);eProgStatus = PROG_E_NOT_OK;
                break;
        }
        if ((PROG_E_OK == eProgStatus)?BCOV(1345):!BCOV(1346))
        {
            m_stDiagMessageInfo.ubDiagStatus = UDS_ACK;
...
tUdsStatus PROG_ControlSelfCheckParameters_Impl80(PduLengthType *puwLen, u8 * pubData)
{
    ...
    ICOV(1347);if (((NULL_PTR != puwLen) && (NULL_PTR != pubData))?BCOV(1348):!BCOV(1349))
    {
        ...
        if ((PROG_FALSE == m_stProgrammingFlags.ubWriteOk)?BCOV(1350):!BCOV(1351))
        {
            /* Prepare response */
            *puwLen = 6U;
            /* Request is not ok */
        ...
        else
        {
            ...
            if ((PROG_REQUEST_ROUTINE_RESULT_PENDING != ubRtneStatus)?BCOV(1352):!BCOV(1353))
            {
...
void PROG_Entry_SelfCheck_Impl80(void)
{
    ...
    ICOV(1354);m_stDataProgVerification.uwVal = PROG_CRC16_INIT_VAL;
    ...
    if ((PROG_E_ERASED != eEraseStatus)?BCOV(1355):!BCOV(1356))
    {
        /* Retrieve the latest sizes for each segments */
        for (ubIndex = 0U; (ubIndex < PROG_SEGMENT_NB)?BCOV(1357):!BCOV(1358); ubIndex++)
        {
        ...
        if ((PROG_E_OK == eProgStatus)?BCOV(1359):!BCOV(1360))
        {
            if ((m_aulDownloadedSegSize[m_stCurrentSegment.ubSegmentId] > PROG_CRC_CALC_SIZE)?BCOV(1361):!BCOV(1362))
            {
            ...
            if ((PROG_E_OK == eProgStatus)?BCOV(1363):!BCOV(1364))
            {
                /* Call CRC API */
    ...
    if ((PROG_E_NOT_OK == eProgStatus)?BCOV(1365):!BCOV(1366))
    {
...
void PROG_Do_SelfCheck_Impl80(void)
{
    ...
    ICOV(1368);eProgStatus = PROG_E_OK;

    if ((m_aulDownloadedSegSize[m_stCurrentSegment.ubSegmentId] != 0U)?BCOV(1369):!BCOV(1370))
    {
        if ((m_aulDownloadedSegSize[m_stCurrentSegment.ubSegmentId] > PROG_CRC_CALC_SIZE)?BCOV(1371):!BCOV(1372))
        {
            ulReadLength = PROG_CRC_CALC_SIZE;
        }
        else
        ...
        if ((PROG_E_OK == eProgStatus)?BCOV(1373):!BCOV(1374))
        {
            /* Call CRC API */
            PROG_CalcCrc16(&m_aubCrcData[0], ulReadLength, &m_stDataProgVerification.uwVal);
            /* Update next address */
    ...
    {
        if ((m_stCurrentSegment.ubSegmentNbr > 1U)?BCOV(1375):!BCOV(1376))
        {
            /* Next segment */
        ...
        else
        {
            ...
            if ((m_auwCrcValue != m_stDataProgVerification.uwVal)?BCOV(1377):!BCOV(1378))
            {
                /* CRC check failed */
                eProgStatus = PROG_E_NOT_OK;
            }
            ...
            {
                if ((PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)?BCOV(1379):!BCOV(1380))
                {
                    ...
                    eEraseStatus = PROG_CustomGetEraseStatus(m_stCurrentSegment.ubLogicalBlockId);
                    if ((PROG_E_ERASED != eEraseStatus)?BCOV(1381):!BCOV(1382))
                    {
                        /* Get segment address of the first segment Id */
                        m_stCurrentSegment.ulStartAddress =
                                stConfigSegment[m_stCurrentSegment.ubSegmentId].ulStartAddress;
    ...
    if ((PROG_E_NOT_OK == eProgStatus)?BCOV(1383):!BCOV(1384))
    {
        if ((PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)?BCOV(1385):!BCOV(1386))
        {
            /* Application CRC verification failed */
            m_ubErrorType = PROG_IMPL80_SOFTWARE_NOT_OK;
        }
...
{
    ICOV(1387);if ((PROG_TRUE == m_stProgrammingFlags.ubProgInit)?BCOV(1388):!BCOV(1389))
    {
...
void PROG_Entry_ACT_Check_Impl80(void)
{
    ...
    /* Push EVENT_FINISHED */
    ICOV(1390);PROG_HsmEmit(&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_FINISHED);
}
...
void PROG_Do_RTE_Impl80(void)
{
    ...
    ICOV(1392);eProgStatus = PROG_E_NOT_OK;

    if ((  (PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)
        ||(PROG_IMPL80_LOGICAL_MARKER_CALIB     == m_ubLogicalMarker) )?BCOV(1393):!BCOV(1394))
    {
        eProgStatus = PROG_MemJobResult(m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType);

        if ((PROG_E_OK == eProgStatus)?BCOV(1395):!BCOV(1396))
        {
            /* Compare byte by byte */
            eProgStatus = PROG_CompareByteByByte_Impl80();
            if ((PROG_E_OK == eProgStatus)?BCOV(1397):!BCOV(1398))
            {
                if ((PROG_TRUE == m_stProgrammingFlags.ubRDOk)?BCOV(1399):!BCOV(1400))
                {
                    if((PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)?BCOV(1401):!BCOV(1402))
                    {
                        /* Store the size of the new downloaded segments for application + calibration*/
                        ubIndex = 0U;
                        ubMaxSegments = PROG_SEGMENT_NB;
                    ...
                    for (/* No init */; (ubIndex < ubMaxSegments)?BCOV(1403):!BCOV(1404); ubIndex++)
                    {
                        PROG_CustomStoreDownloadedSegmentSize(&m_aulDownloadedSegSize[ubIndex], ubIndex);
                    }
                    /* Reset RD ok flag */
    ...
    if ((PROG_E_OK == eProgStatus)?BCOV(1405):!BCOV(1406))
    {
        m_stDiagMessageInfo.ubDiagStatus = UDS_ACK;
        /* Emit event PROG_EVENT_FINISHED */
        PROG_HsmEmit(&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_FINISHED);
    }
    else if ((PROG_E_BUSY != eProgStatus)?BCOV(1407):!BCOV(1408))
    {
        m_stDiagMessageInfo.ubDiagStatus = UDS_NRC_24;
        /* Emit event PROG_EVENT_FAILED*/
        PROG_HsmEmit(&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_FAILED);
...
NCS_PRI_FCT void PROG_AppCalibWriteCheck_Impl80(void)
{
  ...
  ICOV(1409);ulMinValueToWrite = m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].ulMinValueToWrite;

  eProgStatus            = PROG_E_BYPASS;
  ulAddressDelta         = 0U;
  ulRemainingDataToWrite = 0U;

  if ((0U < m_stWriteInfo.ulWriteLength)?BCOV(1410):!BCOV(1411))
  {
      if ((0U < m_stPageBuffer.ulOldDataLength)?BCOV(1412):!BCOV(1413))
      {
  ...
  else
  {
      ...
      if ((0U < m_stPageBuffer.ulOldDataLength)?BCOV(1414):!BCOV(1415))
      {
          ...
          for (uwIdx = 0U; (uwIdx < m_stPageBuffer.ulOldDataLength)?BCOV(1416):!BCOV(1417); uwIdx++)
          {
      ...
      if (( (m_stWriteInfo.ulAddressToWrite + m_stPageBuffer.ulOldDataLength) == m_stWriteInfo.ulNewAddressToWrite)?BCOV(1418):!BCOV(1419))
      {
          eProgStatus = PROG_E_OK;
      }
      /* Are both addresses in the same page size */
      else if ((  (m_stWriteInfo.ulAddressToWrite / ulMinValueToWrite) ==
                      (m_stWriteInfo.ulNewAddressToWrite / ulMinValueToWrite) )?BCOV(1420):!BCOV(1421))
      {
      ...
      {
          if ((0U < m_stPageBuffer.ulOldDataLength)?BCOV(1422):!BCOV(1423))
          {
  ...
  if ((PROG_E_OK == eProgStatus)?BCOV(1424):!BCOV(1425))
  {
      /* Copy the new received data */
      for (uwIdx = 0U; (uwIdx < m_stWriteInfo.ulDataToBeWritten)?BCOV(1426):!BCOV(1427); uwIdx++)
      {
      ...
      if (((ulRemainingDataToWrite + ulAddressDelta) >= ulMinValueToWrite)?BCOV(1428):!BCOV(1429))
      {
          ...
          /* Save remaining data */
          for (uwIdx = 0U; (uwIdx < m_stPageBuffer.ulOldDataLength)?BCOV(1430):!BCOV(1431); uwIdx++)
          {
      ...
      else
      {
          ...
          for (uwIdx = 0U; (uwIdx < (ulRemainingDataToWrite + ulAddressDelta))?BCOV(1432):!BCOV(1433); uwIdx++)
          {
...
{
    ICOV(1434);if ((m_ubWaitCryptoBeforeTdEnd != PROG_FALSE)?BCOV(1435):!BCOV(1436))
    {
        ...
        if ((m_stDiagMessageInfo.ubDiagStatus == UDS_ACK)?BCOV(1437):!BCOV(1438))
        {
            /* Writing OK */
    ...
    else if ((UDS_ACK == m_stDiagMessageInfo.ubDiagStatus)?BCOV(1439):!BCOV(1440))
    {
        if ((  (PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)
            ||(PROG_IMPL80_LOGICAL_MARKER_CALIB     == m_ubLogicalMarker) )?BCOV(1441):!BCOV(1442))
        {
            /* Call WriteCheck sub-function */
            PROG_AppCalibWriteCheck_Impl80();
        }
        else if ((  (PROG_IMPL80_LOGICAL_MARKER_KEY_APPLI == m_ubLogicalMarker)
                 ||(PROG_IMPL80_LOGICAL_MARKER_ZI        == m_ubLogicalMarker) )?BCOV(1443):!BCOV(1444))
        {
    ...
    {
        if ((m_stWriteInfo.ubTDReceived == TRUE)?BCOV(1445):!BCOV(1446))
        {
            /* Emit event PROG_EVENT_FAILED*/
            PROG_HsmEmit(&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_FAILED);
        }
...
void PROG_Entry_Write_Impl80(void)
{
    ...
    ICOV(1447);switch (m_ubLogicalMarker)
    {
        case PROG_IMPL80_LOGICAL_MARKER_KEY_APPLI:
            /* Trigger the writing of KEY APPLI */
            ICOV(1448);eProgStatus = PROG_CustomWriteKeyAppli(&m_stDiagMessageInfo.pubData[PROG_IMPL80_KEY_APPLI_FIRST_BYTE_IN_TD], PROG_IMPL80_APPLI_KEY_LENGTH);
            break;

        case PROG_IMPL80_LOGICAL_MARKER_APP_CALIB:
        case PROG_IMPL80_LOGICAL_MARKER_CALIB:
            /* Trigger the writing of the received TD */
            ICOV(1449);eProgStatus = PROG_MemDataAccess(
                                m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType,
                                PROG_MEM_OPERATION_TYPE_WRITE,
                                m_stWriteInfo.ulAddressToWrite,
        ...
        case PROG_IMPL80_LOGICAL_MARKER_ZI:
            ...
            ICOV(1450);eProgStatus = PROG_CustomWriteZI(&m_stDiagMessageInfo.pubData[PROG_IMPL80_ZI_FIRST_BYTE_IN_TD], PROG_IMPL80_LOG_ZONE_LENGTH);
            break;

        default:
            ICOV(1451);eProgStatus = PROG_E_NOT_OK;
            break;
    }
...
void PROG_Write_Impl80(void)
{
    ...
    ICOV(1452);switch (m_ubLogicalMarker)
    {
        case PROG_IMPL80_LOGICAL_MARKER_KEY_APPLI:
        case PROG_IMPL80_LOGICAL_MARKER_ZI:
            /* Get job result for key appli or ZI */
            ICOV(1453);eProgStatus = PROG_CustomGetWriteJobResult(m_ubLogicalMarker);
            break;

        case PROG_IMPL80_LOGICAL_MARKER_APP_CALIB:
        case PROG_IMPL80_LOGICAL_MARKER_CALIB:
            /* Get job result */
            ICOV(1454);eProgStatus = PROG_MemJobResult(m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType);
            break;

        default:
            ICOV(1455);eProgStatus = PROG_E_NOT_OK;
            break;
    }
...
NCS_PRI_FCT void PROG_WriteStatusCheck_Impl80(tProgStatus eProgStatus)
{
    ...
    ICOV(1456);if ((PROG_E_OK == eProgStatus)?BCOV(1457):!BCOV(1458))
    {
        /* Case writing application and/or calibration */
        if ((  (PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)
            ||(PROG_IMPL80_LOGICAL_MARKER_CALIB     == m_ubLogicalMarker) )?BCOV(1459):!BCOV(1460))
        {
            /* Compare byte by byte */
            eProgStatus = PROG_CompareByteByByte_Impl80();
            if ((PROG_E_OK == eProgStatus)?BCOV(1461):!BCOV(1462))
            {
                ...
                /* Reset buffer used for writing */
                for (uwIdx = 0U; (uwIdx < PROG_IMPL80_MAX_WRITTEN_DATA_NBR)?BCOV(1463):!BCOV(1464); uwIdx++)
                {
        ...
        else
        {
            ...
            if ((PROG_IMPL80_LOGICAL_MARKER_ZI == m_ubLogicalMarker)?BCOV(1465):!BCOV(1466))
            {
                /* Set ECU status to $F0 */
                PROG_CustomSetECUStatus(PROG_IMPL80_ECU_STATUS_END_ALIGNEMENT);
                m_stProgrammingFlags.ubAuthorizeLog = PROG_FALSE;
    ...
    if (( (PROG_E_OK   != eProgStatus)
       &&(PROG_E_BUSY != eProgStatus))?BCOV(1467):!BCOV(1468))
    {
        if ((  (PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)
            ||(PROG_IMPL80_LOGICAL_MARKER_CALIB     == m_ubLogicalMarker) )?BCOV(1469):!BCOV(1470))
        {
            /* Write is failed */
            m_stProgrammingFlags.ubWriteOk = PROG_FALSE;
        }
        else if ((PROG_IMPL80_LOGICAL_MARKER_ZI == m_ubLogicalMarker)?BCOV(1471):!BCOV(1472))
        {
...
tUdsStatus PROG_ControlEraseParameters_Impl80(PduLengthType *puwLen, u8 * pubData)
{
    ...
    /* Retrieve routine identifier */
    ICOV(1473);m_uwRoutineId  = (u16)((u16)pubData[3U] & 0x00FFU);
    m_uwRoutineId |= (u16)(((u16)pubData[2U] << 8U) & 0xFF00U);
    ...
    if ((PROG_REQUEST_ROUTINE_RESULT_PENDING == ubRtneStatus)?BCOV(1474):!BCOV(1475))
    {
        /* Prepare response pending */
        *puwLen = 5U;
        /* Request is pending */
...
tProgStatus PROG_EraseFirstCheck_Impl80(void)
{
    ...
    ICOV(1478);m_stProgrammingFlags.ubProgInit    = PROG_TRUE;
    m_stProgrammingFlags.ubWriteOk     = PROG_TRUE;
    m_stProgrammingFlags.ubTunBlank    = PROG_FALSE;
    m_stProgrammingFlags.ubGlobalBlank = PROG_FALSE;
    ...
    eEraseStatus = PROG_CustomGetEraseStatus(m_stCurrentSegment.ubLogicalBlockId);
    if ((PROG_E_ERASED == eEraseStatus)?BCOV(1479):!BCOV(1480))
    {
        m_stProgrammingFlags.ubTunBlank = PROG_TRUE;
        if ((PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)?BCOV(1481):!BCOV(1482))
        {
            ...
            eEraseStatus = PROG_CustomGetEraseStatus(m_stCurrentSegment.ubLogicalBlockId);
            if ((PROG_E_ERASED == eEraseStatus)?BCOV(1483):!BCOV(1484))
            {
    ...
    if ((PROG_E_OK == eProgStatus)?BCOV(1485):!BCOV(1486))
    {
        /* Get segment address */
        m_stCurrentSegment.ulStartAddress =
...
void PROG_EraseStatusCheck_Impl80(tProgStatus eProgStatus)
{
    ...
    ICOV(1487);eEraseComplete = PROG_E_NOT_OK;
    while(((PROG_E_OK == eProgStatus) && (m_stCurrentSegment.ubSegmentNbr >= 1U))?BCOV(1488):!BCOV(1489))
    {
#if (PROG_SLICED_ERASING_FOR_NRC78_SENDING == STD_ON)
        /* Perform Erase status check only if the Memory type is not RAM, as erase sector by sector operation is not meant for RAM */
        if ((PROG_MEM_TYPE_RAM != m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType)?BCOV(1490):!BCOV(1491))
        {
        ...
        if(((ulNextAddress     < (m_stCurrentSegment.ulStartAddress + m_stCurrentSegment.ulSize)) && \
           (PROG_MEM_TYPE_RAM != m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType))?BCOV(1492):!BCOV(1493))
        {
            m_stCurrentSegment.ulSize -= ulNextAddress-m_stCurrentSegment.ulStartAddress;
            m_stCurrentSegment.ulStartAddress = ulNextAddress;

            if ((m_ubNbSectEraseCnt < PROG_NB_OF_SECTOR_TO_ERASE_BEFORE_SENDING_NRC78)?BCOV(1494):!BCOV(1495))
            {
                m_ubNbSectEraseCnt++;
...
#endif /* PROG_SLICED_ERASING_FOR_NRC78_SENDING == STD_ON */
        {
            if((m_stCurrentSegment.ubSegmentNbr > 1U)?BCOV(1496):!BCOV(1497))
            {
    ...
    if ((PROG_E_OK == eProgStatus)?BCOV(1498):!BCOV(1499))
    {
        if ((PROG_IMPL80_LOGICAL_MARKER_APP_CALIB == m_ubLogicalMarker)?BCOV(1500):!BCOV(1501))
        {
            if ((PROG_FALSE == m_stProgrammingFlags.ubTunBlank)?BCOV(1502):!BCOV(1503))
            {
                ...
                /* Check if logical block (application) is already erased */
                eEraseStatus = PROG_CustomGetEraseStatus(m_stCurrentSegment.ubLogicalBlockId);
                if ((PROG_E_ERASED == eEraseStatus)?BCOV(1504):!BCOV(1505))
                {
                    m_stProgrammingFlags.ubGlobalBlank = PROG_TRUE;
                ...
                else
                {
                    ...
                    if ((PROG_E_NOT_OK == eProgStatus)?BCOV(1506):!BCOV(1507))
                    {
    ...
    else if ((PROG_E_BUSY != eProgStatus)?BCOV(1508):!BCOV(1509))
    {
    ...
    if ((PROG_E_NOT_OK != eEraseComplete)?BCOV(1510):!BCOV(1511))
    {
#if (PROG_SLICED_ERASING_FOR_NRC78_SENDING == STD_ON)
        /* Reset counter of number sector to erase */
        m_ubNbSectEraseCnt = 0U;
...
NCS_PRI_FCT tProgStatus PROG_CompareByteByByte_Impl80(void)
{
    ...
    /* Read data written */
    ICOV(1512);eProgStatus = PROG_MemDataAccess(
                        m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType,
                        PROG_MEM_OPERATION_TYPE_READ,
                        m_stWriteInfo.ulAddressToWrite,
                        m_stWriteInfo.ulWriteLength,
    ...
    for (uwIndex = 0U; ((uwIndex < m_stWriteInfo.ulWriteLength) && (PROG_E_OK == eProgStatus))?BCOV(1513):!BCOV(1514); uwIndex++)
    {
        if ((m_aubWriteDataBuffer[uwIndex] != m_aubReadDataBuffer[uwIndex])?BCOV(1515):!BCOV(1516))
        {
            /* check failed */
            eProgStatus = PROG_E_NOT_OK;
        }
...
NCS_PRI_FCT tProgStatus PROG_GetLookUpIndex_Impl80(u16 uwRoutineId, u8 *ubTableRtneIndex)
{
    ...
    ICOV(1517);eProgStatus = PROG_E_NOT_OK;
    for (ubIndex = 0U; (ubIndex < PROG_ROUTINE_RESULT_NB)?BCOV(1518):!BCOV(1519); ubIndex++)
    {
        if ((stCfgRoutineResultTable[ubIndex].uwRoutineId == uwRoutineId)?BCOV(1520):!BCOV(1521))
        {
...
void PROG_UpdateRequestRoutineResult_Impl80(u16 uwRoutineId, tProgRequestRoutineResult eReqRtneResult)
{
    ...
    ICOV(1522);eProgStatus = PROG_GetLookUpIndex_Impl80(uwRoutineId, &ubIndexList);
    if ((PROG_E_OK == eProgStatus)?BCOV(1523):!BCOV(1524))
    {
...
NCS_PRI_FCT u8 PROG_GetRoutineResult_Impl80(u16 uwRoutineId)
{
    ...
    ICOV(1525);ubRet = PROG_REQUEST_ROUTINE_RESULT_INIT;
    /* Look for the corresponding index */
    eProgStatus = PROG_GetLookUpIndex_Impl80(uwRoutineId, &ubIndexList);
    if ((PROG_E_OK == eProgStatus)?BCOV(1526):!BCOV(1527))
    {
...
tUdsStatus PROG_GetRequestRoutineResult_Impl80(PduLengthType *puwLen, u8 *aubUdsData)
{
    ...
    ICOV(1528);eUdsStatus = UDS_NRC_31;

    if (((NULL_PTR != puwLen) && (NULL_PTR != aubUdsData))?BCOV(1529):!BCOV(1530))
    {
        ...
        if ((PROG_E_OK == eProgStatus)?BCOV(1531):!BCOV(1532))
        {
            aubUdsData[4U] = m_aubRequestRoutineResult[ubRtneIdInTable];
            *puwLen        = 5U;
            eUdsStatus     = UDS_ACK;

            if ((PROG_REQUEST_ROUTINE_RESULT_FAIL == m_aubRequestRoutineResult[ubRtneIdInTable])?BCOV(1533):!BCOV(1534))
            {
                /* If routine is erase */
                if ((PROG_IMPL80_ERASE_ROUTINE_ID == uwRoutineId)?BCOV(1535):!BCOV(1536))
                {
                    eUdsStatus = UDS_NRC_72;
                }

                /* If routine is self-check */
                if ((PROG_IMPL80_SELF_CHECK_ROUTINE_ID == uwRoutineId)?BCOV(1537):!BCOV(1538))
                {
                    aubUdsData[5U] = m_ubErrorType;
                    *puwLen        = 6U;
                }
            }
            else if ((PROG_REQUEST_ROUTINE_RESULT_INIT == m_aubRequestRoutineResult[ubRtneIdInTable])?BCOV(1539):!BCOV(1540))
            {
...
NCS_PRI_FCT u16 PROG_CalculateInversedCrc16_Impl80(const u8 *aubData, u16 uwLen, const u16 uwCrcInit)
{
    ...
    ICOV(1541);uwCrcValue = uwCrcInit;

    if((aubData != NULL_PTR)?BCOV(1542):!BCOV(1543))
    {
        for (uwIdx1 = 0U; (uwIdx1 < uwLen )?BCOV(1544):!BCOV(1545); uwIdx1++)
        {
            /* XOR(Crc,data byte) */
            uwCrcValue = (u16)(uwCrcValue ^ ((u16)aubData[uwIdx1]));

            for (uwIdx2 = 0U; (uwIdx2 < 8U)?BCOV(1546):!BCOV(1547); uwIdx2++)
            {
                if (( (u16)(uwCrcValue & 0x0001U) != 0U)?BCOV(1548):!BCOV(1549))
                {
                    /* XOR(CRC,Polynomial) */
