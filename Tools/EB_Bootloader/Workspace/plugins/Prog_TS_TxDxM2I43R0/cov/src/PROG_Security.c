...
{
    ICOV(2582);DBG_PROG_SECURITYINIT_ENTRY();
...
tProgBoolean PROG_SecurityPendingOperation(void)
{
    ...
    ICOV(2583);DBG_PROG_SECURITYPENDINGOPERATION_ENTRY();

    if ((PROG_NO_AUTHENT != m_ubBootAuthenticationPending)?BCOV(2584):!BCOV(2585))
    {
...
{
    ICOV(2586);DBG_PROG_SECURITYCHECKSUMNOTIFICATION_ENTRY();

#if (PROG_SECURE_BOOT_CHECKSUM_COMPUTATION == STD_ON)
    if ((PROG_BOOT_AUTHENT == m_ubBootAuthenticationPending)?BCOV(2587):!BCOV(2588))
    {
        /* Invalid checksum */
        ...
        if ((PROG_NO_EVENT != m_ubBootAuthenticationStoredEvent)?BCOV(2589):!BCOV(2590))
        {
            /* Post Event in state machine */
            PROG_HsmEmit(&PROG_HsmScPROG, m_ubBootAuthenticationStoredEvent);
        }
    }
    else if ((PROG_APP_AUTHENT == m_ubBootAuthenticationPending)?BCOV(2591):!BCOV(2592))
#endif
    {
#if (PROG_SECURE_APPLI_CHECKSUM_COMPUTATION == STD_ON)
#if (PROG_BLU_ACTIVE == STD_ON)
        if ((PROG_TRUE == m_ubBLUPattern)?BCOV(2593):!BCOV(2594))
        {
...
{
    ICOV(2595);DBG_PROG_SECURITYSTOREPENDINGEVENT_ENTRY(EventToStore);
...
{
    ICOV(2596);DBG_PROG_SECURITYCOMPUTEBOOTCHECKSUM_ENTRY();
...
tProgStatus PROG_GetComputedBootloaderChecksum(u8 * pubComputedChecksum)
{
    ...
    ICOV(2597);DBG_PROG_GETCOMPUTEDBOOTLOADERCHECKSUM_ENTRY(pubComputedChecksum);

    eValidBootloaderChecksum = PROG_CustomIsValidBootloaderChecksum();

    /* Verify checksum validity */
    if ((PROG_TRUE == eValidBootloaderChecksum)?BCOV(2598):!BCOV(2599))
    {
        /* Get checksum */
...
{
#if (PROG_SECURE_APPLI_CHECKSUM_COMPUTATION == STD_ON)
    ICOV(2600);DBG_PROG_SECURITYCOMPUTEAPPCHECKSUM_ENTRY();
    ...
    if((0 == m_stCurrentSegment.ubLogicalBlockId)?BCOV(2601):!BCOV(2602))
    {
    ...
    ICOV(2603);PROG_HsmEmit(&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_CRYPTO_FINISH);
#endif /*(PROG_IMPL_VARIANT != PROG_IMPL_4)|| (PROG_IMPL_VARIANT == PROG_IMPL_5)*/
...
void PROG_HsmMacGenerateStart(void)
{
    ...
    ICOV(2604);m_ubHsmCurrentStep = PROG_HSM_UPDATEINIT_BLOCK;
}
#endif /* PROG_HSM_MAC_USED */
...
void PROG_HsmManage(void)
{
    ...
    ICOV(2605);ubLocJobResult = PROG_E_BUSY;
    ...
    switch (m_ubHsmCurrentStep)
    {
        case PROG_HSM_UPDATEINIT_BLOCK:
#if (PROG_IMPL_VARIANT == PROG_IMPL_11)
            /* Deviation MISRAC2012-1 */
            ICOV(2606);pubLocMemoryAddress = (u8 *)m_ulBlockAddr;
            ...
            /* Deviation MISRAC2012-1 */
            ICOV(2607);pubLocMemoryAddress = (u8 *)(stCfgBlockSecureBootInfo[m_stCurrentSegment.ubLogicalBlockId].ulStartAddressForSecureBoot);
...
#else
            /* Deviation MISRAC2012-1 */
            ICOV(2608);pubLocMemoryAddress = (u8 *)(stCfgBlockSecureBootInfo[m_stCurrentSegment.ubSegmentId].ulStartAddressForSecureBoot);
...
#if (PROG_IMPL_VARIANT == PROG_IMPL_11)
            /* Deviation MISRAC2012-1 */
            ICOV(2610);pubLocMemoryAddress = (u8 *)m_ulBlockAddr;
            ulLocMemorySize = m_ulBlockLength;
...
#elif (PROG_CHECKMEMORY_TYPE == PROG_CHECKMEMORY_BY_BLOCK)
            /* Deviation MISRAC2012-1 */
            ICOV(2611);pubLocMemoryAddress = (u8 *)(stCfgBlockSecureBootInfo[m_stCurrentSegment.ubLogicalBlockId].ulStartAddressForSecureBoot);
            ulLocMemorySize = stCfgBlockSecureBootInfo[m_stCurrentSegment.ubLogicalBlockId].ulLengthForSecureBoot;
            ubLocJobResult = PROG_CustomHsmUpdateBlock(m_stCurrentSegment.ubLogicalBlockId,
                                                       pubLocMemoryAddress,
                                                       ulLocMemorySize);
#else
            /* Deviation MISRAC2012-1 */
            ICOV(2612);pubLocMemoryAddress = (u8 *)(stCfgBlockSecureBootInfo[m_stCurrentSegment.ubSegmentId].ulStartAddressForSecureBoot);
            ulLocMemorySize = stCfgBlockSecureBootInfo[m_stCurrentSegment.ubSegmentId].ulLengthForSecureBoot;
        ...
        case PROG_HSM_UPDATE_FINISH:
            /* update is OK */
            ICOV(2614);PROG_CustomHsmUpdateFinish(PROG_E_OK);
            PROG_SecurityChecksumNotification();
    ...
    if ((ubLocJobResult == PROG_E_OK)?BCOV(2616):!BCOV(2617))
    {
        /* go to the next step */
        m_ubHsmCurrentStep++;
    }
    else if ((ubLocJobResult == PROG_E_NOT_OK)?BCOV(2618):!BCOV(2619))
    {
        m_ubHsmCurrentStep = PROG_HSM_IDLE;
        /* report failure */
