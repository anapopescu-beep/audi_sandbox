...
/*----------------------------------{UDS_RxRequest}--------------------------------*/
tUdsBoolean UDS_RxRequest (PduLengthType *pulLen, u8 *aubUdsData)
{
    tUdsBoolean eUdsBoolean;

    DBG_UDS_RXREQUEST_ENTRY(pulLen,aubUdsData);
    ...
    else
    {
        ...
        eUdsBoolean = UDS_RxRequestWithAddrMode (pulLen, aubUdsData, UDS_ADDR_PHYSICAL, UDS_TYPE_DIAG_OBD);
    }
...
tUdsBoolean UDS_RxRequestWithAddrMode (PduLengthType *pulLen, u8 *aubUdsData, tUdsAddrMode eUdsAddrMode, tUdsAddrType eUdsAddrType)
{
    tUdsBoolean eUdsBoolean;

    DBG_UDS_RXREQUESTWITHADDRMODE_ENTRY(pulLen,aubUdsData,eUdsAddrMode,eUdsAddrType);
...
NCS_PRI_FCT tUdsStatus UDS_RdbiManagement(PduLengthType *pulLen, u8 *aubUdsData)
{
    ...
    u8 aubLocalRDBIBuffer[RDBI_MAX_RESPONSE_LENGTH] = {0};
    ...
    if (((*pulLen >= 3U) &&
        (((*pulLen - 1U) % 2U) == 0U)) &&
        (*pulLen <= (1U + (2U*UDS_SRV_CFG_RDBI_MAX))))
    {
        ...
        if(eUdsStatus != UDS_NRC_33)
        {
            ...
            if (0U != ulTableCfgLength)
            {
                ...
                while (((ulIdx < ulTableCfgLength) && (UDS_ACK == eUdsStatus)) && (*pulLen > 0U))
                {
                    ...
                    aubLocalRDBIBuffer[0U] = UDS_SRV_RDBI;
                    aubLocalRDBIBuffer[1U] = (u8)((u8)(m_astDiagSrvCfg2[ubLocalDidIdx].uwDid >> 8U) & 0xFFU);
                    aubLocalRDBIBuffer[2U] = (u8)(m_astDiagSrvCfg2[ubLocalDidIdx].uwDid & 0xFFU);
                    if (m_astDiagSrvCfg2[ubLocalDidIdx].pfuRxMsgInd != NULL_PTR)
                    {
                        ...
                        *pulLen = 3U;
                        ...
                        /* Individual callback for each service */
                        eUdsStatus =
                            m_astDiagSrvCfg2[ubLocalDidIdx].pfuRxMsgInd(pulLen, &aubLocalRDBIBuffer[0]);
                    ...
                    if (*pulLen > 0U)
                    {
                        ...
                        else
                        {
                          ...
                          {
                              aubUdsData[ulLocalLength+ulLocalRDBILength] = aubLocalRDBIBuffer[ulLocalRDBILength];
                          }
